 ...
 * nD processes is assumed. We further assume that the total number of processes
 * can be divided by the number of processes required by each MD simulation.
...
private:
  ...
  // number of MPI processes needs to be a multiple of the number of processes
  // per MD simulation (=product of the vector components)
  ...
  // want to run 26 MD simulations, then this value is given by 26/(12/4) = 26/3
  // = 8
  unsigned int _avgNumberMDSimulationsPerLocalComm;
  // number of MD simulations for this current local communicator. Except for
  // the "last communicator", this value equals
  // _avgNumberMDSimulationsPerLocalComm. The last comunicator group fills up
  // the missing MD simulations. If we have 4 processes per MD simulation and 12
  // processes available and want to run 26 MD simulations, then we have 12/4=3
  // communicator groups, of which group 0 and 1 handle 26/3=8 MD simulations.
  // The last group 2 handles 26-2*8 = 10 MD simulations.
