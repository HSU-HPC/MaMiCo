...
static const Entity entities[NUM_ENTITIES] = {
  { "quot", 4, DOUBLE_QUOTE }, { "amp", 3, '&' }, { "apos", 4, SINGLE_QUOTE },
  { "lt", 2, '<' }, { "gt", 2, '>' }
};
...
char *XMLText::ParseDeep(char *p, StrPair *) {
    ...
    if (this->CData()) {
        ...
        int flags =
            _document->ProcessEntities() ? StrPair::TEXT_ELEMENT
...
void XMLPrinter::Print(const char * format, ...) {
    ...
    if (_fp) {
        ...
        int expand = 1000;
        while (len < 0) {
            len = vsnprintf_s(_accumulator.Mem(), _accumulator.Capacity(),
                              _TRUNCATE, format, va);
            if (len < 0) {
                expand *= 3 / 2;
