...
private:
   ...
   * this case.
   *  If the value is not specified in the configuration, a default value of 1
   * is used. */
  unsigned int _insertDeleteMassEveryTimestep;

  /** r_sigma value. In the USHER paper, this value resembles the respective
   * parameter for choosing the stepsize
   *  when two particles overlap. In the paper, it is stated that it should be
   * close to unity (in sigma units; the
   *  parameter here is also used in sigma units of the respective molecules)
   * and is chosen to be 0.9. If the parameter
   ...
   * right energy level.
   *  From the USHER paper (Usher - An algorithm for particle insertion in dense
   * fluids), this
   *  coefficient is optimally chosen to be 0.1 (in sigma units; this
   * configuration also determines
   *  this parameter in sigma units). Thus, the parameter is set to 0.1, if it
   * is not defined
   ...
   * distance from an outer boundary.
   *  This is useful since open boundary forces are not considered in the
   * current USHER implementation. Inserting a particle
   *  close to an outer open boundary may thus result in very strong forces and,
   * thus, yield instabilities.
   */
