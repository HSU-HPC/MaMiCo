...
namespace coupling {
namespace configurations { class MomentumInsertionConfiguration; }
}
    ...
    if (_insertionType == VELOCITY_GRADIENT_RELAXATION) {
      ...
      if ((_velocityRelaxationFactor <= 0.0) ||
        ...
        std::cout << "ERROR coupling::MomentumInsertionConfiguration: "
                     "velocity-relaxation-factor=" << _velocityRelaxationFactor
                  << "!";
      ...
      if (buf <= 0) {
        std::cout << "ERROR coupling::MomentumInsertionConfiguration: "
                     "innermost-overlap-layer=" << buf << "!" << std::endl;
        _isValid = false;
        exit(EXIT_FAILURE);
      }
      ...
      if (((unsigned int) buf > _innerOverlap) || (buf <= 0)) {
        std::cout << "ERROR coupling::MomentumInsertionConfiguration: "
                     "outermost-overlap-layer=" << buf << "!" << std::endl;
        _isValid = false;
        exit(EXIT_FAILURE);
      }
