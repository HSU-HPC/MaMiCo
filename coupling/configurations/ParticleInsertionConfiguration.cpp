...
void coupling::configurations::ParticleInsertionConfiguration::
parseSubtag( tinyxml2::XMLElement* node ){
  ...
  if (_tolerance <= 0.0){
    std::cout << "ERROR coupling::configurations::ParticleInsertionConfiguration::parseSubtag(): " << TOLERANCE << " is smaller than or equal zero!" << std::endl;
    exit(EXIT_FAILURE);
  }
  ...
  // if not defined, set offset for particle insertions from outer boundary to 0.0; may yield instabilities for dense fluids
