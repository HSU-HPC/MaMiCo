...
public:
  ...
  void parseSubtag(tinyxml2::XMLElement* node) {
    ...
    // shall be applied this may not be useful for NO_BOUNDARYFORCE; however,
    // once read, the user can still use the flags, even in the NO_BOUNDARYFORCE
    ...
    if (_insertionType == ZHOU) {
      if (dim != 3) {
        std::cout << "ERROR "
                     ...
                     "interpreteConfiguration(): Zhou boundary force only "
                     "valid in 3D!"
                  << std::endl;
        exit(EXIT_FAILURE);
      }
