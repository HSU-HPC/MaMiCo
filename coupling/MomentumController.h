...
class coupling::MomentumController {
  public:
    ...
    /** @brief computes and returns the momentum and mean velocity of a macroscopic cell
     *  @param cell the macroscopic cell, for which the values shall be calculated
     *  @param momentum vector to which the momentum of the cell will be written to
     *  @param meanVelocity vector to which the mean velocity of the cell will be written to*/
    ...
    /** This cell does not consider the kinetic energy of the system, so the kinetic energy will
     *  be different after executing this method! In order to also retain the kinetic energy,
     *  one might compute the kinetic energy before calling this method (using the KineticEnergyController)
     *  and set the old kinetic energy afterwards (again using the KineticEnergyController).
     *  This will then set the right kinetic energy again, conserving both mass and momentum over this cell.
    ...
    void setMomentum(
      ...
      coupling::cellmappings::SetMomentumMapping<LinkedCell,dim> setMomentumMapping(
        currentMomentum,newMomentum,numberParticles,
        _mdSolverInterface
      );
