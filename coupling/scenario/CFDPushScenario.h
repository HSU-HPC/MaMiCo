...
#ifndef _COUPLING_SCENARIO_CFDPUSHSCENARIO_H_
#define _COUPLING_SCENARIO_CFDPUSHSCENARIO_H_
...
public:
  ...
  void equilibrateMicro() override {
    ...
    for (auto pair : _couplingBuffer.md2macroBuffer)
      buffer << pair;
    fillSendBuffer(_cfg.density, buffer);
...
protected:
  ...
  void advanceMacro(int cycle) {
    if (_couetteSolver != NULL) {
      ...
      fillSendBuffer(_cfg.density, _couplingBuffer.macro2MDBuffer);
    }
    if (_cfg.macro2Md) {
