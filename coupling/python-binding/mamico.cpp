...
template <class T>
T *makeConfiguration(const std::string filename,
                     const std::string topleveltag) {
  T *cfg = new T{}; // corresponding delete will be called by python, because of
                    // return_value_policy::take_ownership
  tarch::configuration::ParseConfiguration::parseConfiguration<T>(
      filename, topleveltag, *cfg);
  return cfg;
}
...
public:
  ...
  static handle cast(Vec3ui src, return_value_policy /* policy */,
                     handle /* parent */) {
    return PyTuple_Pack(3, PyLong_FromUnsignedLong(src[0]),
                        PyLong_FromUnsignedLong(src[1]),
                        PyLong_FromUnsignedLong(src[2]));
  }
};
} // namespace detail
} // namespace pybind11
...
public:
  ...
  /*** Conversion part 2 (C++ -> Python)  */
  static handle cast(Vec3d src, return_value_policy /* policy */,
                     handle /* parent */) {
    return PyTuple_Pack(3, PyFloat_FromDouble(src[0]),
                        PyFloat_FromDouble(src[1]), PyFloat_FromDouble(src[2]));
  }
};
} // namespace detail
} // namespace pybind11
...
public:
  CouplingBuffer(const coupling::IndexConversion<3> &indexConversion,
      ...
      : _sendBuffer(), _globalCellIndices4SendBuffer(), _recvBuffer(),
        _globalCellIndices4RecvBuffer(), _idcv(indexConversion),
  ...
  void recv2CSV(const std::string filename) const {
    ...
    for (unsigned int i = 0; i < numCellsRecv; i++) {
      tarch::la::Vector<3, double> vel(
          _recvBuffer[i]->getMacroscopicMomentum());
      ...
      file << counter[0] << " ; " << counter[1] << " ; " << counter[2] << " ; "
           << vel[0] << " ; " << vel[1] << " ; " << vel[2] << " ; "
           << _recvBuffer[i]->getMacroscopicMass() << ";";
      file << std::endl;
    }
    file.close();
  }

  void store2send(const double cellmass, py::array_t<double> velocity,
    ...
    for (unsigned int d = 0; d < 3; d++) {
      if (velocity.shape(d) != numcells[d])
        throw std::runtime_error("velocity.shape(" + std::to_string(d) +
                                 ") should be " + std::to_string(numcells[d]) +
                                 " but is " +
                                 std::to_string(velocity.shape(d)));
      if (density.shape(d) != numcells[d])
        throw std::runtime_error("density.shape(" + std::to_string(d) +
                                 ") should be " + std::to_string(numcells[d]) +
    ...
    for (unsigned int i = 0; i < size; i++) {
      const tarch::la::Vector<3, unsigned int> globalIndex(
          _idcv.getGlobalVectorCellIndex(_globalCellIndices4SendBuffer[i]) -
      ...
      const tarch::la::Vector<3, double> momentum(
          mass *
              velocity_raw(globalIndex[0], globalIndex[1], globalIndex[2], 0),
          mass *
              velocity_raw(globalIndex[0], globalIndex[1], globalIndex[2], 1),
          mass *
              velocity_raw(globalIndex[0], globalIndex[1], globalIndex[2], 2));
  ...
  py::array_t<double> *loadRecvVelocity() {
    auto numcells = _idcv.getGlobalNumberMacroscopicCells() -
                    tarch::la::Vector<3, unsigned int>(2 * _outerRegion);
  ...
  py::array_t<double> *loadRecvDensity(double cellmass) {
    ...
    for (unsigned int i = 0; i < numCellsRecv; i++) {
      const tarch::la::Vector<3, unsigned int> idx(
          _idcv.getGlobalVectorCellIndex(_globalCellIndices4RecvBuffer[i]) -
          tarch::la::Vector<3, unsigned int>(_outerRegion + 1));
      res_raw(idx[0], idx[1], idx[2]) =
          _recvBuffer[i]->getMacroscopicMass() / cellmass;
    }
...
private:
  ...
  void allocateSendBuffer(const coupling::IndexConversion<3> &indexConversion,
    ...
    for (unsigned int i = 0; i < num; i++) {
      ...
      // solver process
      if (macroscopicSolverInterface.sendMacroscopicQuantityToMDSolver(
              indexConversion.getGlobalVectorCellIndex(i))) {
        std::vector<unsigned int> ranks =
            macroscopicSolverInterface.getSourceRanks(
                indexConversion.getGlobalVectorCellIndex(i));
  ...
  void allocateRecvBuffer(const coupling::IndexConversion<3> &indexConversion,
    ...
    for (unsigned int i = 0; i < num; i++) {
      if (macroscopicSolverInterface.receiveMacroscopicQuantityFromMDSolver(
              indexConversion.getGlobalVectorCellIndex(i))) {
        std::vector<unsigned int> ranks =
            macroscopicSolverInterface.getTargetRanks(
                indexConversion.getGlobalVectorCellIndex(i));
        bool containsThisRank = false;
        for (unsigned int k = 0; k < ranks.size(); k++) {
          containsThisRank = containsThisRank || (ranks[k] == rank);
  ...
  /** deletes the buffer */
  void deleteBuffer(
      std::vector<coupling::datastructures::MacroscopicCell<3> *> &buffer) {
...
PYBIND11_MODULE(mamico, mamico) {
  ...
  py::module coupling = mamico.def_submodule(
      "coupling", "Contains the coupling tool and interface implementations");
  py::module tarch = mamico.def_submodule(
      "tarch", "Contains helper classes and files, e.g. for XML parsing");
  py::module simplemd =
      mamico.def_submodule("simplemd", "Contains the MD simulation SimpleMD");

  py::module configuration =
      tarch.def_submodule("configuration", "XML configuration utils");
  ...
  utils.def("initMPI", &initMPI,
            "Calls MPI_Init and returns rank of this process");
  ...
  py::class_<simplemd::configurations::MolecularDynamicsConfiguration>(
      configuration, "MolecularDynamicsConfiguration")
      .def("__repr__",
           [](const simplemd::configurations::MolecularDynamicsConfiguration
                  &c) {
             return "<MolecularDynamicsConfiguration read from XML tag \"" +
                    c.getTag() + "\">";
           })
      .def("isValid",
           &simplemd::configurations::MolecularDynamicsConfiguration::isValid)
      .def("getDomainConfiguration",
           &simplemd::configurations::MolecularDynamicsConfiguration::
               getDomainConfiguration,
           py::return_value_policy::reference)
      .def("getMoleculeConfiguration",
           &simplemd::configurations::MolecularDynamicsConfiguration::
               getMoleculeConfiguration,
           py::return_value_policy::reference)
      .def("getMPIConfiguration",
           &simplemd::configurations::MolecularDynamicsConfiguration::
               getMPIConfiguration,
           py::return_value_policy::reference)
      .def("getVTKConfiguration",
           &simplemd::configurations::MolecularDynamicsConfiguration::
               getVTKConfiguration,
           py::return_value_policy::reference)
      .def("getSimulationConfiguration",
           &simplemd::configurations::MolecularDynamicsConfiguration::
               getSimulationConfiguration,
           py::return_value_policy::reference)
      .def("getRDFConfiguration",
           &simplemd::configurations::MolecularDynamicsConfiguration::
               getRDFConfiguration,
           py::return_value_policy::reference)
      .def("getCheckpointConfiguration",
           &simplemd::configurations::MolecularDynamicsConfiguration::
               getCheckpointConfiguration,
           py::return_value_policy::reference)
      .def("getProfilePlotterConfigurations",
           &simplemd::configurations::MolecularDynamicsConfiguration::
               getProfilePlotterConfigurations,
           py::return_value_policy::reference)
      .def("getExternalForceConfigurations",
           &simplemd::configurations::MolecularDynamicsConfiguration::
               getExternalForceConfigurations,
           py::return_value_policy::reference);

  py::class_<simplemd::configurations::DomainConfiguration>(
      configuration, "DomainConfiguration")
      .def("getMoleculesPerDirection",
           &simplemd::configurations::DomainConfiguration::
               getMoleculesPerDirection)
      .def("getGlobalDomainSize",
           &simplemd::configurations::DomainConfiguration::getGlobalDomainSize)
      .def(
          "getGlobalDomainOffset",
          &simplemd::configurations::DomainConfiguration::getGlobalDomainOffset)
      .def("getCutoffRadius",
           &simplemd::configurations::DomainConfiguration::getCutoffRadius)
      .def("getMeshWidth",
           &simplemd::configurations::DomainConfiguration::getMeshWidth)
      .def("getKB", &simplemd::configurations::DomainConfiguration::getKB)
      .def("getBlockSize",
           &simplemd::configurations::DomainConfiguration::getBlockSize)
      .def("getBoundary",
           &simplemd::configurations::DomainConfiguration::getBoundary)
      .def(
          "getCheckpointFilestem",
          &simplemd::configurations::DomainConfiguration::getCheckpointFilestem)
      .def("initFromCheckpoint",
           &simplemd::configurations::DomainConfiguration::initFromCheckpoint)
      .def("initFromSequentialCheckpoint",
           &simplemd::configurations::DomainConfiguration::
               initFromSequentialCheckpoint);

  py::class_<simplemd::configurations::MoleculeConfiguration>(
      configuration, "MoleculeConfiguration")
      .def("getMeanVelocity",
           &simplemd::configurations::MoleculeConfiguration::getMeanVelocity)
      .def("getTemperature",
           &simplemd::configurations::MoleculeConfiguration::getTemperature)
      .def("getMass", &simplemd::configurations::MoleculeConfiguration::getMass)
      .def("getEpsilon",
           &simplemd::configurations::MoleculeConfiguration::getEpsilon)
      .def("getSigma",
           &simplemd::configurations::MoleculeConfiguration::getSigma);

  py::class_<simplemd::configurations::MPIConfiguration>(configuration,
                                                         "MPIConfiguration")
      .def("getNumberOfProcesses",
           &simplemd::configurations::MPIConfiguration::getNumberOfProcesses);

  py::class_<simplemd::configurations::VTKConfiguration>(configuration,
                                                         "VTKConfiguration")
      .def("getFilename",
           &simplemd::configurations::VTKConfiguration::getFilename)
      .def("getWriteEveryTimestep",
           &simplemd::configurations::VTKConfiguration::getWriteEveryTimestep);

  py::class_<simplemd::configurations::SimulationConfiguration>(
      configuration, "SimulationConfiguration")
      .def("getDt", &simplemd::configurations::SimulationConfiguration::getDt)
      .def("getNumberOfTimesteps",
           &simplemd::configurations::SimulationConfiguration::
               getNumberOfTimesteps)
      .def("getReorganiseMemoryEveryTimestep",
           &simplemd::configurations::SimulationConfiguration::
               getReorganiseMemoryEveryTimestep)
      .def("computeMacroscopicQuantitiesEveryTimestep",
           &simplemd::configurations::SimulationConfiguration::
               computeMacroscopicQuantitiesEveryTimestep)
      .def("fixSeed",
           &simplemd::configurations::SimulationConfiguration::fixSeed)
      .def("useOverlappingCommunicationWithForceComputation",
           &simplemd::configurations::SimulationConfiguration::
               useOverlappingCommunicationWithForceComputation);

  py::class_<simplemd::configurations::RDFConfiguration>(configuration,
                                                         "RDFConfiguration")
      .def("isDefined", &simplemd::configurations::RDFConfiguration::isDefined)
      .def("getStartAtTimestep",
           &simplemd::configurations::RDFConfiguration::getStartAtTimestep)
      .def(
          "getEvaluateEveryTimestep",
          &simplemd::configurations::RDFConfiguration::getEvaluateEveryTimestep)
      .def("getWriteEveryTimestep",
           &simplemd::configurations::RDFConfiguration::getWriteEveryTimestep)
      .def("getNumberOfPoints",
           &simplemd::configurations::RDFConfiguration::getNumberOfPoints);

  py::class_<simplemd::configurations::CheckpointConfiguration>(
      configuration, "CheckpointConfiguration");

  py::class_<simplemd::configurations::ProfilePlotterConfiguration>(
      configuration, "ProfilePlotterConfiguration")
      .def("getStartCell",
           &simplemd::configurations::ProfilePlotterConfiguration::getStartCell)
      .def("getRange",
           &simplemd::configurations::ProfilePlotterConfiguration::getRange)
      .def("getWriteEveryTimestep",
           &simplemd::configurations::ProfilePlotterConfiguration::
               getWriteEveryTimestep)
      .def("getSampleEveryTimestep",
           &simplemd::configurations::ProfilePlotterConfiguration::
               getSampleEveryTimestep)
      .def("getStartAtTimestep",
           &simplemd::configurations::ProfilePlotterConfiguration::
               getStartAtTimestep);

  py::class_<simplemd::configurations::ExternalForceConfiguration>(
      configuration, "ExternalForceConfiguration")
  ...
  py::class_<coupling::configurations::MaMiCoConfiguration<3>>(
      configuration, "MaMiCoConfiguration")
      .def("__repr__",
      ...
      .def("isValid",
           &coupling::configurations::MaMiCoConfiguration<3>::isValid)
      .def("getMacroscopicCellConfiguration",
           &coupling::configurations::MaMiCoConfiguration<
               3>::getMacroscopicCellConfiguration,
           py::return_value_policy::reference)
      .def("getParticleInsertionConfiguration",
           &coupling::configurations::MaMiCoConfiguration<
               3>::getParticleInsertionConfiguration,
           py::return_value_policy::reference)
      .def("getMomentumInsertionConfiguration",
           &coupling::configurations::MaMiCoConfiguration<
               3>::getMomentumInsertionConfiguration,
           py::return_value_policy::reference)
      .def("getBoundaryForceConfiguration",
           &coupling::configurations::MaMiCoConfiguration<
               3>::getBoundaryForceConfiguration,
           py::return_value_policy::reference)
      .def("getTransferStrategyConfiguration",
           &coupling::configurations::MaMiCoConfiguration<
               3>::getTransferStrategyConfiguration,
           py::return_value_policy::reference)
      .def("getParallelTopologyConfiguration",
           &coupling::configurations::MaMiCoConfiguration<
               3>::getParallelTopologyConfiguration,
           py::return_value_policy::reference)
      .def("getThermostatConfiguration",
           &coupling::configurations::MaMiCoConfiguration<
               3>::getThermostatConfiguration,
           py::return_value_policy::reference);

  py::class_<coupling::configurations::MacroscopicCellConfiguration<3>>(
      configuration, "MacroscopicCellConfiguration")
      .def("getMacroscopicCellSize",
           &coupling::configurations::MacroscopicCellConfiguration<
               3>::getMacroscopicCellSize)
      .def("getNumberLinkedCellsPerMacroscopicCell",
           &coupling::configurations::MacroscopicCellConfiguration<
               3>::getNumberLinkedCellsPerMacroscopicCell)
      .def("getWriteEveryMicroscopicTimestep",
           &coupling::configurations::MacroscopicCellConfiguration<
               3>::getWriteEveryMicroscopicTimestep)
      .def("getMicroscopicFilename",
           &coupling::configurations::MacroscopicCellConfiguration<
               3>::getMicroscopicFilename)
      .def("getWriteEveryMacroscopicTimestep",
           &coupling::configurations::MacroscopicCellConfiguration<
               3>::getWriteEveryMacroscopicTimestep)
      .def("getMacroscopicFilename",
           &coupling::configurations::MacroscopicCellConfiguration<
               3>::getMacroscopicFilename);

  py::class_<coupling::configurations::ParticleInsertionConfiguration>(
      configuration, "ParticleInsertionConfiguration")
      .def("getParticleInsertionType",
           &coupling::configurations::ParticleInsertionConfiguration::
               getParticleInsertionType);

  py::class_<coupling::configurations::MomentumInsertionConfiguration>(
      configuration, "MomentumInsertionConfiguration")
      .def("getMomentumInsertionType",
  ...
  py::class_<coupling::configurations::BoundaryForceConfiguration<3>>(
      configuration, "BoundaryForceConfiguration")
      .def("getBoundaryForceType",
           &coupling::configurations::BoundaryForceConfiguration<
               3>::getBoundaryForceType);

  py::class_<coupling::configurations::TransferStrategyConfiguration<3>>(
      configuration, "TransferStrategyConfiguration")
      .def("getStrategyType",
           &coupling::configurations::TransferStrategyConfiguration<
               3>::getStrategyType);

  py::class_<coupling::configurations::ParallelTopologyConfiguration>(
      configuration, "ParallelTopologyConfiguration")
      .def("getParallelTopologyType",
           &coupling::configurations::ParallelTopologyConfiguration::
               getParallelTopologyType);

  py::class_<coupling::configurations::ThermostatConfiguration>(
      configuration, "ThermostatConfiguration")
      .def("getThermostatRegionType",
           &coupling::configurations::ThermostatConfiguration::
               getThermostatRegionType)
      .def("getCells2Use",
           &coupling::configurations::ThermostatConfiguration::getCells2Use);

  configuration.def(
      "parseMolecularDynamicsConfiguration",
      &makeConfiguration<
          simplemd::configurations::MolecularDynamicsConfiguration>,
      py::return_value_policy::take_ownership);
  configuration.def(
      "parseMaMiCoConfiguration",
      &makeConfiguration<coupling::configurations::MaMiCoConfiguration<3>>,
      py::return_value_policy::take_ownership);
  ...
  py::class_<tarch::utils::MultiMDService<3>>(utils, "MultiMDService")
      ...
      .def("getLocalNumberOfMDSimulations",
           &tarch::utils::MultiMDService<3>::getLocalNumberOfMDSimulations)
      .def("getGlobalNumberOfLocalMDSimulation",
           &tarch::utils::MultiMDService<3>::getGlobalNumberOfLocalMDSimulation)
      .def("getLocalCommunicator",
           [](const tarch::utils::MultiMDService<3> &s) {
             return (void *)(s.getLocalCommunicator());
           });

  py::class_<coupling::interface::MDSimulation>(coupling, "MDSimulation")
      .def("init", (void(coupling::interface::MDSimulation::*)(
                       const tarch::utils::MultiMDService<3> &, unsigned int)) &
                       coupling::interface::MDSimulation::init)
      .def("switchOffCoupling",
           &coupling::interface::MDSimulation::switchOffCoupling)
      .def("switchOnCoupling",
           &coupling::interface::MDSimulation::switchOnCoupling)
      .def("simulateTimesteps",
           &coupling::interface::MDSimulation::simulateTimesteps,
           "numberTimesteps"_a = 1, "firstTimestep"_a)
      .def("setMacroscopicCellService",
           &coupling::interface::MDSimulation::setMacroscopicCellService)
      .def("shutdown", &coupling::interface::MDSimulation::shutdown);

  py::class_<coupling::services::MacroscopicCellService<3>>(
      services, "MacroscopicCellService")
      .def("computeAndStoreTemperature",
           &coupling::services::MacroscopicCellService<
               3>::computeAndStoreTemperature)
      .def("getIndexConversion",
           &coupling::services::MacroscopicCellService<3>::getIndexConversion,
           py::return_value_policy::reference)
      .def("plotEveryMacroscopicTimestep",
           &coupling::services::MacroscopicCellService<
               3>::plotEveryMacroscopicTimestep)
      .def(
          "addFilterToSequence",
          [](coupling::services::MacroscopicCellService<3> *service,
             const char *filter_sequence, int filter_index,
             // use of std::optional is neccessary because pybind11 doesnt
             // support implicit None conversion for STL datatypes
             std::optional<
                 std::function<py::array_t<double>(py::array_t<double>)>>
                 scalar_filter_func,
             std::optional<
                 std::function<py::array_t<double>(py::array_t<double>)>>
                 vector_filter_func) {
            std::function<py::array_t<double>(py::array_t<double>)> *sf_ptr =
                nullptr;
            if (scalar_filter_func.has_value())
              sf_ptr = &(scalar_filter_func.value());

            std::function<py::array_t<double>(py::array_t<double>)> *vf_ptr =
                nullptr;
          ...
          "filter_sequence"_a, "filter_index"_a,
  ...
  coupling.def(
      ...
      [](const simplemd::configurations::MolecularDynamicsConfiguration &c1,
         ...
         void *comm) {
        return coupling::interface::SimulationAndInterfaceFactory::getInstance()
            .getMDSimulation(c1, c2, (MPI_Comm)comm);
      },
      "simpleMDConfig"_a, "mamicoConfig"_a, "localComm"_a,
      py::return_value_policy::take_ownership);

  py::class_<coupling::interface::MDSolverInterface<MY_LINKEDCELL, 3>>(
      interface, "MDSolverInterface");
  ...
  coupling.def(
      ...
      [](const simplemd::configurations::MolecularDynamicsConfiguration &c1,
        ...
        return coupling::interface::SimulationAndInterfaceFactory::getInstance()
            .getMDSolverInterface(c1, c2, sim);
      },
      py::return_value_policy::take_ownership);

  py::class_<coupling::interface::MacroscopicSolverInterface<3>> maSoIf(
      interface, "MacroscopicSolverInterface");

  // we indicate that CouetteSolverInterface is a subclass of
  // MacroscopicSolverInterface by passing maSoIf here
  py::class_<coupling::solvers::CouetteSolverInterface<3>>(
      solvers, "CouetteSolverInterface", maSoIf)
  ...
  coupling.def("setMacroscopicSolverInterface",
               [](coupling::interface::MacroscopicSolverInterface<3> *iface) {
                 coupling::interface::MamicoInterfaceProvider<MY_LINKEDCELL,
                                                              3>::getInstance()
                     .setMacroscopicSolverInterface(iface);
               });
  coupling.def(
      "getMacroscopicSolverInterface",
      []() {
        return coupling::interface::MamicoInterfaceProvider<MY_LINKEDCELL,
                                                            3>::getInstance()
            .getMacroscopicSolverInterface();
      },
      py::return_value_policy::reference);
  ...
  coupling.def("setMacroscopicCellService",
               [](coupling::services::MacroscopicCellService<3> *service) {
                 coupling::interface::MamicoInterfaceProvider<MY_LINKEDCELL,
                                                              3>::getInstance()
                     .setMacroscopicCellService(service);
               });

  py::class_<coupling::services::MultiMDCellService<MY_LINKEDCELL, 3>>(
      services, "MultiMDCellService")
      .def(
          py::init(
              [](std::vector<
                     coupling::interface::MDSolverInterface<MY_LINKEDCELL, 3> *>
                     mdSolverInterfaces,
                 coupling::interface::MacroscopicSolverInterface<3>
                     *macroscopicSolverInterface,
                 const simplemd::configurations::MolecularDynamicsConfiguration
                     &simpleMDConfig,
                 unsigned int rank, unsigned int totalNumberMDSimulations,
                 const coupling::configurations::MaMiCoConfiguration<3>
                     &mamicoConfig,
                 const std::string filterPipelineConfiguration,
                 const tarch::utils::MultiMDService<3> &multiMDService) {
                return new coupling::services::MultiMDCellService<MY_LINKEDCELL,
                                                                  3>(
                    mdSolverInterfaces, macroscopicSolverInterface,
                    simpleMDConfig.getMPIConfiguration().getNumberOfProcesses(),
                    rank, totalNumberMDSimulations,
          ...
          "mdSolverInterfaces"_a, "macroscopicSolverInterface"_a,
          "simpleMDConfig"_a, "rank"_a = 0, "totalNumberMDSimulations"_a = 1,
          "mamicoConfig"_a, "xmlConfigFilename"_a, "multiMDService"_a)
      .def(
          "getMacroscopicCellService",
          &coupling::services::MultiMDCellService<MY_LINKEDCELL,
                                                  3>::getMacroscopicCellService,
          py::return_value_policy::reference)
      .def("sendFromMacro2MD",
           [](coupling::services::MultiMDCellService<MY_LINKEDCELL, 3> &service,
              CouplingBuffer &buf) {
             service.sendFromMacro2MD(buf._sendBuffer,
                                      buf._globalCellIndices4SendBuffer);
           })
      .def("sendFromMD2Macro",
           [](coupling::services::MultiMDCellService<MY_LINKEDCELL, 3> &service,
              CouplingBuffer &buf) {
             service.sendFromMD2Macro(buf._recvBuffer,
                                      buf._globalCellIndices4RecvBuffer);
           });
  ...
  py::class_<CouplingBuffer>(coupling, "Buffer")
      ...
      .def("loadRecvVelocity", &CouplingBuffer::loadRecvVelocity,
           py::return_value_policy::take_ownership)
      .def("loadRecvDensity", &CouplingBuffer::loadRecvDensity,
           py::return_value_policy::take_ownership);
}
