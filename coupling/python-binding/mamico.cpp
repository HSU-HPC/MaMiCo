...
T *makeConfiguration(const std::string filename,
  ...
  T *cfg = new T {}
  ; // corresponding delete will be called by python, because of
...
public:
  ...
  void recv2CSV(const std::string filename) const {
    ...
    for (unsigned int i = 0; i < numCellsRecv; i++) {
      tarch::la::Vector<3, double> vel(_recvBuffer[i]
...
PYBIND11_MODULE(mamico, mamico) {
  ...
  py::class_<coupling::configurations::MaMiCoConfiguration<3> >(
    ...
    return "<MaMiCoConfiguration read from XML tag \"" + c.getTag() + "\">";
      ...
      .def("getTransferStrategyConfiguration",
           ...
           py::return_value_policy::reference)
      .def("getNoiseReductionConfiguration",
           &coupling::configurations::MaMiCoConfiguration<
               3>::getNoiseReductionConfiguration,
           py::return_value_policy::reference)
  ...
  py::class_<coupling::configurations::NoiseReductionConfiguration>(
      configuration, "NoiseReductionConfiguration")
      .def("getNoiseReductionType",
           &coupling::configurations::NoiseReductionConfiguration::
               getNoiseReductionType);
  ...
  coupling.def("setMacroscopicSolverInterface",
    ...
    coupling::interface::MamicoInterfaceProvider<
        MY_LINKEDCELL, 3>::getInstance().setMacroscopicSolverInterface(iface);
  });
  coupling.def("getMacroscopicSolverInterface", []() {
    return coupling::interface::MamicoInterfaceProvider<
        MY_LINKEDCELL, 3>::getInstance().getMacroscopicSolverInterface();
  },
               py::return_value_policy::reference);
  coupling.def(
    ...
    coupling::interface::MamicoInterfaceProvider<
        MY_LINKEDCELL, 3>::getInstance().setMDSolverInterface(iface);
  });
  ...
  coupling.def("setMacroscopicCellService",
    ...
    coupling::interface::MamicoInterfaceProvider<
        MY_LINKEDCELL, 3>::getInstance().setMacroscopicCellService(service);
  });

  py::class_<coupling::services::MultiMDCellService<MY_LINKEDCELL, 3> >(
      services, "MultiMDCellService")
      .def(py::init([](
               std::vector<coupling::interface::MDSolverInterface<
                   MY_LINKEDCELL, 3> *> mdSolverInterfaces,
               coupling::interface::MacroscopicSolverInterface<3> *
                   macroscopicSolverInterface,
               const simplemd::configurations::MolecularDynamicsConfiguration &
                   simpleMDConfig,
    ...
    return new coupling::services::MultiMDCellService<MY_LINKEDCELL, 3>(
        ...
        simpleMDConfig.getMPIConfiguration().getNumberOfProcesses(), rank,
        ...
        mamicoConfig.getMacroscopicCellConfiguration(), filename.c_str(),
        multiMDService);
  }),
      ...
      .def("getMacroscopicCellService",
           &coupling::services::MultiMDCellService<
               MY_LINKEDCELL, 3>::getMacroscopicCellService,
           py::return_value_policy::reference)
      .def(
          "sendFromMacro2MD",
