...
// template<class T>
py::array_t<double> stlVectorToNumpyArray_Scalar(
  ...
  std::vector<unsigned int> shape{(indices.back()[0]) + 1,
                                  (indices.back()[1]) + 1,
                                  (indices.back()[2]) + 1};
...
// template<class T>
py::array_t<double> stlVectorToNumpyArray_Vector(
  ...
  std::vector<unsigned int> shape(
      {indices.back()[0] + 1, indices.back()[1] + 1, indices.back()[2] + 1, 3});
  py::array_t<double> res = py::array_t<double>(shape);
...
// template<class T>
std::vector<double>
numpyArrayToStlVector_Scalar(const py::array_t<double> &numpy_array) {
...
numpyArrayToStlVector_Vector(const py::array_t<double> &numpy_array) {
  ...
  for (unsigned int i = 0; i < numpy_array.shape(0); i++)
    for (unsigned int j = 0; j < numpy_array.shape(1); j++)
      for (unsigned int k = 0; k < numpy_array.shape(2); k++) {
        std::array<double, 3> vec = {np_unchecked(i, j, k, 0),
                                     np_unchecked(i, j, k, 1),
                                     np_unchecked(i, j, k, 2)};
        res.push_back(vec);
      }
...
// template<class T>
std::function<
    std::vector<double>(std::vector<double> /*stl_vector*/,
                        std::vector<std::array<unsigned int, 3>> /*indices*/)> *
functionWrapper_Scalar(
    std::function<py::array_t<double>(py::array_t<double>)> *py_func_ptr) {
  // case: py_func exists
  if (py_func_ptr) {
    // create copy at current scope
    auto py_func = *py_func_ptr;
    return new std::function<std::vector<double>(
        std::vector<double> /*stl_vector*/,
        std::vector<std::array<unsigned int, 3>> /*indices*/)>{
        [py_func](std::vector<double> stl_vector,
                  std::vector<std::array<unsigned int, 3>> indices) {
          py::array_t<double> np_input =
              conversion::stlVectorToNumpyArray_Scalar(stl_vector, indices);
...
std::function<std::vector<std::array<double, 3>>(
    ...
    std::vector<std::array<unsigned int, 3>> /*indices*/)> *
functionWrapper_Vector(
    std::function<py::array_t<double>(py::array_t<double>)> *py_func_ptr) {
  // case: py_func exists
  if (py_func_ptr) {
    // create copy at current scope
    auto py_func = *py_func_ptr;
    return new std::function<std::vector<std::array<double, 3>>(
        std::vector<std::array<double, 3>> /*stl_vector*/,
        std::vector<std::array<unsigned int, 3>> /*indices*/)>{
        [py_func](std::vector<std::array<double, 3>> stl_vector,
                  std::vector<std::array<unsigned int, 3>> indices) {
          auto np_input =
              conversion::stlVectorToNumpyArray_Vector(stl_vector, indices);
