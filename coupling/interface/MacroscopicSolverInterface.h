...
holds data of the macroscopic cell at index globalCellIndex.
     *  By default, this method is used for send/recv operations.
     *  However, there are situations where the target and source rank
definitions (target rank=rank of macroscopic solver that shall receive data from
MD;
     *  source rank=rank of macroscopic solver that shall send data to MD) may
differ. For example, assume a domain decomposition using ghost layers.
     *  You may want to send data from cells inside a process-local domain and
want to receive data in all copies of a given macroscopic cell (incl. ghost
cells),
     *  but you may not be able to provide valid data on every process for each
cell instance (e.g., if this cell is part of a ghost layer).
...
macroscopic solver that
     *  provides a valid copy of the cell at globalCellIndex to the coupling
tool. You may have multiple source ranks; however, each source rank should hold
a valid copy.
...
macroscopic solver that
     *  obtains a valid copy of the cell at globalCellIndex from the coupling
tool. You may have multiple target ranks. Default: return all ranks that the
cell
