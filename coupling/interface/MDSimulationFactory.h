...
class SimpleMDSimulation: public coupling::interface::MDSimulation {
  public:
    ...
    virtual void simulateTimesteps(const unsigned int &numberTimesteps, const unsigned int &firstTimestep){
      for (unsigned int t = firstTimestep; t < firstTimestep+numberTimesteps; t++){ _molecularDynamicsSimulation.simulateOneCouplingTimestep(t); }
    }
    virtual void sortMoleculesIntoCells(){
      // nop required, since the linked cells are very tightly linked to mamico
    }

    virtual void setMacroscopicCellService(
      ...
      // set the cell service also in singleton of mamico interface provider -> typically not required in coupling, but makes the
      // simulation state more consistent compared to using LAMMPS
      coupling::interface::MamicoInterfaceProvider<simplemd::LinkedCell,MDSIMULATIONFACTORY_DIMENSION>::getInstance().setMacroscopicCellService(macroscopicCellService);
    }
...
class LammpsMDSimulation: public coupling::interface::MDSimulation {
  ...
  public:
    ...
    // switch on coupling -> add mamico fix with ID=2; we further check correctness of number of cells
    // -> afterwards, the MDSolverInterface of mamico will be initialised and provided via MamicoInterfaceProvider.
    virtual void switchOnCoupling() {
      ...
      for (unsigned int d = 0; d < MDSIMULATIONFACTORY_DIMENSION; d++){
        ...
        if (!tarch::la::equals(cells*numberProcs[d]*meshsize[d],domainSize[d],_tolerance)){
          std::cout << "ERROR switchOnCoupling(): cells and domainsize/meshsize/numberProcs do not match!" << std::endl; exit(EXIT_FAILURE);
        }
    ...
    virtual void sortMoleculesIntoCells(){
      LAMMPS_NS::MamicoLammpsMDSolverInterface<MDSIMULATIONFACTORY_DIMENSION> *interface = (LAMMPS_NS::MamicoLammpsMDSolverInterface<MDSIMULATIONFACTORY_DIMENSION>*) coupling::
        interface::MamicoInterfaceProvider<LAMMPS_NS::MamicoCell,MDSIMULATIONFACTORY_DIMENSION>::getInstance().getMDSolverInterface();
      if (interface==NULL){std::cout << "ERROR sortMoleculesIntoCells(): interface==NULL!" << std::endl; exit(EXIT_FAILURE); }
      coupling::services::MacroscopicCellService<MDSIMULATIONFACTORY_DIMENSION> *macroscopicCellService = coupling::
        interface::MamicoInterfaceProvider<LAMMPS_NS::MamicoCell,MDSIMULATIONFACTORY_DIMENSION>::getInstance().getMacroscopicCellService();
      if (macroscopicCellService==NULL){std::cout << "ERROR sortMoleculesIntoCells(): macroscopicCellService==NULL!" << std::endl; exit(EXIT_FAILURE); }
    ...
    virtual void setMacroscopicCellService(coupling::services::MacroscopicCellService<MDSIMULATIONFACTORY_DIMENSION> *macroscopicCellService){
      coupling::interface::MamicoInterfaceProvider<LAMMPS_NS::MamicoCell,MDSIMULATIONFACTORY_DIMENSION>::getInstance().setMacroscopicCellService(macroscopicCellService);
    }
  ...
  private:
    void initSingleSimulation(MPI_Comm comm,unsigned int localMDSimulation){
      ...
      ss.str(""); ss << "log log.lammps" << localMDSimulation;
      _lmp->input->one(ss.str().c_str());
      ss.str(""); ss << "log log.lammps" << localMDSimulation << " append";
      _lmp->input->one(ss.str().c_str());
      ...
      for (int d = 0; d < MDSIMULATIONFACTORY_DIMENSION; d++) { procs[d] = _configuration.getMPIConfiguration().getNumberOfProcesses()[d]; }
      ss << "processors " << procs[0] << " " << procs[1] << " " << procs[2] << " map xyz";
      _lmp->input->one(ss.str().c_str());

      // define 2D if necessary
      if (MDSIMULATIONFACTORY_DIMENSION==2){ _lmp->input->one("dimension 2"); }
      else { _lmp->input->one("dimension 3");}

      // incorporate boundary conditions: currently, only reflecting and periodic boundaries are supported; to be extended to 2D
      ss.str("");
      ss << "boundary";
      if (MDSIMULATIONFACTORY_DIMENSION==3){
        ...
        for (int i = 0; i < MDSIMULATIONFACTORY_DIMENSION; i++){
          if (bc[indices[i]]==simplemd::PERIODIC_BOUNDARY){        ss << " p";}
          else if (bc[indices[i]]==simplemd::REFLECTING_BOUNDARY){ ss << " f";}
          else { std::cout << "ERROR LammpsMDSimulation::initSingleSimulation(): only reflecting and periodic boundaries supported!" << std::endl; exit(EXIT_FAILURE);}
        }
      } else { std::cout << "ERROR LammpsMDSimulation::initSingleSimulation(): only 3D supported at the moment!" << std::endl; }
      _lmp->input->one(ss.str().c_str());
      ...
      ss.str("");
      ss << "region box block " << domainOffset[0] << " " << (domainOffset[0]+domainSize[0]) << " " << domainOffset[1] << " " << (domainOffset[1]+domainSize[1]) << " ";
      ...
      if (MDSIMULATIONFACTORY_DIMENSION==3){
        const tarch::la::Vector<MD_LINKED_CELL_NEIGHBOURS,simplemd::BoundaryType> bc = _configuration.getDomainConfiguration().getBoundary();
        int indices[3] = {12,10,4}; std::string faces[3] = {"x", "y", "z"};
        for (int i = 0; i < MDSIMULATIONFACTORY_DIMENSION; i++){
          if (bc[indices[i]]==simplemd::REFLECTING_BOUNDARY){
            ss.str(""); ss << "fix reflection" << faces[i] << " all wall/reflect " << faces[i] << "lo EDGE " << faces[i] << "hi EDGE units box";
            _lmp->input->one(ss.str().c_str());
          }
        }
      }
      ...
      ss.str(""); ss << "pair_style lj/cut " << _configuration.getDomainConfiguration().getCutoffRadius(); _lmp->input->one(ss.str().c_str());
      _lmp->input->one("pair_modify shift yes");
      ss.str(""); ss << "pair_coeff 1 1 " << _configuration.getMoleculeConfiguration().getEpsilon() << " ";
      ss << _configuration.getMoleculeConfiguration().getSigma() << " " << _configuration.getDomainConfiguration().getCutoffRadius();
      _lmp->input->one(ss.str().c_str());

      // set time step
      ss.str(""); ss << "timestep " << _configuration.getSimulationConfiguration().getDt(); _lmp->input->one(ss.str().c_str());
      ...
      if (_configuration.getVTKConfiguration().getWriteEveryTimestep() != 0){
        ss.str(""); ss << "dump myDump all custom " << _configuration.getVTKConfiguration().getWriteEveryTimestep() << " ";
        ss << _configuration.getVTKConfiguration().getFilename() << "_" << localMDSimulation << "_" << "*.dat id type x y z vx vy vz";
        _lmp->input->one(ss.str().c_str());
      }
      // write checkpointing
...
class LammpsDPDSimulation: public coupling::interface::MDSimulation {
  ...
  private:
    void initSingleSimulation(MPI_Comm comm,unsigned int localMDSimulation){
      ...
      std::stringstream ss;
      std::string inputFile;
      // particle insertion is currently only supported for single-centered LJ particles
      if (_mamicoConfiguration.getParticleInsertionConfiguration().getParticleInsertionType()!=coupling::configurations::ParticleInsertionConfiguration::NO_INSERTION){
        std::cout << "ERROR coupling::LammpsDPDSImulation::initSingleSimulation(): Particle insertion currently not supported!" << std::endl; exit(EXIT_FAILURE);
      }
      ...
      ss.str(""); ss << "variable kB equal " << _configuration.getDomainConfiguration().getKB();
      _lmp->input->one(ss.str().c_str());
      ss.str(""); ss << "variable kBT equal " << _configuration.getMoleculeConfiguration().getTemperature()*_configuration.getDomainConfiguration().getKB();
      _lmp->input->one(ss.str().c_str());
      ss.str(""); ss << "variable T equal " << _configuration.getMoleculeConfiguration().getTemperature();
      _lmp->input->one(ss.str().c_str());
      ss.str(""); ss << "variable rc equal " << _configuration.getDomainConfiguration().getCutoffRadius();
      _lmp->input->one(ss.str().c_str());
      _lmp->input->one("units lj");
      _lmp->input->one("dimension ${ndim}");

      // set up boundary treatment, Pt1: for "periodic", we just set the "p"-option and "f" otherwise;
      // for the latter, we further have to introduce the respective walls, see boundary treatment, Pt2, further down
      ss.str(""); ss << "boundary";
      if (MDSIMULATIONFACTORY_DIMENSION==3){
        const tarch::la::Vector<MD_LINKED_CELL_NEIGHBOURS,simplemd::BoundaryType> bc = _configuration.getDomainConfiguration().getBoundary();
        int indices[3] = {12,10,4};
        for (int i = 0; i < MDSIMULATIONFACTORY_DIMENSION; i++){
          ...
          else { std::cout << "ERROR LammpsDPDSimulation::initSingleSimulation(): only reflecting and periodic boundaries supported!" << std::endl; exit(EXIT_FAILURE);}
        }
      } else { std::cout << "ERROR LammpsDPDSimulation::initSingleSimulation(): only 3D supported at the moment!" << std::endl; }
      _lmp->input->one(ss.str().c_str());

      _lmp->input->one("atom_style  atomic");
      // define number of processes with xyz-topology
      ss.str(""); ss << "processors " << _configuration.getMPIConfiguration().getNumberOfProcesses()[0] << " " << _configuration.getMPIConfiguration().getNumberOfProcesses()[1] << " ";
      ss << _configuration.getMPIConfiguration().getNumberOfProcesses()[2] << " map xyz";
      _lmp->input->one(ss.str().c_str());
      ss.str(""); ss << "variable    seed1 equal " << localMDSimulation+rank+1;
      _lmp->input->one(ss.str().c_str()); // currently fixed random seed
      ss.str(""); ss << "variable    seed2 equal " << localMDSimulation+rank+10;
      _lmp->input->one(ss.str().c_str()); // new seed for velocity generation; currently deduced from _randSeed+10
      _lmp->input->one("print \"seed1: ${seed1}\"");
      _lmp->input->one("print \"seed2: ${seed2}\"");
      _lmp->input->one("pair_style dpd/general ${kBT} ${rc} ${seed1}"); // DPD scheme with forcing by Xin
      ...
      ss.str(""); ss << "variable  Lx equal " << domainSize[0];
      _lmp->input->one(ss.str().c_str());
      ss.str(""); ss << "variable  Ly equal " << domainSize[1];
      _lmp->input->one(ss.str().c_str());
      ss.str(""); ss << "variable  Lz equal " << domainSize[2];
      _lmp->input->one(ss.str().c_str());
      // The following command computes the average density based on molecules-per-direction. However, particles will be randomly distributed - they are not arranged on a grid
      ...
      unsigned int numberMolecules=1; for (unsigned int d = 0; d < MDSIMULATIONFACTORY_DIMENSION; d++){ numberMolecules = numberMolecules*molecules[d]; }
      ss.str(""); ss << "variable Npart equal " << numberMolecules;
      _lmp->input->one(ss.str().c_str());
      _lmp->input->one("variable  Ntype equal 1");

      ss.str("");
      ss << "region region_dpd block " << domainOffset[0] << " " << domainOffset[0]+domainSize[0] << " " << domainOffset[1] << " " << domainOffset[1]+domainSize[1] << " ";
      ss << domainOffset[2] << " " << domainOffset[2]+domainSize[2] << " units box";
      _lmp->input->one(ss.str().c_str());
      _lmp->input->one("create_box    ${Ntype} region_dpd");
      _lmp->input->one("create_atoms 1 random ${Npart} 12345 NULL"); // new randomized initialisation

      // boundary treatment, Pt2: introduce reflecting walls if required (fixes need to be defined after box definition)
      if (MDSIMULATIONFACTORY_DIMENSION==3){
        const tarch::la::Vector<MD_LINKED_CELL_NEIGHBOURS,simplemd::BoundaryType> bc = _configuration.getDomainConfiguration().getBoundary();
        int indices[3] = {12,10,4};
        if (bc[indices[0]]==simplemd::REFLECTING_BOUNDARY){ _lmp->input->one("fix reflectionX all wall/reflect xlo EDGE xhi EDGE"); }
        if (bc[indices[1]]==simplemd::REFLECTING_BOUNDARY){ _lmp->input->one("fix reflectionY all wall/reflect ylo EDGE yhi EDGE"); }
        if (bc[indices[2]]==simplemd::REFLECTING_BOUNDARY){ _lmp->input->one("fix reflectionZ all wall/reflect zlo EDGE zhi EDGE"); }
      }

      ss.str(""); ss << "variable m equal " << _configuration.getMoleculeConfiguration().getMass();
      _lmp->input->one(ss.str().c_str());

      ss.str(""); ss << "variable alpha equal " << _configuration.getMoleculeConfiguration().getEpsilon(); // configurable via epsilon parameter in xml config
      _lmp->input->one(ss.str().c_str());
      ss.str(""); ss <<"variable sigma equal " << _configuration.getMoleculeConfiguration().getSigma();
      _lmp->input->one(ss.str().c_str());
      _lmp->input->one("variable  gamma  equal ${sigma}^2/(2*${kBT})");

      _lmp->input->one("variable power equal 0.25");

      ss.str(""); ss << "variable Ntime equal " << _configuration.getSimulationConfiguration().getNumberOfTimesteps();
      _lmp->input->one(ss.str().c_str());
      ss.str(""); ss << "variable Nrepeat equal " << (int)(0.8*_configuration.getSimulationConfiguration().getNumberOfTimesteps());
      _lmp->input->one(ss.str().c_str());
      ...
      _lmp->input->one("group               extra_atoms id > ${Npart}"); // remove particles to reach correct density
      _lmp->input->one("delete_atoms        group extra_atoms");

      ss.str(""); ss << "timestep " << _configuration.getSimulationConfiguration().getDt();
      _lmp->input->one(ss.str().c_str());
      ...
      if (_mamicoConfiguration.getBoundaryForceConfiguration().getBoundary()[4]){
        if (_configuration.getDomainConfiguration().getBoundary()[4]!=simplemd::REFLECTING_BOUNDARY){
          std::cout << "ERROR coupling::MDSimulationFactory::LammpsDPDSimulation::initSingleSimulation(): bottom boundary is not reflecting!" << std::endl; exit(EXIT_FAILURE);
        }
        ss.str("");
        ss << "fix lower_wall all dpd/boundary/force ${rc} 100 dpd_plane_fc.dat dpd_plane_fd1.dat dpd_plane_fd2.dat 2 0 " << domainOffset[2] << " 0.0";
        _lmp->input->one(ss.str().c_str());
      }
      // wall fix top
      if (_mamicoConfiguration.getBoundaryForceConfiguration().getBoundary()[5]){
        if (_configuration.getDomainConfiguration().getBoundary()[21]!=simplemd::REFLECTING_BOUNDARY){
          std::cout << "ERROR coupling::MDSimulationFactory::LammpsDPDSimulation::initSingleSimulation(): top boundary is not reflecting!" << std::endl; exit(EXIT_FAILURE);
        }
        ss.str("");
        ss << "fix upper_wall all dpd/interface/force ${rc} 100 dpd_plane_fc.dat dpd_plane_fd1.dat dpd_plane_fd2.dat 2 1 " << domainOffset[2]+domainSize[2] << " 0.0";
        _lmp->input->one(ss.str().c_str());
      }
...
class SimulationAndInterfaceFactory {
  public:
    ...
    coupling::interface::MDSolverInterface<MY_LINKEDCELL,MDSIMULATIONFACTORY_DIMENSION>* getMDSolverInterface(
      ...
      #if defined(SIMPLE_MD)
      // for the simple MD code, we create a new md solver interface and also add it to the mamico interface provider (the latter is not really required,
      ...
      if (simpleMDSimulation==NULL){
        std::cout << "ERROR MDSimulationFactory::getMDSolverInterface(): Could not cast to SimpleMDSimulation!" << std::endl; exit(EXIT_FAILURE);
      }
      mdSolverInterface = new coupling::interface::SimpleMDSolverInterface(
        ...
        simpleMDSimulation->getParallelTopologyService(),
        simpleMDSimulation->getMoleculeService(),
        simpleMDSimulation->getLinkedCellService(),
        simpleMDSimulation->getMolecularPropertiesService(),
        (simpleMDSimulation->getParallelTopologyService()).getLocalBoundaryInformation(),
      ...
      #elif defined(LAMMPS_MD)
      // as switchOnCoupling() should have been called before this method, the fix-mamico should be already initialised. hence the MDSolverInterface of
