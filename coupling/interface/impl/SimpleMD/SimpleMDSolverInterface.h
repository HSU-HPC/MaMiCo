...
class SimpleMDSolverInterface: public MDSolverInterface<simplemd::LinkedCell,MD_DIM> {
  private:
    ...
    tarch::la::Vector<MD_DIM,unsigned int> computeNumberOfLinkedCellsPerMacroscopicCells(
      const tarch::la::Vector<MD_DIM,unsigned int>& localNumberOfMacroscopicCells
    ) const {
  ...
  public:
    SimpleMDSolverInterface(
      tarch::la::Vector<MD_DIM,unsigned int> numberLinkedCellsPerMacroscopicCell,
      ...
      const tarch::la::Vector<MD_LINKED_CELL_NEIGHBOURS,simplemd::BoundaryType>& localBoundaryInformation,
      const double &dt
    ):
    ...
    _cutoffEnergy(
      4.0 * _epsilon * _sigma6 /(_cutoffRadiusSquared*_cutoffRadiusSquared*_cutoffRadiusSquared)
    * (_sigma6/(_cutoffRadiusSquared*_cutoffRadiusSquared*_cutoffRadiusSquared) - 1.0) ),
    ...
    void getInitialVelocity(
      const tarch::la::Vector<MD_DIM,double>& meanVelocity, const double &kB, const double& temperature, tarch::la::Vector<MD_DIM,double>& initialVelocity
    ) const {
    ...
    tarch::la::Vector<MD_DIM,unsigned int> getLinkedCellIndexForMoleculePosition(
      ...
      tarch::la::Vector<MD_DIM,unsigned int> localIndexOfFirstCell = _linkedCellService.getLocalIndexOfFirstCell();
    ...
    void setupPotentialEnergyLandscape(
      const tarch::la::Vector<MD_DIM,unsigned int>& indexOfFirstMacroscopicCell,
      ...
      for (unsigned int d = 0; d < MD_DIM; d++){
        ...
        if (indexOfFirstMacroscopicCell[d]==0){std::cout << "ERROR setupPotentialEnergyLandscape: " << indexOfFirstMacroscopicCell[d] << std::endl; exit(EXIT_FAILURE);}
        #endif
    ...
    void calculateForceAndEnergy(
      coupling::interface::Molecule<MD_DIM>& molecule
    ){
      const tarch::la::Vector<MD_DIM,double> position = molecule.getPosition();
