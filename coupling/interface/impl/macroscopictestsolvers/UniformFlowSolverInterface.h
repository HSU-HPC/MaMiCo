...
namespace coupling {
  namespace interface {
    ...
    class UniformFlowSolverInterface;
  }
}
 ...
 */
template<unsigned int dim>
class coupling::interface::UniformFlowSolverInterface: public coupling::interface::TestMacroscopicSolverInterface<dim> {
  public:
    ...
    _totalNumberMacroscopicCells(initTotalNumberMacroscopicCells(globalNumberMacroscopicCells)),
    ...
    virtual std::vector<unsigned int> getRanks(tarch::la::Vector<dim,unsigned int> globalCellIndex){
      std::vector<unsigned int> result; result.push_back(0); return result;
    }
    ...
    virtual std::vector<coupling::datastructures::MacroscopicCell<dim>* > getMacroscopicCells4Sending(){
      ...
      for (unsigned int i = 0; i < _totalNumberMacroscopicCells; i++){
        ...
        if (result[i]==NULL){std::cout << "ERROR coupling::interface::UniformFlowSolverInterface: result[i]==NULL!" << std::endl; exit(EXIT_FAILURE); }
    ...
    virtual unsigned int * getMacroscopicCellIndices4Sending(){
      ...
      if (indices==NULL){std::cout << "ERROR coupling::interface::UniformFlowSolverInterface: indices==NULL!" << std::endl; exit(EXIT_FAILURE); }
      for (unsigned int i = 0; i < _totalNumberMacroscopicCells; i++){ indices[i] = i; }
      return indices;
    }

    /** nothing is received */
    virtual std::vector<coupling::datastructures::MacroscopicCell<dim>* > getMacroscopicCells4Receiving(){
      std::vector<coupling::datastructures::MacroscopicCell<dim>* > result; return result;
    }
  ...
  private:
    ...
    unsigned int initTotalNumberMacroscopicCells(tarch::la::Vector<dim,unsigned int> globalNumberMacroscopicCells) const {
      ...
      for (unsigned int d = 1; d < dim; d++){ num = num*totalNumberCells[d]; }
      return num;
    }
};
