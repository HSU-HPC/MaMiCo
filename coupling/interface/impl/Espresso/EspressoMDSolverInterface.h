  ...
  void calculateForceAndEnergy(coupling::interface::Molecule<3> &molecule) {
    ...
    for (unsigned int n = 0; n < dd.cell_inter[c].n_neighbors; n++) {
      ...
      for (unsigned int j = 0; j < np2; j++) {
        ...
        // calc_non_bonded_pair_force have a check, if the two particles
        // sent to them have the same location or are the same, force or
        // potential energy is not calculated then.
