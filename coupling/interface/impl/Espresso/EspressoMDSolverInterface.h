...
/*        	for(int i=0; i<3; i++) {
    ...
    void getInitialVelocity(const tarch::la::Vector<3,double>& meanVelocity, const double &kB, const double& temperature, tarch::la::Vector<3,double>& initialVelocity) const {
    ...
    // Compute the force and potential energy of the molecule using Espresso methods calc_non_bonded_pair_energy and calc_non_bonded_pair_force
    void calculateForceAndEnergy(
      coupling::interface::Molecule<3> &molecule
    ) {
		// Create a new temporary particle in Espresso and set its velocity and position
		Particle temp_part;
		...
		unsigned int ctr=0;
		// loop over all the neighbor cells and calculate potential energy and force contribution from each molecule in the cell
        for (unsigned int n = 0; n < dd.cell_inter[c].n_neighbors; n++) {
			...
			for(unsigned int j = 0; j < np2; j++) {	
				// The functions calc_non_bonded_pair_energy and calc_non_bonded_pair_force have a check, if the two particles
				// sent to them have the same location or are the same, force or potential energy is not calculated then.
    ...
    // Synchronize Molecules after mass modification using cells_update_ghosts() function in Espresso
