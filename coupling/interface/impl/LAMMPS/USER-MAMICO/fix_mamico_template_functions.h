...
void LAMMPS_NS::FixMamico::modifyMDSystem(){
  ...
  LAMMPS_NS::MamicoLammpsMDSolverInterface<dim> *mdSolverInterface = (LAMMPS_NS::MamicoLammpsMDSolverInterface<dim>*) 
    coupling::interface::MamicoInterfaceProvider<MamicoCell,dim>::getInstance().getMDSolverInterface();

  // extract ghost atoms and update all mamico cell lists ------------------------
  #if (COUPLING_MD_DEBUG==COUPLING_MD_YES)
  if (mdSolverInterface==NULL){std::cout << "ERROR modifyMDSystem(): Could not cast MDSolverInterface!" << std::endl; exit(EXIT_FAILURE);}
...
void LAMMPS_NS::FixMamico::modifyMomentumAndTemperature(){
  ...
  // increment time step counter; since this method is called in post_force, this is the right location for this purpose!
  _timestepCounter++;
}
