...
template <unsigned int dim> void LAMMPS_NS::FixMamico::modifyMDSystem() {
  coupling::services::MacroscopicCellService<dim> *macroscopicCellService =
      coupling::interface::MamicoInterfaceProvider<
          MamicoCell, dim>::getInstance().getMacroscopicCellService();
  ...
  LAMMPS_NS::MamicoLammpsMDSolverInterface<dim> *mdSolverInterface =
      (LAMMPS_NS::MamicoLammpsMDSolverInterface<
          ...
          MamicoCell, dim>::getInstance().getMDSolverInterface();

// extract ghost atoms and update all mamico cell lists ------------------------
#if (COUPLING_MD_DEBUG == COUPLING_MD_YES)
  if (mdSolverInterface == NULL) {
    std::cout << "ERROR modifyMDSystem(): Could not cast MDSolverInterface!"
              << std::endl;
    exit(EXIT_FAILURE);
  }
  std::cout << "FixMamico, Rank " << indexConversion.getThisRank()
            << ": Sort particles into cells" << std::endl;
#endif
...
#if (COUPLING_MD_DEBUG == COUPLING_MD_YES)
  std::cout << "FixMamico, Rank " << indexConversion.getThisRank()
            << ": Process inner cells after MD timestep" << std::endl;
#endif
  macroscopicCellService->processInnerMacroscopicCellAfterMDTimestep();
#if (COUPLING_MD_DEBUG == COUPLING_MD_YES)
  std::cout << "FixMamico, Rank " << indexConversion.getThisRank()
            << ": Distribute mass" << std::endl;
#endif
  macroscopicCellService->distributeMass(_timestepCounter);
#if (COUPLING_MD_DEBUG == COUPLING_MD_YES)
  std::cout << "FixMamico,Rank " << indexConversion.getThisRank()
            << ": Plot every microscopic time step..." << std::endl;
#endif
  macroscopicCellService->plotEveryMicroscopicTimestep(_timestepCounter);
#if (COUPLING_MD_DEBUG == COUPLING_MD_YES)
  std::cout << "FixMamico,Rank " << indexConversion.getThisRank()
            << ": Go to next time step..." << std::endl;
#endif
}

template <unsigned int dim>
void LAMMPS_NS::FixMamico::modifyMomentumAndTemperature() {
  coupling::services::MacroscopicCellService<dim> *macroscopicCellService =
      coupling::interface::MamicoInterfaceProvider<
          MamicoCell, dim>::getInstance().getMacroscopicCellService();
  const coupling::IndexConversion<dim> &indexConversion =
