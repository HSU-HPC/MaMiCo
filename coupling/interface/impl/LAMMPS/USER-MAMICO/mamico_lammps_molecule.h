...
class MamicoLammpsMolecule: public coupling::interface::Molecule<dim> {
  public:
    MamicoLammpsMolecule(double **x,double **v, double **f, int n,double cutoff): coupling::interface::Molecule<dim>(),
    _x(x), _v(v), _f(f), _n(n),_cutOffRadiusSquared(cutoff*cutoff),_tolerance(1.0e-8){
    ...
    virtual tarch::la::Vector<dim,double> getVelocity() const {
      #if (COUPLING_MD_DEBUG==COUPLING_MD_YES)
      if (_v==NULL){ std::cout << "ERROR getVelocity(): _v==NULL!" << std::endl;exit(EXIT_FAILURE); }
      #endif
      tarch::la::Vector<dim,double> vel(0.0);
      for (int d = 0; d < dim; d++) vel[d] = _v[_n][d];
      return vel;
    }
    virtual void setVelocity(const tarch::la::Vector<dim,double> &vel){
      #if (COUPLING_MD_DEBUG==COUPLING_MD_YES)
      if (_v==NULL){ std::cout << "ERROR setVelocity(): _v==NULL!" << std::endl;exit(EXIT_FAILURE); }
      #endif
      for (int d = 0; d < dim; d++) _v[_n][d] = vel[d];
    }

    virtual tarch::la::Vector<dim,double> getPosition() const {
      tarch::la::Vector<dim,double> pos(0.0);
      for (int d = 0; d < dim; d++) pos[d] = _x[_n][d];
      return pos;
    }
    virtual void setPosition(const tarch::la::Vector<dim,double> &pos){
      for (int d = 0; d < dim; d++) _x[_n][d] = pos[d];
    }

    virtual tarch::la::Vector<dim,double> getForce() const {
      #if (COUPLING_MD_DEBUG==COUPLING_MD_YES)
      if (_f==NULL){ std::cout << "ERROR getForce(): _f==NULL!" << std::endl;exit(EXIT_FAILURE); }
      #endif
      tarch::la::Vector<dim,double> force(0.0);
      for (int d = 0; d < dim; d++) force[d] = _f[_n][d];
      return force;
    }
    virtual void setForce(const tarch::la::Vector<dim,double> &force){
      #if (COUPLING_MD_DEBUG==COUPLING_MD_YES)
      if (_f==NULL){ std::cout << "ERROR setForce(): _f==NULL!" << std::endl;exit(EXIT_FAILURE); }
      #endif
      for (int d = 0; d < dim; d++) _f[_n][d] = force[d];
    }


    /** copy/paste of calculateForceAndEnergy(molecule), but ignores molecule interaction, if two molecules are very close to each other. We use this function
    ...
    virtual double getPotentialEnergy() const {
      ...
      for (int d = 0; d < dim;d++){
        #if (COUPLING_MD_DEBUG==COUPLING_MD_YES)
        if (cellIndex[d]==0){std::cout << "ERROR MamicoLammpsMolecule::getPotentialEnergy(): Cannot compute energy for molecules in ghost layer!";
      ...
      for (loop[0] = start[0]; loop[0] < end[0]; loop[0]++){
        ...
        // we take a copy instead of a reference since we do not want to have conflicts
        // in the access of the underlying molecule iterators;
        // Example VTK-plotting: the Mamico-plotter iterates over cells and uses references
        //                       -> using another reference dereferences the molecule that
        //                          the "outer" VTK cell iterator points to
        LAMMPS_NS::MamicoCell cell = mdSolverInterface->getLinkedCell(
          macroscopicCellIndex,linkedCellInMacroscopicCell,linkedCellsPerMacroscopicCell, indexConversion);
        ...
        for (it->begin(); it->continueIteration(); it->next()){
          ...
          if ( (rij2 > _cutOffRadiusSquared) || (rij2<_tolerance) ){
            ...
            #if (COUPLING_MD_DEBUG==COUPLING_MD_YES)
            std::cout << "MamicoLammpsMolecule::getPotentialEnergy(): Compute energy with molecule at " << it->getConst().getPosition() << std::endl;
            #endif
