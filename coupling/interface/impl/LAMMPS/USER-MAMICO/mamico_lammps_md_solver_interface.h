 ...
 *  - currently, only simple LJ-atoms are supported (at least in particle insertion)
...
class MamicoLammpsMDSolverInterface: public coupling::interface::MDSolverInterface<LAMMPS_NS::MamicoCell,dim> {
  public:
    MamicoLammpsMDSolverInterface(LAMMPS* lmp,int seed,int numberCells, double cutoffRadius):
      ...
      _lmp(lmp), _sorting(numberCells,lmp), _cutOffRadiusSquared(cutoffRadius*cutoffRadius), _cutOffRadius(cutoffRadius),
      _atomType(1),_precision(10),_tolerance(1.0e-8),
      _random(new RanPark(lmp,seed))
    {
      if (_random==NULL) _lmp->error->all(FLERR,"Could not allocated new RanPark!");
    }
    virtual ~MamicoLammpsMDSolverInterface(){
      delete _random; _random = NULL;
      _lmp = NULL;
    }
     ...
     */
    virtual LAMMPS_NS::MamicoCell& getLinkedCell(
      ...
      // we currently embed one cell per macroscopic cell. It is hence sufficient to use the same enumeration as used for the macroscopic cells
      const unsigned int index = indexConversion.getLocalCellIndex(macroscopicCellIndex);
      #if (COUPLING_MD_DEBUG==COUPLING_MD_YES)
      std::cout << "Return linked cell at macro-cell " << macroscopicCellIndex << " no. " << linkedCellInMacroscopicCell << ", assuming " << linkedCellsPerMacroscopicCell << " linked cells per cell: " << index << std::endl;
      #endif
    ...
    virtual tarch::la::Vector<dim,double> getGlobalMDDomainSize() const {
      ...
      for (int d = 0; d < dim; d++){ domainsize[d] = _lmp->domain->boxhi[d]-_lmp->domain->boxlo[d]; }
      #if (COUPLING_MD_DEBUG==COUPLING_MD_YES)
    ...
    virtual tarch::la::Vector<dim,double> getGlobalMDDomainOffset() const {
      ...
      for (int d = 0; d < dim; d++){ domainoffset[d] = _lmp->domain->boxlo[d]; }
      #if (COUPLING_MD_DEBUG==COUPLING_MD_YES)
    ...
    virtual double getMoleculeMass() const{
      // currently, return mass of atom type no "1". We currently assume thus, that all our molecules are of type 1.
      #if (COUPLING_MD_DEBUG==COUPLING_MD_YES)
    ...
    /** returns Boltzmann's constant. For LJ fluids, this should always be 1.0, see doc/units.html. */
    ...
    /** returns the sigma parameter of the LJ potential. We always return 1.0, see also doc/units.html for LJ style. */
    virtual double getMoleculeSigma() const {
      return 1.0;
    }

    /** returns the epsilon parameter of the LJ potential. We always return 1.0, see also doc/units.html for LJ style. */
    ...
    virtual void getInitialVelocity(
      ...
      const double stdDeviation = std::sqrt(dim*kB*temperature/getMoleculeMass()); // temperature-based std-deviation of gaussian distr.

      tarch::la::Vector<dim,double> ran(0.0); // buffer for one gaussian and dim-1 uniform random numbers (latter ones are used for random rotation of velocity vector)
      ran[0] = _random->gaussian();
      for (int d = 1; d < dim; d++) ran[d] = _random->uniform();
    ...
    /** deletes the molecule from the MD simulation. This only handles the process-local deletion. The synch. is carried out afterwards in
    ...
    virtual void deleteMoleculeFromMDSimulation(const coupling::interface::Molecule<dim>& molecule,LAMMPS_NS::MamicoCell& cell) {
      ...
      int i = 0;
      while (i < nlocal) {
        bool found = true; for (unsigned int d = 0; d < dim; d++){ found = found && tarch::la::equals(pos[d],atom->x[i][d],_tolerance); }
      ...
      // delete molecule from list in mamico cells; as global enumeration may change, we need to completely re-new the cell sorting in the NON-GHOST cells
      // -> we cannot put this to synchronizeMoleculesAfterMassModification(), since the local numbering of existing atoms might
      //    have already changed as well...
      const coupling::IndexConversion<dim> &indexConversion = coupling::interface::MamicoInterfaceProvider<
        LAMMPS_NS::MamicoCell,dim>::getInstance().getMacroscopicCellService()->getIndexConversion();
    ...
    virtual void addMoleculeToMDSimulation(const coupling::interface::Molecule<dim>& molecule) {
      const coupling::IndexConversion<dim> &indexConversion = coupling::interface::MamicoInterfaceProvider<MamicoCell,dim>::
                                                                                 getInstance().getMacroscopicCellService()->getIndexConversion();
      ...
      double posVec[3] = {0.0,0.0,0.0};
        for (int d = 0; d<dim; d++) posVec[d] = pos[d];
        for (int d = dim; d<3; d++) posVec[d] = 0.5*(_lmp->domain->boxhi[d]+_lmp->domain->boxlo[d]);
      ...
      // this should yield a process-LOCAL insertion of the atom
      ...
      // find index of this molecule; since we expect that the molecule is inserted at the last position, we start searching from the back
      int indexAtom     = -1;
      for (int i = nlocal-1; i > -1; i--){
        bool found = true;
        for (int d = 0; d < dim; d++){ found = found && tarch::la::equals(_lmp->atom->x[i][d], pos[d], _tolerance ); }
        if (found) { indexAtom = i; break; }
      }
      // if the molecules does not exist on this rank, we just jump out of this method...
      if (indexAtom==-1){
        #if (COUPLING_MD_DEBUG==COUPLING_MD_YES)
      ...
      for (unsigned int d = 0; d < dim; d++){ _lmp->atom->v[indexAtom][d] = vel[d]; _lmp->atom->f[indexAtom][d] = f[d]; }

      // add molecule to list in mamico cells; as global enumeration may change, we need to completely re-new the cell sorting in all NON-GHOST cells
      // -> we cannot put this to synchronizeMoleculesAfterMassModification(), since the local numbering of existing atoms might
      //    have already changed as well...
      _sorting.updateNonGhostCells(indexConversion);
    }
    ...
    virtual void setupPotentialEnergyLandscape(
      ...
      // nop, potential energy is evaluated for each atom separately when calling getPotentialEnergy()
    }
    ...
    virtual tarch::la::Vector<dim,unsigned int> getLinkedCellIndexForMoleculePosition(
      ...
      // currently, we have one mamico(linked) cell per macroscopic cell, so we can just use the indexing of the macroscopic cells of MaMiCo
      // note: typically, one should not really access other coupling modules from an interface; however, since this method is expected to be only
      // called AFTER all initialisation processes are finished, this is safe
      const coupling::IndexConversion<dim> &indexConversion = coupling::interface::MamicoInterfaceProvider<MamicoCell,dim>::
        getInstance().getMacroscopicCellService()->getIndexConversion();
    ...
    virtual void calculateForceAndEnergy(
      ...
      const coupling::IndexConversion<dim>& indexConversion = coupling::interface::
        MamicoInterfaceProvider<LAMMPS_NS::MamicoCell,dim>::getInstance().getMacroscopicCellService()->getIndexConversion();
      ...
      for (int d = 0; d < dim;d++){
        #if (COUPLING_MD_DEBUG==COUPLING_MD_YES)
        if (cellIndex[d]==0){std::cout << "ERROR MamicoLammpsMDSolverInterface::calculateForceAndEnergy(): Cannot compute force/energy for molecules in ghost layer!";
                             std::cout << std::endl; exit(EXIT_FAILURE);}
        #endif
      ...
      for (loop[0] = start[0]; loop[0] < end[0]; loop[0]++){
        ...
        for (it->begin(); it->continueIteration(); it->next()){
          #if (COUPLING_MD_DEBUG==COUPLING_MD_YES)
          std::cout << "MamicoLammpsMDSolverInterface::calculateForceAndEnergy(): Compute force/energy with molecule at " << it->getConst().getPosition() << std::endl;
          #endif
          const tarch::la::Vector<dim,double> rij = it->getConst().getPosition() - position1;
          const double rij2 = tarch::la::dot(rij,rij);

          // compute LJ force/energy and add the contributions, only if center is within cut-off radius AND
          // if this particle is not contained in this region already. We need the latter condition to re-use this function
          // for the evaluation of the potential energy in the molecule-wrapper (mamico_lammps_molecule)
    ...
    virtual void synchronizeMoleculesAfterMassModification() {
      ...
      if (_lmp->atom->natoms < 0 || _lmp->atom->natoms > MAXBIGINT) _lmp->error->all(FLERR,"Too many total atoms");
      ...
      if (_lmp->atom->natoms > MAXSMALLINT) {
        ...
        if (_lmp->atom->molecular == 0 && compress_flag) {
          ...
          for (int i = 0; i < _lmp->atom->nlocal; i++) tag[i] = 0;
      ...
      // exchange information and re-build lists (extracted and modified from verlet.cpp) ------------------------------
      // regular communication vs neighbor list rebuild -> we definitely want to have a complete re-build of lists, so we
      //  choose the "else"-branch from verlet.cpp
      {
        Verlet *verlet = (Verlet*) _lmp->update->integrate;
        if (verlet == NULL){
          std::cout << "ERROR synchronizeMoleculesAfterMassModification(): Could not cast Integrate* to Verlet*! Only verlet-integration supported!" << std::endl;
          exit(EXIT_FAILURE);
        }
        if (_lmp->modify->n_pre_exchange) _lmp->modify->pre_exchange();
          if (_lmp->domain->triclinic) _lmp->domain->x2lamda(_lmp->atom->nlocal);
          _lmp->domain->pbc();
          if (_lmp->domain->box_change) {
            _lmp->domain->reset_box();
            _lmp->comm->setup();
            if (_lmp->neighbor->style) _lmp->neighbor->setup_bins();
          }
          _lmp->timer->stamp();
          _lmp->comm->exchange();
          //if (sortflag && ntimestep >= atom->nextsort) atom->sort();
          _lmp->comm->borders();
          if (_lmp->domain->triclinic) _lmp->domain->lamda2x(_lmp->atom->nlocal+_lmp->atom->nghost);
          _lmp->timer->stamp(Timer::COMM);
          if (_lmp->modify->n_pre_neighbor) _lmp->modify->pre_neighbor();
          _lmp->neighbor->build(1);
          _lmp->timer->stamp(Timer::NEB);
      }

      // one more sorting for all cells, since the molecules might have been reordered in parallel exchange
      const coupling::IndexConversion<dim> &indexConversion = coupling::interface::MamicoInterfaceProvider<
        LAMMPS_NS::MamicoCell,dim>::getInstance().getMacroscopicCellService()->getIndexConversion();
    ...
    /** update all cell information -> forward call to _sorting. This is required once per time step before the actual coupling */
    void updateAllCells(const coupling::IndexConversion<dim>& indexConversion){ _sorting.updateAllCells(indexConversion); }

    /** prints molecules in all cells/inner cells/only ghost cells. For debugging purposes only. */
  ...
  private:
    ...
    const unsigned int _atomType; // this is the atom type used in the coupled simulation. Currently, it is hard-coded and set to 1.
