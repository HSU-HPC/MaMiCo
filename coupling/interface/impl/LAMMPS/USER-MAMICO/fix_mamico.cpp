...
LAMMPS_NS::FixMamico::FixMamico(LAMMPS *lmp, int argc, char **argv)
    : Fix(lmp, argc, argv), _lmp(lmp), _use2D(lmp->domain->dimension == 2),
      _timestepCounter(0) {
  if (argc != 6) {
    error->all(
        FLERR,
        "Specify the mamico fix as 'fix id group-id mamico maxCells seed "
        "cutoffRadius' where maxCells is the maximum process-local number of "
        "macroscopic cells (incl. ghost layer), seed is a random seed and "
  ...
  std::cout << "Max. number of local macroscopic cells to be allocated: "
            << atoi(argv[3]) << ", random seed: " << seed << std::endl;
#endif

  // access MaMiCo via InterfaceProvider
  if (_use2D) {
    MamicoLammpsMDSolverInterface<2> *mdSolverInterface2D =
        new MamicoLammpsMDSolverInterface<2>(_lmp, seed, maxCells,
                                             cutoffRadius);
    if (mdSolverInterface2D == NULL)
      error->all(FLERR, "FixMamico: mdSolverInterface2D==NULL!");
    coupling::interface::MamicoInterfaceProvider<MamicoCell, 2>::getInstance()
        .setMDSolverInterface(mdSolverInterface2D);
  } else {
    MamicoLammpsMDSolverInterface<3> *mdSolverInterface3D =
        new MamicoLammpsMDSolverInterface<3>(_lmp, seed, maxCells,
                                             cutoffRadius);
    if (mdSolverInterface3D == NULL)
      error->all(FLERR, "FixMamico: mdSolverInterface3D==NULL!");
    coupling::interface::MamicoInterfaceProvider<MamicoCell, 3>::getInstance()
        .setMDSolverInterface(mdSolverInterface3D);
  }
}
  ...
  if (_use2D) {
    coupling::interface::MDSolverInterface<MamicoCell, 2> *interface =
        coupling::interface::MamicoInterfaceProvider<
            MamicoCell, 2>::getInstance().getMDSolverInterface();
    if (interface != NULL)
      delete interface;
    coupling::interface::MamicoInterfaceProvider<MamicoCell, 2>::getInstance()
    ...
    coupling::interface::MDSolverInterface<MamicoCell, 3> *interface =
        coupling::interface::MamicoInterfaceProvider<
            MamicoCell, 3>::getInstance().getMDSolverInterface();
    if (interface != NULL)
      delete interface;
    coupling::interface::MamicoInterfaceProvider<MamicoCell, 3>::getInstance()
        .setMDSolverInterface(NULL);
