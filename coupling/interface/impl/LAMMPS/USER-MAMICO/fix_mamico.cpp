...
LAMMPS_NS::FixMamico::FixMamico(LAMMPS *lmp,int argc, char **argv): Fix(lmp,argc,argv),
_lmp(lmp),
_use2D(lmp->domain->dimension==2),
_timestepCounter(0)
{
  if (argc != 6){
    error->all(FLERR, "Specify the mamico fix as 'fix id group-id mamico maxCells seed cutoffRadius' where maxCells is the maximum process-local number of macroscopic cells (incl. ghost layer), seed is a random seed and cutoffRadius is the cut-off radius in the respective LJ simulation");
  }
  ...
  std::cout << "Max. number of local macroscopic cells to be allocated: " << atoi(argv[3]) << ", random seed: " << seed << std::endl;
  #endif
  ...
  if (_use2D){
    ...
    if (mdSolverInterface2D==NULL) error->all(FLERR, "FixMamico: mdSolverInterface2D==NULL!");
    ...
    if (mdSolverInterface3D==NULL) error->all(FLERR, "FixMamico: mdSolverInterface3D==NULL!");
...
LAMMPS_NS::FixMamico::~FixMamico(){
  // destroy interface
  if (_use2D){
    coupling::interface::MDSolverInterface<MamicoCell,2> *interface = coupling::interface::
      MamicoInterfaceProvider<MamicoCell,2>::getInstance().getMDSolverInterface();
    ...
    coupling::interface::MDSolverInterface<MamicoCell,3> *interface = coupling::interface::
      MamicoInterfaceProvider<MamicoCell,3>::getInstance().getMDSolverInterface();
    if (interface != NULL) delete interface;
...
void LAMMPS_NS::FixMamico::pre_force(int vflag){
  ...
  #endif
  // branching for 2D/3D
  if (_use2D){ modifyMDSystem<2>(); }
  else {       modifyMDSystem<3>(); }
  #if (COUPLING_MD_DEBUG==COUPLING_MD_YES)
...
void LAMMPS_NS::FixMamico::post_force(int vflag){
  ...
  #endif
  // branching 2D/3D
  if (_use2D){ modifyMomentumAndTemperature<2>(); }
  else {       modifyMomentumAndTemperature<3>(); }
  #if (COUPLING_MD_DEBUG==COUPLING_MD_YES)
