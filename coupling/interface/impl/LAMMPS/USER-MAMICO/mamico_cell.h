...
namespace LAMMPS_NS {
  // implements the storage of molecule information in form of 2D pointer structures
  ...
  class MamicoCell {
    public:
      ...
      void deleteAtom(int id){
        ...
        for (unsigned int i = 0; i < size; i++){ if (_atoms[i]==id) index = i; }
        // if atom is contained, delete it from vector; therefore, shift entries towards front of vector and remove last vector entry
        if (index != size){
          for (unsigned int i = index; i<size-1; i++){ _atoms[i] = _atoms[i+1]; }
          _atoms.pop_back();
        }
      }
