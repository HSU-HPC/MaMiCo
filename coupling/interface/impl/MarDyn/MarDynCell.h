...
class MarDynCell {
public:
  MarDynCell(ParticleCell *cell, double cutoffRadius)
      : _myCell(cell), _cutoffRadius(cutoffRadius) {
    _molecules = &(cell->getParticlePointers());
  }
  ...
  bool continueIteration() const {
    return (_moleculeIter != _molecules->end());
  }
