 ...
 * cell size Other LinkedCell parameters are set according to this cell size
 * 	@author Hanno Flohr
 */
  ...
  LinkedCellsForCoupling(
    ...
    for (int d = 0; d < 3; d++) {
      ...
      _boxWidthInNumCells[d] =
          (_boundingBoxMax[d] - _boundingBoxMin[d]) / _cellLength[d];
      ...
      _cellsPerDimension[d] =
          _boxWidthInNumCells[d] + 2 * _haloWidthInNumCells[d];
    ...
    if (_cellsInCutoff < 1) {
      global_log->error()
          << "LinkedCellsForCoupling (constructor): cellsInCutOff < 1! cell "
    ...
    // region a parallelization is not possible (with the used algorithms). If a
    // particle leaves the box, it would need to be communicated to the two next
    // neighbors.
