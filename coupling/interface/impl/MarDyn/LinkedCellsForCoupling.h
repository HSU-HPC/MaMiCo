 ...
 * 	Gives a Constructor that provides the creation of cells based on a given cell size
...
public:
	LinkedCellsForCoupling(
			tarch::la::Vector<3,double> mamicoCellSize, tarch::la::Vector<3,unsigned int> linkedCellsPerMacroscopicCell,
		...
		for(int d=0; d<3; d++) {
			...
			if(_boxWidthInNumCells[d] == 0) _boxWidthInNumCells[d] = 1;
		...
		global_log->debug() << "Cells per dimension (incl. halo): " << _cellsPerDimension[0] << ", " << _cellsPerDimension[1] << ", " << _cellsPerDimension[2] << std::endl;

		_cellsInCutoff = floor( _cellLength[0] / cutoffRadius );
		if(_cellsInCutoff < 1) {
			global_log->error() << "LinkedCellsForCoupling (constructor): cellsInCutOff < 1! cell length should be >= cutoff radius" << std::endl;
			exit(1);
		}

		_cells.resize(cellCount);

		// If the width of the inner region is less than the width of the halo region a parallelization is not possible (with the used algorithms).
		// If a particle leaves the box, it would need to be communicated to the two next neighbors.
		if(_boxWidthInNumCells[0] < 2* _haloWidthInNumCells[0] ||
			...
			global_log->error() << "LinkedCellsForCoupling (constructor): width of inner region is less than the width of halo." << std::endl;
			global_log->error() << "_cellsPerDimension: " << _cellsPerDimension[0] << " / " << _cellsPerDimension[1] << " / " << _cellsPerDimension[2] << std::endl;
			global_log->error() << "_haloWidthInNumCells: " << _haloWidthInNumCells[0] << " / " << _haloWidthInNumCells[1] << " / " << _haloWidthInNumCells[2] << std::endl;
			exit(5);
		}
