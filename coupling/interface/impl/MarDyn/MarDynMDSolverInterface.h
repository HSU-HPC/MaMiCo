...
public:
  ...
  virtual MarDynCell &getLinkedCell(
    ...
    if (_mySimulation->getContainerType() == 0) {
      ...
      for (int d = 0; d < 3; d++) {
        requestedCellIndex[d] = (int)
            floor(macroscopicCellIndex[d] * macroCellSize[d] / mdCellsize[d]);
      ...
      std::cout << "ERROR (MarDynSolverInterface): Container type is not set "
                   "or supported! (" << _mySimulation->getContainerType() << ")"
                << std::endl;
      exit(1);
    }
 ...
 1.0 is used.
  ...
  getInitialVelocity(const tarch::la::Vector<3, double> &meanVelocity,
    ...
    for (unsigned int d = 1; d < 3; d++)
      random[d] = 2.0 * M_PI * tarch::utils::RandomNumberService::getInstance()
  ...
  virtual void setupPotentialEnergyLandscape(
    ...
    //the getPotentialEnergy() method
    //of the MoleculeWrapper interface. Therefore this method does nothing for
    //now.
  }
  ...
  calculateForceAndEnergy(coupling::interface::Molecule<3> &molecule) {
      ...
      if (!moleculeFound) {
        global_log->debug()
            << "MarDynMDSolverInterface::calculateForceAndEnergy(): Molecule "
               "not found, creating new molecule..." << std::endl;
    ...
    potentialEnergy = _mySimulation->getMolecules()
        ->getEnergy(_mySimulation->getParticlePairsHandler(), moleculeInSim,
                    legacyCellProcessor);
    if (!moleculeFound) {
        ...
        for (loop[0] = start[0]; loop[0] < end[0]; loop[0]++) {
          unsigned int cellIndex =
              (loop[2] * (boxWidthInNumCells[1] + _haloWidthInNumCells) +
               loop[1]) * (boxWidthInNumCells[0] + _haloWidthInNumCells) +
              loop[0];
