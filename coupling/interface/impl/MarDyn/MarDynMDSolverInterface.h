...
public:
	MarDynMDSolverInterface(MarDynCoupledSimulation* simulation) :
			...
			_mySimulation(simulation), _cutoffRadius(_mySimulation->getLJCutoff()), _tolerance(1.0e-8) {
	...
	virtual ~MarDynMDSolverInterface() {
		//clear the MarDyn cell storage
                for (unsigned int i = 0; i < _marDynCellPtrStorage.size(); i++){
                  if (_marDynCellPtrStorage[i] != NULL){ delete _marDynCellPtrStorage[i]; _marDynCellPtrStorage[i] = NULL;}
                }
	...
	virtual MarDynCell& getLinkedCell(
		...
		for(unsigned int d=0; d<3; d++) {
			if(macroscopicCellIndex[d]==0) {
				std::cout << "ERROR in MarDynMDSolverInterface::getLinkedCell(): macroscopic cell index out of range for linked cells!" << std::endl;
				exit(EXIT_FAILURE);
			}
		}
		...
		if(_mySimulation->getContainerType() == 0) {
			...
			for(int d=0; d<3; d++) {
				...
				//adjust requested cell index if multiple linked cells per macroscopic cell are used
				if(linkedCellsPerMacroscopicCell[d]>1) requestedCellIndex[d] -= (linkedCellsPerMacroscopicCell[d] - 1) - linkedCellInMacroscopic[d];
			}
			//compute the Mardyn cell index of the requested cell
			unsigned int cellIndex = (requestedCellIndex[2] * (boxWidthInNumCells[1]+_haloWidthInNumCells) + requestedCellIndex[1]) *
										(boxWidthInNumCells[0]+_haloWidthInNumCells) + requestedCellIndex[0];
			...
			std::cout << "ERROR (MarDynSolverInterface): Container type is not set or supported! (" << _mySimulation->getContainerType() << ")" << std::endl;
			exit(1);
		}
	...
	virtual tarch::la::Vector<3,double> getGlobalMDDomainSize() const {
		...
		for(int i=0; i<3; i++) { globalMDsize[i] = _mySimulation->getEnsemble()->domain()->rmax()[i] - _mySimulation->getEnsemble()->domain()->rmin()[i]; }
		return globalMDsize;
	}
	...
	virtual tarch::la::Vector<3,double> getGlobalMDDomainOffset() const {
		...
		for(int i=0; i<3; i++) { globalMDoffset[i] = _mySimulation->getEnsemble()->domain()->rmin()[i]; }
	 ...
	 * The Boltzmann's constant is not stored in MarDyn. When needed, the value 1.0 is used.
	...
	virtual void getInitialVelocity(
			const tarch::la::Vector<3,double>& meanVelocity,
			const double& kB,
			const double& temperature,
			tarch::la::Vector<3,double>& initialVelocity
	) const {
	...
	virtual void deleteMoleculeFromMDSimulation(
			const coupling::interface::Molecule<3>& molecule,
			MarDynCell& cell
	) {
		//molecule position
		...
		for(cell.begin(); cell.continueIteration(); cell.next()) {
			...
			for(int d=0; d<3; d++) curMoleculePosition[d] = curMolecule->r(d);
			...
			else moleculeFound = true;
		}

		//if a molecule to delete was found delete it, else through error message and continue
		if(success) {
		...
		else std::cout << "ERROR (delete molecule): no molecule found at desired delete position!" << std::endl;
	}
	...
	virtual void addMoleculeToMDSimulation(
		...
		//create a MarDynMolecule and set molecule id to maxiID+1
                ...
                const tarch::la::Vector<3,double> f  (molecule.getForce()); double force[3] = {f[0],f[1],f[2]};
                
		mdmw.setid( _mySimulation->getMaxID() + 1 );
                for (unsigned int d = 0; d < 3; d++){
                  mdmw.setr(d,pos[d]); mdmw.setv(d,vel[d]);
                }
                mdmw.setF(force);
	...
	virtual void setupPotentialEnergyLandscape(
		...
		//The potential energy is calculated for each molecule individually using the getPotentialEnergy() method
		//of the MoleculeWrapper interface. Therefore this method does nothing for now.
	}
	...
	virtual tarch::la::Vector<3,unsigned int> getLinkedCellIndexForMoleculePosition(
		...
		if(_mySimulation->getContainerType() == 0) {
			...
			std::cout << "ERROR (MarDynSolverInterface): Container type is not supported or unknown! (" << _mySimulation->getContainerType() << ")" << std::endl;
			exit(1);
		}

		//compute cell index for each dimension
		for(int d=0; d<3; d++) cellIndex[d] = (int) floor( (position[d] - haloBoundingBoxMin[d]) / cellLength[d]);
	...
	virtual void calculateForceAndEnergy(
		...
		// use a legacy cell processor  (the vectorized version seems not to work for molecules that are not part of the sim)
		LegacyCellProcessor legacyCellProcessor( _cutoffRadius, _mySimulation->getLJCutoff(), _mySimulation->getTersoffCutoff(), _mySimulation->getParticlePairsHandler());

		if(_mySimulation->getContainerType() == 0) {
			//get cell index for the molecule
			LinkedCells* lc = (LinkedCells*) _mySimulation->getMolecules();
			Molecule refMolecule; for (int d = 0; d < 3; d ++) refMolecule.setr(d,molecule.getPosition()[d]);
			...
			//if no molecule at this position was found create new mardyn molecule and compute force for it
			if(!moleculeFound) {
				global_log->debug() << "MarDynMDSolverInterface::calculateForceAndEnergy(): Molecule not found, creating new molecule..." << std::endl;
				//create new Mardyn molecule
				moleculeInSim = new Molecule(_mySimulation->getMaxID()+1, _mySimulation->getEnsemble()->component(0),
						molecule.getPosition()[0], molecule.getPosition()[1], molecule.getPosition()[2],
						molecule.getVelocity()[0], molecule.getVelocity()[1], molecule.getVelocity()[2],
				...
				for(int d=0; d<3; d++) force[d] = moleculeInSim->F(d);
			}
		}
		else {
			std::cout << "ERROR (MarDynSolverInterface): Container type is not supported or unknown! (" << _mySimulation->getContainerType() << ")" << std::endl;
...
std::cout << "here5" << std::endl;
		...
		potentialEnergy = _mySimulation->getMolecules()->getEnergy(_mySimulation->getParticlePairsHandler(), moleculeInSim, legacyCellProcessor);
                if (!moleculeFound){ delete moleculeInSim; }
std::cout << "here6" << std::endl;
	...
	/* called after insertion/deletion of molecules from the MD simulation by MaMiCo */
...
protected:
		...
		for (loop[2] = start[2]; loop[2] < end[2]; loop[2]++){
			for (loop[1] = start[1]; loop[1] < end[1]; loop[1]++){
				for (loop[0] = start[0]; loop[0] < end[0]; loop[0]++){
					unsigned int cellIndex = (loop[2] * (boxWidthInNumCells[1]+_haloWidthInNumCells) + loop[1]) * (boxWidthInNumCells[0]+_haloWidthInNumCells) + loop[0];
	 ...
	 * 	to avoid memory leak the pointers are stored and this vector cleared in the Destructor
	 */
