 ...
 */
class MarDynMoleculeIterator : public coupling::interface::MoleculeIterator<MarDynCell,3> {
public:
	/*	Constructor setting the iterator for the input MarDynCell,
	 * 	the cutoff radius to the value of the input cell and
	 * 	the MarDynMolecule '_mdMolecule' to NULL (prevents memory leaks in get-methods)
	 */
	MarDynMoleculeIterator(MarDynCell& cell) :
		coupling::interface::MoleculeIterator<MarDynCell,3>(cell), _cutoffRadius(cell.getCutoffRadius()){
	}
	...
	virtual bool continueIteration() const { return coupling::interface::MoleculeIterator<MarDynCell,3>::_cell.continueIteration(); }

	/* sets iterator to next molecule of the cell */
	virtual void next() { coupling::interface::MoleculeIterator<MarDynCell,3>::_cell.next(); }
