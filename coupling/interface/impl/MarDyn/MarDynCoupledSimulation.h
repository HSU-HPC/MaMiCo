...
public:
    ...
    while (inputfilestream) {
      ...
      if (token == "phaseSpaceFile") {
        ...
        if (phaseSpaceFileFormat == "OldStyle") {
          ...
          global_log->info()
              << "phaseSpaceFileFormat is Generator!" << std::endl;
        ...
        if (token == "DomainDecomposition") {
          ...
          if (_mpiGridDimensions[0] > 0 && _mpiGridDimensions[1] > 0 &&
            ...
            int gridDims[3]{(int)_mpiGridDimensions[0],
                            (int)_mpiGridDimensions[1],
        ...
        if (token == "ResultWriter") {
          ...
          global_log->debug()
              << "ResultWriter '" << outputPathAndPrefix << "'.\n";
          ...
          if (_particleContainerType == LINKED_CELL) {
            ...
            global_log->warning()
                ...
                << std::endl;
            global_log->warning()
                << "Generating no VTK output for the grid!" << std::endl;
          }
#else
          Log::global_log->error()
              << std::endl
          ...
          global_log->warning()
              << "Unknown output plugin " << token << std::endl;
        }
      } else if (token == "cutoffRadius") {
        double rc;
        inputfilestream >> rc;
        ...
        if (token != "")
          global_log->warning()
              << "Did not process unknown token " << token << std::endl;
      }
    }
  ...
  void setInitValuesForCoupling() {
    ...
    // _simstep==_initCanonical by setting it to zero, no initial temperature
