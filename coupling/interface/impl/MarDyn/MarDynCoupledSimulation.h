...
public:
  ...
  MarDynCoupledSimulation(
      tarch::la::Vector<3, double> mamicoCellSize,
      tarch::la::Vector<3, unsigned int> linkedCellsPerMacroscopicCell,
      tarch::la::Vector<3, unsigned int> mpiGridDimensions)
      : Simulation(), _mamicoCellSize(mamicoCellSize),
        _linkedCellsPerMacroscopicCell(linkedCellsPerMacroscopicCell),
        _macroscopicCellService(NULL), _couplingOn(true),
  ...
  MarDynCoupledSimulation(
      ...
      : Simulation(), _mamicoCellSize(mamicoCellSize),
        _linkedCellsPerMacroscopicCell(linkedCellsPerMacroscopicCell),
        _macroscopicCellService(NULL), _couplingOn(true),
        _mpiGridDimensions(tarch::la::Vector<3, unsigned int>(0)),
        ensemble(NULL) {
    global_simulation = this;
  }
 ...
 the coupling removed
  ...
  void initConfigOldstyle(const std::string &inputfilename) {
    ...
    if (!inputfilestream.is_open()) {
      global_log->error() << "Could not open file " << inputfilename
                          << std::endl;
      exit(1);
    }
    ...
    // The first line of the config file has to contain the token
    // "MDProjectConfig"
    inputfilestream >> token;
    if ((token != "mardynconfig") && (token != "MDProjectConfig")) {
      global_log->error() << "Not a mardynconfig file! First token: " << token
                          << std::endl;
      exit(1);
    }
    ...
    while (inputfilestream) {
      ...
      if (token == "phaseSpaceFile") {
        ...
        if (phaseSpaceFileFormat == "OldStyle") {
          ...
          _inputReader =
              GeneratorFactory::loadGenerator(generatorName, inputFile);
          _inputReader->readPhaseSpaceHeader(_domain, timestepLength);
        } else {
...
#ifndef ENABLE_MPI
        global_log->warning()
            << "Input file demands parallelization, but the current "
        ...
        if (token == "DomainDecomposition") {
          //needed for coupled simulation to assure that Mardyn works with the
          //same grid dimensions as MaMiCo
          if (_mpiGridDimensions[0] > 0 && _mpiGridDimensions[1] > 0 &&
              _mpiGridDimensions[2] > 0) {
            int gridDims[3] {
              (int) _mpiGridDimensions[0], (int) _mpiGridDimensions[1],
                  (int) _mpiGridDimensions[2]
            }
            ;
            delete _domainDecomposition;
            _domainDecomposition =
                (DomainDecompBase *)new DomainDecomposition(gridDims);
          }
        }
#endif
        ...
        if (_domainDecomposition == NULL) {
          global_log->error()
              << "_domainDecomposition is NULL! Probably you compiled for MPI, "
                 "but didn't specify line \"parallelization\" before line "
        ...
        if (token == "LinkedCells") {
          ...
          _moleculeContainer =
              new LinkedCells(bBoxMin, bBoxMax, _cutoffRadius, _LJCutoffRadius,
                              cellsInCutoffRadius);
        } else if (token == "LinkedCellsForCoupling") {
          //LINKED CELLS FOR COUPLING
          //check if mamico values are set, abort otherwise
          if (_mamicoCellSize[0] == 0.0 || _mamicoCellSize[1] == 0.0 ||
              _mamicoCellSize[2] == 0.0 ||
              _linkedCellsPerMacroscopicCell[0] == 0 ||
              _linkedCellsPerMacroscopicCell[1] == 0 ||
              _linkedCellsPerMacroscopicCell[2] == 0) {
            global_log->error()
                << "Input file demands 'LinkedCellsForCoupling' as particle "
                   "container, "
          ...
          if (this->_LJCutoffRadius == 0.0)
            _LJCutoffRadius = this->_cutoffRadius;
          _moleculeContainer = new LinkedCellsForCoupling(
              _mamicoCellSize, _linkedCellsPerMacroscopicCell, bBoxMin, bBoxMax,
              _cutoffRadius);
          //LINKED CELLS FOR COUPLING
        } else {
        ...
        if (token == "ResultWriter") {
          ...
          inputfilestream >> writeFrequency >> outputPathAndPrefix;
          _outputPlugins.push_back(
              new XyzWriter(writeFrequency, outputPathAndPrefix, true));
          ...
          inputfilestream >> writeFrequency >> outputPathAndPrefix;
          _outputPlugins.push_back(
              new CheckpointWriter(writeFrequency, outputPathAndPrefix, true));
          ...
          inputfilestream >> writeFrequency >> outputPathAndPrefix;
          _outputPlugins.push_back(
              new PovWriter(writeFrequency, outputPathAndPrefix, true));
          ...
          inputfilestream >> writeFrequency >> outputPathAndPrefix;
          _outputPlugins.push_back(
              new VISWriter(writeFrequency, outputPathAndPrefix, true));
          ...
          inputfilestream >> writeFrequency >> outputPathAndPrefix;
          _outputPlugins.push_back(
              new VTKMoleculeWriter(writeFrequency, outputPathAndPrefix));
          global_log->debug() << "VTKWriter " << writeFrequency << " '"
                              << outputPathAndPrefix << "'.\n";
#else
          Log::global_log->error()
              << std::endl
              << "VKT-Plotting demanded, but programme compiled without -DVTK!"
              << std::endl << std::endl;
#endif
          ...
          if (_particleContainerType == LINKED_CELL) {
            _outputPlugins.push_back(
                new VTKGridWriter(writeFrequency, outputPathAndPrefix));
            ...
            global_log->warning()
                ...
                << std::endl;
            global_log->warning() << "Generating no VTK output for the grid!"
                                  << std::endl;
          }
#else
          Log::global_log->error()
          ...
          global_log->warning() << "Unknown output plugin " << token
                                << std::endl;
        }
      } else if (token == "cutoffRadius") {
        double rc;
        ...
        if (token != "")
          global_log->warning() << "Did not process unknown token " << token
                                << std::endl;
      }
    }
  ...
  void setInitValuesForCoupling() {
    //The system is equilibrated at a higher temperature until
    //_simstep==_initCanonical
    ...
    if (_macroscopicCellService == NULL) {
      _macroscopicCellService = (coupling::services::MacroscopicCellServiceImpl<
          MarDynCell, 3> *)coupling::interface::MamicoInterfaceProvider<
          MarDynCell, 3>::getInstance().getMacroscopicCellService();
    }

    //set the Ensemble and update global variables
    ensemble = new CanonicalEnsemble(
        _moleculeContainer, global_simulation->getEnsemble()->components());
    ensemble->updateGlobalVariable(NUM_PARTICLES);
    global_log->debug()
        << "Number of particles in the ensemble: " << ensemble->N()
        << std::endl;
  ...
  void simulateOneMDTimestep(const unsigned int &timestep) {
    ...
    global_log->debug() << "simulation time: " << getSimulationTime()
                        << std::endl;

    //exectue MaMiCo coupling steps (if coupling is turned on)
    if (_couplingOn)
      executeCouplingSteps();
    ...
    //force calculation
    global_log->debug() << "Traversing pairs for force calculation.."
                        << std::endl;
    _moleculeContainer->traverseCells(*_cellProcessor);

    //clear halo
    global_log->debug() << "Delete outer particles / clearing halo.."
                        << std::endl;
    _moleculeContainer->deleteOuterParticles();

    //inform integrator about calculated forces & execute steps between force
    //calculation and end of timestep
    global_log->debug() << "Inform integrator about calculated forces.."
                        << std::endl;
  ...
  bool removeMoleculeFromContainer(unsigned long id) {
    ...
    for (curMolecule = _moleculeContainer->begin();
         curMolecule != _moleculeContainer->end();
         curMolecule = _moleculeContainer->next()) {
...
protected:
  ...
  coupling::services::MacroscopicCellServiceImpl<MarDynCell, 3> *
