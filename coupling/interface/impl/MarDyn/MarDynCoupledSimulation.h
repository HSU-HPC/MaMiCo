...
public:
  ...
  void initConfigOldstyle(const std::string &inputfilename) {
    ...
    if ((token != "mardynconfig") && (token != "MDProjectConfig")) {
      global_log->error() << "Not a mardynconfig file! First token: " << token << std::endl;
      exit(1);
    }
    ...
    while (inputfilestream) {
      ...
      if (token == "phaseSpaceFile") {
        ...
        if (phaseSpaceFileFormat == "OldStyle") {
          ...
          global_log->info() << "phaseSpaceFileFormat is Generator!" << std::endl;
          ...
          global_log->error() << "Don't recognize phasespaceFile reader " << phaseSpaceFileFormat << std::endl;
          exit(1);
        }
...
#ifndef ENABLE_MPI
        global_log->warning() << "Input file demands parallelization, but the current "
        ...
        if (_domainDecomposition == NULL) {
          global_log->error() << "_domainDecomposition is NULL! Probably you compiled for MPI, "
        ...
        if (token == "ResultWriter") {
          ...
          std::string outputPathAndPrefix;
          inputfilestream >> writeFrequency >> outputPathAndPrefix;
          _outputPlugins.push_back(new VISWriter(writeFrequency, outputPathAndPrefix, true));
        ...
        if (token != "")
          global_log->warning() << "Did not process unknown token " << token << std::endl;
      }
    }
