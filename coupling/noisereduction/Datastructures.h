...
private:
  ...
  friend class coupling::noisereduction::Patch<
      dim>; // patches need direct access to their field's _data for memory
            // efficiency
};

template <unsigned int dim, typename T>
std::allocator<T> coupling::noisereduction::Field<dim, T>::allo;
...
private:
  ...
  void fillFromBasefield(const Flowfield<dim> &basefield,
    ...
    static_assert(dim == 2 || dim == 3,
                  "ERROR noisereduction::Patch::fillFromBasefield only "
