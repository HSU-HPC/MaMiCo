...
class coupling::noisereduction::Field{
public:
...
private:
	...
	friend class coupling::noisereduction::Patch<dim>; // patches need direct access to their field's _data for memory efficiency
};
...
*/
template<unsigned int dim> 
class coupling::noisereduction::Patch{
public:
...
private:
	...
	void fillFromBasefield(const Flowfield<dim> &basefield, const tarch::la::Vector<dim,unsigned int> &pos, const unsigned int &t){
		static_assert(dim==2 || dim==3, "ERROR noisereduction::Patch::fillFromBasefield only implemented for 2D and 3D");
