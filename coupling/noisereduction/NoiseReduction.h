...
/** 
 *  Interface for noise reduction algorithm, that is for smoothing operations to be applied on data from MD
...
class coupling::noisereduction::NoiseReduction {
  public:
     ...
     *  data to the macroscopic solver, but after the call to TransferStrategy::processInnerMacroscopicCellBeforeSendingMDSolverData
    ...
    /** Is called for every macroscopic cell right before sending the macroscopicMass and -Momentum
     *  data to the macroscopic solver, but after the call to TransferStrategy::processOuterMacroscopicCellBeforeSendingMDSolverData
