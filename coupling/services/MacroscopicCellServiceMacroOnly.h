...
public:
  MacroscopicCellServiceMacroOnly(
      ...
      const coupling::configurations::MacroscopicCellConfiguration<dim>&
          macroscopicCellConfiguration, // configuration for macroscopic cells and respective plotting
    ...
    if (_macroscopicSolverInterface == NULL) {
      std::cout << "ERROR coupling::services::MacroscopicCellServiceMacroOnly::MacroscopicCellServiceMacroOnly(): _macroscopicSolverInterface==NULL!"
                << std::endl;
      exit(EXIT_FAILURE);
    }
  }
...
private:
  ...
  coupling::IndexConversion<dim>* initIndexConversion(tarch::la::Vector<dim, double> macroscopicCellSize, tarch::la::Vector<dim, unsigned int> numberProcesses,
    ...
    if (ic == NULL) {
      std::cout << "coupling::services::MacroscopicCellServiceImpl::initIndexConversion(): ic==NULL!" << std::endl;
      exit(EXIT_FAILURE);
    }
    return ic;
  }
