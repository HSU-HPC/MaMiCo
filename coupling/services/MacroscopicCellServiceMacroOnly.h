...
namespace coupling {
  namespace services {
    ...
    class MacroscopicCellServiceMacroOnly;
  }
}



/** class for functionality of data exchange in hybrid Micro-Macro simulations. This implementation assumes that MD does not run on this rank.
 *  We therefore do not have any particle-related operations implemented here. Besides, no grid for data exchange is allocated (except for some
...
class coupling::services::MacroscopicCellServiceMacroOnly : public coupling::services::MacroscopicCellService<dim> {
  public:
    MacroscopicCellServiceMacroOnly(
      ...
      tarch::la::Vector<dim,double> globalMDDomainSize, // domain size of MD simulation -> required for index conversion
      tarch::la::Vector<dim,double> globalMDDomainOffset, // domain offset of MD simulation -> required for index conversion
      ...
      _indexConversion(
        ...
        globalMDDomainSize,globalMDDomainOffset,
    ...
    virtual ~MacroscopicCellServiceMacroOnly(){
      if (_indexConversion!=NULL){
        delete _indexConversion; _indexConversion=NULL;
      }
    }
  ...
  private:
    ...
    coupling::IndexConversion<dim>* initIndexConversion(
      ...
      unsigned int intNumberProcesses = numberProcesses[0];
      for(unsigned int d = 1;d < dim ; ++d) { intNumberProcesses *= numberProcesses[d];}
      coupling::IndexConversion<dim> *ic = new coupling::IndexConversion<dim>(globalNumberMacroscopicCells,numberProcesses,rank,globalMDDomainSize,globalMDDomainOffset,parallelTopologyType,topologyOffset, rank % intNumberProcesses);
