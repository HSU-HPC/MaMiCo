...
public:
  MultiMDCellService(
      ...
      const char
          *filterPipelineConfiguration, // location of the .xml file containing
                                        // the static filter pipeline config
      const tarch::utils::MultiMDService<dim> &multiMDService, int tws = 0)
      : _localNumberMDSimulations((unsigned int)mdSolverInterfaces.size()),
        _totalNumberMDSimulations(totalNumberMDSimulations),
        _macroscopicCellServices(NULL),
        _topologyOffset(computeTopologyOffset(numberProcesses, rank)),
        _tws(tws),
        _intNumberProcesses(computeScalarNumberProcesses(numberProcesses)),
        _macroscopicSolverInterface(macroscopicSolverInterface),
        _filterPipelineConfiguration(filterPipelineConfiguration),
        _multiMDService(multiMDService), _rank(rank),
    ...
    if (mdSolverInterfaces.size() == 0) {
      std::cout << "ERROR: Zero MD instances on rank " << rank
                << ". Maybe you forgot to adjust number-md-simulations in "
                   "couette.xml?"
                << std::endl;
      exit(EXIT_FAILURE);
    }
    ...
    _macroscopicCellServices =
        new coupling::services::MacroscopicCellService<dim>
    ...
    // intNumberProcesses
    //    -> this yields
    //    localNumberMDSimulations*topologyOffset/intNumberProcesses MD
    //    simulations before the actual block of ranks
    for (unsigned int i = 0;
         i < _localNumberMDSimulations * _topologyOffset / _intNumberProcesses;
         i++) {
      _macroscopicCellServices[i] =
          new coupling::services::MacroscopicCellServiceMacroOnly<dim>(
              i, macroscopicSolverInterface, numberProcesses, rank,
              mdDomainSize, mdDomainOffset, parallelTopologyConfiguration,
              macroscopicCellConfiguration,
              (i / _localNumberMDSimulations) * _intNumberProcesses);
      if (_macroscopicCellServices[i] == NULL) {
        std::cout << "ERROR "
                     "coupling::services::MultiMDCellService::"
                     "MultiMDCellService(...): _macroscopicCellServices["
                  << i << "]==NULL!" << std::endl;
        exit(EXIT_FAILURE);
      }
    }
    ...
    for (unsigned int i =
         ...
         i < _localNumberMDSimulations *
                 (_topologyOffset + _intNumberProcesses) / _intNumberProcesses;
         i++) {
      _macroscopicCellServices[i] =
          new coupling::services::MacroscopicCellServiceImpl<LinkedCell, dim>(
              i,
              mdSolverInterfaces[i - _localNumberMDSimulations *
                                         _topologyOffset / _intNumberProcesses],
              macroscopicSolverInterface, numberProcesses, rank,
              particleInsertionConfiguration, momentumInsertionConfiguration,
              boundaryForceConfiguration, transferStrategyConfiguration,
              parallelTopologyConfiguration, thermostatConfiguration,
              numberMDTimestepsPerCouplingCycle, macroscopicCellConfiguration,
              filterPipelineConfiguration, multiMDService, _topologyOffset,
              _tws);
      if (_macroscopicCellServices[i] == NULL) {
        std::cout << "ERROR "
                     "coupling::services::MultiMDCellService::"
    ...
    // topology offset
    for (unsigned int i = _localNumberMDSimulations *
                          (_topologyOffset + _intNumberProcesses) /
                          _intNumberProcesses;
  ...
  coupling::filtering::FilterPipeline<dim> &
  getPostMultiInstanceFilterPipeline() {
    return _postMultiInstanceFilterPipeline;
  }
  ...
  void sendFromMacro2MD(
    ...
    for (unsigned int i = 0; i < _totalNumberMDSimulations; i++) {
      ...
      _macroscopicCellServices[i]->sendFromMacro2MD(
          macroscopicCellsFromMacroscopicSolver,
  ...
  double sendFromMD2Macro(
      ...
      const unsigned int *const globalCellIndicesFromMacroscopicSolver) {
    double res = 0;
    auto size = macroscopicCellsFromMacroscopicSolver
                    .size(); // we assume globalCellIndicesFromMacroscopicSolver
    ...
    if (_macroscopicCells.empty()) {
      ...
      for (unsigned int c = _macroscopicCells.size(); c < size; c++)
        _macroscopicCells.push_back(
            new coupling::datastructures::MacroscopicCell<dim>());
    }
 ...
 */
    ...
    if (_postMultiInstanceFilterPipeline == nullptr) {
      ...
      _postMultiInstanceFilterPipeline =
          new coupling::filtering::FilterPipeline<dim>(
              _macroscopicCells, coupling::filtering::Scope::postMultiInstance,
    ...
    for (unsigned int i = 0; i < size; i++) {
      ...
      _macroscopicCells[i]->setMacroscopicMomentum(
    ...
    for (unsigned int l = 0; l < _totalNumberMDSimulations; l++) {
      ...
      res += _macroscopicCellServices[l]->sendFromMD2Macro(
          ...
          globalCellIndicesFromMacroscopicSolver);
      for (unsigned int i = 0; i < size; i++) {
        _macroscopicCells[i]->addMacroscopicMass(
            macroscopicCellsFromMacroscopicSolver[i]->getMacroscopicMass());
        _macroscopicCells[i]->addMacroscopicMomentum(
    ...
    // average data
    for (unsigned int i = 0; i < size; i++) {
      _macroscopicCells[i]->setMacroscopicMass(
          _macroscopicCells[i]->getMacroscopicMass() /
          _totalNumberMDSimulations);
      _macroscopicCells[i]->setMacroscopicMomentum(
          1.0 / _totalNumberMDSimulations *
          _macroscopicCells[i]->getMacroscopicMomentum());
    }
...
// apply post multi instance FilterPipeline on cell data
    ...
    std::cout << "FP: Now applying post-multi-instance filter pipeline"
              << std::endl;
#endif
    ...
    for (unsigned int i = 0; i < size; i++) {
      macroscopicCellsFromMacroscopicSolver[i]->setMacroscopicMass(
          _macroscopicCells[i]->getMacroscopicMass());
      macroscopicCellsFromMacroscopicSolver[i]->setMacroscopicMomentum(
          _macroscopicCells[i]->getMacroscopicMomentum());
    }
  ...
  void constructFilterPipelines() {
    for (unsigned int md = 0; md < _totalNumberMDSimulations; md++) {
      ...
      if (dynamic_cast<coupling::services::MacroscopicCellServiceImpl<
              LinkedCell, dim> *>(mcs) == nullptr)
        continue;
...
private:
  unsigned int
  computeTopologyOffset(tarch::la::Vector<dim, unsigned int> numberProcesses,
                        unsigned int rank) const {
    // determine topology offset of this rank
    const unsigned int intNumberProcesses =
        computeScalarNumberProcesses(numberProcesses);
    const unsigned int topologyOffset =
        (rank / intNumberProcesses) * intNumberProcesses;
    return topologyOffset;
  }

  unsigned int computeScalarNumberProcesses(
      tarch::la::Vector<dim, unsigned int> numberProcesses) const {
    unsigned int np = numberProcesses[0];
    for (unsigned int d = 1; d < dim; d++) {
      np = np * numberProcesses[d];
    }
    return np;
  }

  const unsigned int
      _localNumberMDSimulations; /** number of MD simulations run on the current
                                    rank. This can differ for different blocks,
                                    i.e. different topologyOffset values. */
  const unsigned int
      _totalNumberMDSimulations; /** total number of MD simulations */
  coupling::services::MacroscopicCellService<dim> *
      *_macroscopicCellServices; /** pointers of MacroscopicCellService type,
                                    one for each MD simulation */
  ...
  std::vector<coupling::datastructures::MacroscopicCell<dim> *>
      _macroscopicCells; /** used to store in MD data in sendFromMDtoMacro */
  std::vector<unsigned int>
      _cellIndices; /** used to store in indexing of the above */
