 ...
 *  MD-To-Macro (M2M) domain (that is the domain of cells that are transfered from MD to continuum)
 *  Note that currently a rework of indexing in MaMiCo takes place, making this obsolete in the future.
 *
 *  @author Felix Maurer
 */
template<unsigned int dim>
class coupling::IndexConversionMD2Macro {
	public:
		 ...
		 *  @param macroscopicSolverInterface the interface to the macroscopic solver in application
		...
		IndexConversionMD2Macro(
				...
				#endif
				/** @todo @felix adapt the params comm and lowestRankInComm for case multimd */
			):
				_ic(indexConversion),
		...
		/** Mimics initMD2MacroDomain's other implementation (see below) where you already have the MD-To-Macro domain at hand.
		 * 	It then (trivially) initializes Md-To-Macro and Outer cells/indices.
		 *
		 *  This function currently only has uses in cases where you do not have access to all cells of the entire domain,
		 *  which is only the case for post multi-instance filtering as of now.
		 *  In this case, the "outer" region is empty.
		 *
		 *  @brief This method assumes the given cells to be the entire m2m-domain (see below).
		 ...
		 *  @param m2mDomainCells Output vector where cells located within the MD-To-Macro cells shall be placed.
		 *  @param m2mIndices Output vector where indices of those MD-To-Macro cells shall be placed.
		 *  @param m2mDomainCells Output vector where cells located NOT within the MD-To-Macro cells shall be placed.
		 *  @param m2mIndices Output vector where indices of those outer cells shall be placed.
		 */
		void initMD2MacroDomain(
		...
		/** Determines the size of the global MD-To-Macro domain, i.e. the number of cells contained in the domain across all ranks.
		 *
		...
		/** Determines the size of this rank's local MD-To-Macro domain, i.e. the number of cells contained in the domain on this rank only.
		 *
		...
		/** Determines the global offset of the MD-To-Macro domain relative to the global MD domain.
		 *
		...
		/** Same as getGlobalVectorCellIndex of base class, but converts in terms of to MD-To-Macro indexing.
		 *  Has the option to "ignore" Ghost Layer cells, i.e. return INT_MAX for all indices in Ghost Layer.
		...
		/** Same as getLocalVectorCellIndex of base class, but converts in terms of to MD-To-Macro indexing.
		 *  Has the option to "ignore" Ghost Layer cells, i.e. return INT_MAX for all indices in Ghost Layer.
		 *  E.g.: Ghost layer at x = 0. Then requesting vector index (x,y,z,..) will return (INT_MAX, y, z,...).
		 *
	...
	private:
		...
		/** IndexConversionMD2Macro is a wrapper class for IndexConversion. This is the underlying IndexConversion. */
		const coupling::IndexConversion<dim>* _ic;
		/** Used to determine which cell indices are received by the Macroscopic Solver. */
		 ...
		 *  Indexing here is relative to the entire MD-Domain (i.e. what is sometimes referred to as MD-Indexing/MaMiCo-Indexing) */
		 ...
		 *  This rank is assumed to manage cell (0,...,0) of both the MD and the MD-To-Macro-domain. */
