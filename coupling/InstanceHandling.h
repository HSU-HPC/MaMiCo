  ...
  ~InstanceHandling() {
    for (unsigned int i = 0; i < _simpleMD.size(); ++i) {
      ...
      _mdSolverInterface[i] = coupling::interface::MamicoInterfaceProvider<
          LinkedCell, dim>::getInstance().getMDSolverInterface();
    }
    _simpleMD.clear();
...
solver and lets the MD simulations run t time steps starting from the time step
T to equilibrate.
...
solver after the new MD instances are equilibrated.
	 *	It should be called right after equilibrate() and before
setMDSolverInterface()
 ...
 index number i and Macroscopic solver.
 	 *	It should be called right after equilibrate() and before
 setMDSolverInterface()
      * 	@param i
 	 */
 ...
 file filestem
  ...
  void simulateTimesteps(const unsigned int &t, unsigned int &T,
                         coupling::services::MultiMDCellService<
                             LinkedCell, dim> &multiMDCellService) {
    for (unsigned int i = 0; i < _simpleMD.size(); ++i) {
      coupling::interface::MamicoInterfaceProvider<LinkedCell,
                                                   dim>::getInstance()
...
return the corresponding MD solver interface. It initializes first an instance
of the slot "slot". Then set the _simpleMD[localIndex] to this new instance.
	 * 	@param slot
	 * 	@param localIndex
  ...
  void rmMDSimulation(const unsigned int &index) {
    if (_simpleMD[index] != nullptr) {
      ...
      std::cout << "WARNING coupling::InstanceHandling::rmMDSimulation() : "
                   "_simpleMD at index " << index << " == null!" << std::endl;
    }
    ...
    if (_mdSolverInterface[index] != nullptr) {
      ...
      std::cout << "WARNING coupling::InstanceHandling::rmMDSimulation() : "
                   "_mdSolverInterface at index " << index << " == null!"
                << std::endl;
    }
...
macroscopic cell service for multi-MD case
	 * 	@param multiMDCellService
	 */
  void setMacroscopicCellServices(coupling::services::MultiMDCellService<
      LinkedCell, dim> &multiMDCellService) {
    for (unsigned int i = 0; i < _simpleMD.size(); ++i) {
