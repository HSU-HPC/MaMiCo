 ...
 * r_c=2.5.
 *  We only consider molecules in the outermost macroscopic cell. Thus, the
 * current method will only be
 ...
 *  A study on boundary force model used in multiscale simulations with
 * non-periodic boundary condition
 *  Microfluid Nanofluid 16(3): 587-595, 2014
 *
 *  The force is computed based on an interpolation formula which has been
 * obtained from various MD simulation
...
public:
  /** currently, we support constant temperature and density and 3D only. To be
extended
	 *	@param density
	 *	@param temperature
	 *	@param epsilon
  ...
  ZhouBoundaryForce(const double &density, const double &temperature,
      ...
      : _boundary(boundary), _domainLowerLeft(domainOffset),
        _domainUpperRight(domainSize + domainOffset),
        _mdSolverInterface(mdSolverInterface), _p1(getP1(density, temperature)),
        _p2(getP2(density, temperature)), _p3(getP3(density, temperature)),
        _q1(getQ1(density)), _q2(getQ2(density)), _q3(getQ3(density)),
        _forceFactor(epsilon / sigma), _sigma(sigma),
  ...
  getPotentialEnergy(const tarch::la::Vector<dim, double> &position) const {
    ...
    for (unsigned int d = 0; d < dim; d++) {
      if (_boundary[2 * d] && (position[d] - _domainLowerLeft[d] < distance)) {
        energy +=
            getPotentialEnergy((position[d] - _domainLowerLeft[d]) / _sigma);
      }
      if (_boundary[2 * d + 1] &&
          (_domainUpperRight[d] - position[d] < distance)) {
        energy +=
            getPotentialEnergy((_domainUpperRight[d] - position[d]) / _sigma);
      }
    }
    return energy;
  }

  /** checks the boundary flags for open boundaries. If an open boundary is
encountered and this molecule is close to that boundary (distance smaller than
2.5, cf. Zhou paper),
  ...
  getBoundaryForces(const tarch::la::Vector<dim, double> &position) const {
    ...
    for (unsigned int d = 0; d < dim; d++) {
      ...
      if (_boundary[2 * d] && (position[d] - _domainLowerLeft[d] < distance)) {
        force[d] +=
            _forceFactor * getScalarBoundaryForce(
                               (position[d] - _domainLowerLeft[d]) / _sigma);
      }
      if (_boundary[2 * d + 1] &&
          (_domainUpperRight[d] - position[d] < distance)) {
        force[d] -=
            _forceFactor * getScalarBoundaryForce(
                               (_domainUpperRight[d] - position[d]) / _sigma);
      }
    }
    return force;
  }
...
private:
  ...
  /** evaluates the force expression for a scalar component of the force vector.
We thus add up dimensional contributions if several boundaries are located
beside each other.
	 *	@param rw
  ...
  double getScalarBoundaryForce(double rw) const {
    if (rw < 1.04) {
      ...
      return -1.0 / (_q1 + _q2 * (2.5 - rw) * (2.5 - rw) +
                     _q3 / ((2.5 - rw) * (2.5 - rw)));
    }
  }
  ...
  double getP1(const double &density, const double &temperature) const {
    ...
    const double p1 =
        (-18.953 + 53.369 * temperature - 1.253 * T2 + 4.599 / 100000.0 * T3 +
         59.871 * logrho + 19.737 * logrho * logrho) /
        (1.0 + 2.592 * temperature - 0.557 * T2 + 0.049 * T3 - 13.912 * logrho +
         18.657 * logrho * logrho);
    return p1;
  }
  ...
  double getP2(const double &density, const double &temperature) const {
    ...
    const double p2 =
        (-0.094 + 2.808 * temperature - 0.019 * T2 - 0.001 * T3 +
         2.823 * logrho + 2.071 * logrho * logrho) /
        (1.0 + 0.168 * temperature - 0.013 * T2 - 4.323 * logrho +
         2.557 * logrho * logrho - 2.155 * logrho * logrho * logrho);
    return p2;
  }
  ...
  double getP3(const double &density, const double &temperature) const {
    ...
    const double p3 =
        3.934 + 0.099 * T394 - 0.097 * rho17437 + 0.075 * T394 * rho17437;
    return p3;
  }
  ...
  double getQ1(const double &density) const {
    ...
    return -30.471 + 113.879 * density - 207.205 * rho2 + 184.242 * rho3 -
           62.879 * rho4;
  }
  ...
  double getQ2(const double &density) const {
    ...
    const double rho3 = rho2 * density;
    const double rho4 = rho2 * rho2;
    return 6.938 - 25.788 * density + 46.773 * rho2 - 41.768 * rho3 +
           14.394 * rho4;
  }
  ...
  double getQ3(const double &density) const {
    ...
    const double rho3 = rho2 * density;
    const double rho4 = rho2 * rho2;
    return 39.634 - 147.821 * density + 269.519 * rho2 - 239.066 * rho3 +
           81.439 * rho4;
  }

  /** an entry is true, if this is an open boundary. For enumeration of
   * boundaries, see BoundaryForceConfiguration */
