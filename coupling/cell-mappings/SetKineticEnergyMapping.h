  ...
  public:
    ...
    void handleCell(LinkedCell& cell,const unsigned int &cellIndex){
      ...
      while(it->continueIteration()){
        ...
        // set new velocity: still with same mean, but re-scale the deviation for correct thermal energy
  ...
  private:
    double getCorrectionFactor(
      ...
      correctionFactor = correctionFactor /(
                           oldKineticEnergy - 0.5*mass*numberParticles*tarch::la::dot(meanVelocity,meanVelocity) );
