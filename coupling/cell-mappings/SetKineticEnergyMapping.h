...
class coupling::cellmappings::SetKineticEnergyMapping {
public:
  /** obtains the old momentum over the region of interest. Besides,
     ...
     */
  SetKineticEnergyMapping(const double &oldKineticEnergy,
      ...
      : _mdSolverInterface(mdSolverInterface), _meanVelocity(meanVelocity),
        _correctionFactor(getCorrectionFactor(oldKineticEnergy,
                                              newKineticEnergy, numberParticles,
                                              meanVelocity)) {}
...
infinity; I just hard-coded 1e-7 for this case
  ...
  double getCorrectionFactor(
    ...
    if (oldKineticEnergy - 0.5 * mass * numberParticles *
                               tarch::la::dot(meanVelocity, meanVelocity) <
        1e-7) {
      return 1.0;
    }

    double correctionFactor =
        newKineticEnergy - 0.5 * mass * numberParticles *
                               tarch::la::dot(meanVelocity, meanVelocity);
    correctionFactor =
        correctionFactor /
        (oldKineticEnergy - 0.5 * mass * numberParticles *
                                tarch::la::dot(meanVelocity, meanVelocity));
