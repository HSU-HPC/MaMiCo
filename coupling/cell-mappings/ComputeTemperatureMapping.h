...
statistical mechanics.
  ...
  void endCellIteration() {
    ...
    if (_particleCounter != 0) {
      _temperature =
          _temperature / (dim * _mdSolverInterface->getKB() * _particleCounter);
    }
  }
 ...
 particles
  ...
  void handleCell(LinkedCell &cell, const unsigned int &cellIndex) {
    ...
    while (it->continueIteration()) {
      ...
      _temperature += tarch::la::dot((wrapper.getVelocity() - _meanVelocity),
                                     (wrapper.getVelocity() - _meanVelocity));
      _particleCounter++;
