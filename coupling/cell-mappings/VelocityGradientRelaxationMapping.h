        ...
        for (unsigned int d = 0; d < dim; d++) {
          ...
          if (normalisedPosition[d] < 0.0) {
            std::cout
                ...
                << _localVectorCellIndex << " ," << normalisedPosition
                << std::endl;
            for (unsigned int i = 0; i < 10; i++) {
              std::cout << macroscopicCellIndex[i] << " ";
            }
  ...
  void interpolateVelocitySecondOrder(
    ...
    if (dim == 2) {
      ...
      if (secondCake) {
        ...
        coefficients[0] =
            2.0 * (velocity[0] + velocity[2] + velocity[9]) +
            4.0 * (velocity[5] - velocity[1] - velocity[6] - velocity[3]) +
            5.0 * velocity[4] - v78;
        coefficients[1] =
            0.5 * (v78 - velocity[0]) +
...
public:
  ...
  VelocityGradientRelaxationTopOnlyMapping(
      ...
      const coupling::datastructures::MacroscopicCellWithLinkedCells<
          LinkedCell, dim> *const macroscopicCells)
      : coupling::cellmappings::VelocityGradientRelaxationMapping<LinkedCell,
                                                                  dim>(
            velocityRelaxationFactor, currentVelocity,
            currentLocalMacroscopicCellIndex, mdSolverInterface,
            indexConversion, macroscopicCells) {
    ...
    coupling::cellmappings::VelocityGradientRelaxationMapping<
        ...
        (globalIndex[dim - 1] <
         coupling::cellmappings::VelocityGradientRelaxationMapping<
             LinkedCell, dim>::_globalNumberCellsWithGhostLayer[dim - 1] -
             3);
  }
