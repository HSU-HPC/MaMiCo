...
class coupling::cellmappings::VelocityGradientRelaxationMapping {
  public:
    ...
    VelocityGradientRelaxationMapping(
      ...
      const coupling::datastructures::MacroscopicCellWithLinkedCells<LinkedCell,dim> * const macroscopicCells
       ...
       _velocityRelaxationFactor(velocityRelaxationFactor),_currentVelocity(currentVelocity),
       _innerLowerLeft(getInnerLowerLeftCorner()),_innerUpperRight(getInnerUpperRightCorner()),
       _outerLowerLeft(getOuterLowerLeftCorner()),_outerUpperRight(getOuterUpperRightCorner()),
	...
	void handleCell(LinkedCell& cell,const unsigned int &cellIndex){
      ...
      //std::cout << "Handle VelocityGradientRelaxation in cell " << _localVectorCellIndex << "=" <<  _currentLocalMacroscopicCellIndex << std::endl;
      ...
      while(it->continueIteration()){
        ...
        std::cout << "VelocityGradientRelaxationMapping: Process cell " << _globalVectorCellIndex << ", molecule " << position << std::endl;
        #endif
        if (relaxMolecule(position)){
          tarch::la::Vector<dim,double> normalisedPosition(0.0);
          bool secondCake = false;
          // index of macroscopic cell values that are involved in interpolation process
          ...
          for (unsigned int d = 0; d < dim; d++){
            if (normalisedPosition[d]>2.0){
              std::cout << "ERROR cellmappings::VelocityGradientRelaxationMapping: normalisedPosition>2.0! " << "Local vector index=" <<_localVectorCellIndex << " , global vector index=" << _globalVectorCellIndex << ", currentLocalCellIndex=" << _currentLocalMacroscopicCellIndex << ", Norm. position=" << normalisedPosition << ", Actual position=" << position << std::endl;
              for (unsigned int i = 0; i < 10; i++){ std::cout << macroscopicCellIndex[i] << " ";} std::cout << std::endl;
              exit(EXIT_FAILURE);
            }
            if (normalisedPosition[d]<0.0){
              std::cout << "ERROR cellmappings::VelocityGradientRelaxationMapping: normalisedPosition<0.0! " << _localVectorCellIndex << " ," << normalisedPosition << std::endl;
              for (unsigned int i = 0; i < 10; i++){ std::cout << macroscopicCellIndex[i] << " ";} std::cout << std::endl;
              exit(EXIT_FAILURE);
            }
          }
  ...
  protected:
    ...
    // _globalNumberCellsWithGhostLayer,_domainOffset and _cellsize are already available in _indexConversion.
  ...
  private:
	...
	void createMacroscopicCellIndex4SecondOrderInterpolation(
      ...
      // the normalised position is chosen such that the origin (0,0...,0) coincides with the lower left... macroscopic cell's midpoint
      ...
      for (unsigned int d = 0; d < dim; d++){
        ...
        if ( (position[d]<cellMidpoint[d]-0.5*_cellSize[d]) || (position[d] > cellMidpoint[d]+0.5*_cellSize[d])){
          std::cout << "ERROR VelocityGradientRelaxationMapping::createMacroscopicCellIndex4SecondOrderInterpolation: Input position " << position;
          std::cout << " out of range of cell " << globalCellIndex << "!" << std::endl; exit(EXIT_FAILURE);
        }
        #endif

        // put lowerLeftCellIndex such that it's right below the position
        if ( position[d] < cellMidpoint[d] ){ lowerLeftCellIndex[d]--; globalCellIndex[d]--; }
        // if this is on the left/ lower boundary, but still to far up, reduce height
        if      (lowerLeftCellIndex[d]==1                                   ){ lowerLeftCellIndex[d]--; globalCellIndex[d]--; }
        else if (lowerLeftCellIndex[d]==_localNumberCellsWithGhostLayer[d]-2){ lowerLeftCellIndex[d]--; globalCellIndex[d]--; }

        // determine normalised position
        normalisedPosition[d] = (position[d] - (_domainOffset[d]-0.5*_cellSize[d]) - globalCellIndex[d]*_cellSize[d]) / _cellSize[d];
      }
      lowerLeftIndex = _indexConversion.getLocalCellIndex(lowerLeftCellIndex);


      if (dim==2){
        std::cout << "Not implemented correctly yet!" << std::endl; exit(EXIT_FAILURE);
      } else if (dim==3){
        const unsigned int localNumberCells01 = _localNumberCellsWithGhostLayer[0]*_localNumberCellsWithGhostLayer[1];
        const unsigned int localNumber2Cells0 = 2*_localNumberCellsWithGhostLayer[0];
        const unsigned int localNumber2Cells01Plus2Cells0 = 2*localNumberCells01+2*_localNumberCellsWithGhostLayer[0];

        tarch::la::Vector<dim,double> normal; normal[0] = 1.0; normal[1] = 1.0; normal[2] = 0.0;
        tarch::la::Vector<dim,double> relPos(normalisedPosition); relPos[0] -= 2.0;
        ...
        std::cout << "ERROR createMacroscopicCellIndex4Interpolation(position): only 2D/3D supported!" << std::endl; exit(EXIT_FAILURE);
      }
    ...
    void interpolateVelocitySecondOrder(
      ...
      if (dim==2){
        ...
        if (secondCake){
          ...
          coefficients[0] = 2.0*(velocity[0] + velocity[2] + velocity[9]) + 4.0*(velocity[5] - velocity[1] - velocity[6] - velocity[3]) + 5.0*velocity[4]  - v78;
          coefficients[1] = 0.5*(v78 - velocity[0]) + 2.0*(velocity[1] - velocity[2] - velocity[5] + velocity[6]) + 4.0*velocity[3] - 3.5*velocity[4] - velocity[9];
          coefficients[2] = 2.0*(velocity[3] - velocity[0] - velocity[5] + velocity[6]) + 4.0*velocity[1] - 3.5*velocity[4] + 0.5*(v78 - velocity[2]) - velocity[9];
          ...
          coefficients[7] = 0.25*(velocity[0] + velocity[2] - v78) - velocity[1] - velocity[3] + 1.5*velocity[4] + velocity[5] - velocity[6] + 0.5*velocity[9];
          ...
          coefficients[7] =  1.5*velocity[0]  +     velocity[6] -      velocity[1] -      velocity[3] -      velocity[5]
                          +  0.5*velocity[7]  + 0.25*(velocity[2]    + velocity[4] -      velocity[8] - velocity[9]);
          coefficients[8] = 0.25*(velocity[0] - velocity[2] - velocity[7] + velocity[8]);
          coefficients[9] = 0.25*(velocity[0] - velocity[4] - velocity[7] + velocity[9]);
        }

        // evaluate polynomial
        const tarch::la::Vector<dim,double> contribution0 = normalisedPosition[0]*(
                  coefficients[1] + coefficients[4]*normalisedPosition[0]
                + coefficients[7]*normalisedPosition[1] + coefficients[8]*normalisedPosition[2] );
        const tarch::la::Vector<dim,double> contribution1 = normalisedPosition[1]*(
              coefficients[2] + coefficients[5]*normalisedPosition[1]
            + coefficients[9]*normalisedPosition[2] );
        const tarch::la::Vector<dim,double> contribution2 = normalisedPosition[2]*(
...
class coupling::cellmappings::VelocityGradientRelaxationTopOnlyMapping: public coupling::cellmappings::VelocityGradientRelaxationMapping<LinkedCell,dim>{
  public:
    ...
    VelocityGradientRelaxationTopOnlyMapping(
      ...
      // the following snippet is basically a replacement of the method ignoreThisCell(). Since this function is called in the constructor of the based class,
      // we cannot overwrite it; hence, we solve it by overwriting the respective variable from within this constructor (called after base object is constructed)
      const tarch::la::Vector<dim,unsigned int> globalIndex =
        coupling::cellmappings::VelocityGradientRelaxationMapping<LinkedCell,dim>::_indexConversion.convertLocalToGlobalVectorCellIndex(
          coupling::cellmappings::VelocityGradientRelaxationMapping<LinkedCell,dim>::_indexConversion.getLocalVectorCellIndex(currentLocalMacroscopicCellIndex));
