...
public:
   ...
   *  @param insertDeleteMassAtTimestep the interval of time steps for the
   * insertion/deletion of mass
   *  @param rSigmaCoeff coefficient to adapt r_sigma (see Usher algorithm for
   * details,), r_sigma = sigma * rSigmaCoeff
   *  @param meanPotentialEnergyFactor scales for the U_0 value of the Usher
   * algorithm
   *  @param uOverlapCoeff scales the overlap energy U_ovlp (refers to the Usher
   * algorithm)
   ...
   *  @param tolerance sets the tolerance, between the reached energy level and
   * the goal energy level
   *  @param offsetFromOuterBoundary offset of the md domain (difference between
   * the left, lower, front corner to 0,0,0)
  ...
  /** the function checks for the value of the microscopic mass and calls upon
...
protected:
  ...
  public:
    ...
    /** sets the tolerance, between the reached energy level and the goal energy
     * level */
    double _tolerance;
    /** offset of the md domain (difference between the left, lower, front
     * corner to 0,0,0) */
    ...
    /** @brief returns the maximal displacement parameter for the usher method,
     * ref. figure 7 in the paper
