...
/** ... from vector to linearised indices as well as from local to global indices.
...
class coupling::IndexConversion: private tarch::utils::Uncopyable {
  public:
     ...
     *  @param globalNumberMacroscopicCells total number of macroscopic cells of the md simulation
     ...
     *  @param parallelTopologyType the type of parallel topology see ParallelTopologyFactory.h
     *  @param topologyOffset the total number of processes for a md simulation, e.g. for two
     *                        md simulations on eight ranks it would be 4; */
    IndexConversion(
     ...
     *  @param globalMDDomainOffset offset of the md domain compared to the coordinate origin
     *  @param parallelTopologyType the type of parallel topology see ParallelTopologyFactory.h */
    IndexConversion(
      tarch::la::Vector<dim,unsigned int> globalNumberMacroscopicCells,
      tarch::la::Vector<dim,unsigned int> numberProcesses,
    ...
    // ----------------------------- INDEX CONVERSIONS ------------------------------------
    /** We assume a lexicographic ordering of the macroscopic cells for this purpose.
     *  Besides, we assume the number of global macroscopic cells to be extended by a global ghost layer
     *  (so that we have globalNumberMacroscopicCells_d+2 cells in each direction).
     *  @brief returns the linearised global cell index from a global cell index vector.
     *  @param globalCellIndex the global cell index as a vector (according to the dimension of the simulation)
    ...
    /** We assume a lexicographic ordering of the macroscopic cells for this purpose.
     *  Besides, we assume the number of local macroscopic cells to be extended by a global ghost layer
     *  (so that we have globalNumberMacroscopicCells_d+2 cells in each direction).
     *  @brief returns the linearised global cell index from a local cell index vector.
     *  @param localCellIndex the local (only valid on this rank) cell index as a vector
    ...
    /** This function just performs the inverse operation to getGlobalCellIndex(vectorIndex),
     *  i.e. getGlobalVectorCellIndex(getGlobalCellIndex(somevector) ) should again return somevector.
    ...
    /** This function just performs the inverse operation to getLocalCellIndex(vectorIndex),
     *  i.e. getLocalVectorCellIndex(getLocalCellIndex(somevector) ) should again return somevector.
     *  @brief returns the local vector cell index from a linearised index.
     *  @param localCellIndex local (only valid on this rank) continuous index of a cell
     *  @returns the dimensional global index for the given linear index */
    ...
    // ------------------------------ GETTERS ----------------------------------------------
    /** "Average" in this case means the number of cells on all processes except for the
     *  processes on the final coordinate range; we may have more cells there, due to the
     *  domain decomposition. The number of ghost cells is not included in this vector.
    ...
    /** @brief returns the global number of macroscopic cells, exluding ghost cells.
     *  @returns the total number of macroscopic cells over all ranks (dimensional) */
    ...
    /** @brief returns the rank which corresponds to the linearised index of the vector returned by
     *  getThisProcess().
     *  @returns the linearised index of this process's coordinates */
    unsigned int getThisRank() const;
    /** @brief returns the vector with the number of processes used in all directions.
    ...
    /** @brief returns the offset, i.e. the lower,left... corner coordinate, of the MD domain.
    ...
    /** @brief returns the global vector cell index of the macroscopic cell in which a point is
     *  @param position vector of the point to get the corresponding macroscopic cell
    ...
    /** Forwards the call to the ParallelTopology implementation.
     *  @brief returns the linearised index (i.e. rank) of the given process coordinates.
    ...
    /** If the cell is contained in the MD volume, the rank is uniquely chosen by using the process which contains
     *  the macroscopic cell as non-ghost (i.e. real inner) cell. If the cell is a global ghost cell, we choose
     *  the rank according to the block-decomposition of the grid applied to the complete grid incl. the ghost layer.
     ...
     *  @param globalCellIndex the global vector coordinates of a macroscopic cell
     *  @returns a bool, indicating if the given global index is valid (true) or not (false) */
     ...
     *  @param localCellIndex the local (only valid on this rank) vector coordinates of a macroscopic cell
     *  @returns a bool, indicating if the given local index is valid (true) or not (false)  */
  ...
  private:
     ...
     *  @brief converts a vector cell index into a linearised cell index using a predefined number of cells.
     ...
     *  @brief returns the average number of local macroscopic cells on each process.
     *  @param initAverageLocalNumberMacroscopicCells average number of macroscopic cells per process
     ...
     *  @param initLocalNumberMacroscopicCells the total number of macroscopic cells on this process
