...
namespace coupling { template <unsigned int dim> class IndexConversion; }

/** handles the index convertion from vector to linearised indices as well as
 * from local to global indices.
 *  This class inherits privately from Uncopyable (see CouplingMDDefinitions).
 * We do this in order to hold the
 *  pointer to the ParallelTopology consistent in exactly one instance of
 * IndexConversion (or require to have
...
public:
  ...
  /** returns the linearised global cell index from a global cell index vector.
   * We assume a lexicographic ordering
   *  of the macroscopic cells for this purpose. Besides, we assume the number
   * of global macroscopic cells to be
  ...
  /** returns the linearised global cell index from a local cell index vector.
   * We assume a lexicographic ordering
   *  of the macroscopic cells for this purpose. Besides, we assume the number
   * of local macroscopic cells to be
   *  extended by a global ghost layer (so that we have
   * globalNumberMacroscopicCells_d+2 cells in each direction).
   */
  ...
  /** returns the global vector cell index from a linearised index. This
   * function just performs the inverse operation to
  ...
  /** Does the same as getGlobalVectorCellIndex() but
     ...
     */
  tarch::la::Vector<dim, unsigned int>
      getGlobalInnerVectorCellIndex(unsigned int globalCellIndex) const;

  //TODO: unused function
  /** Translates index vector of inner cell to make it usable in global
   * context*/
  tarch::la::Vector<dim, unsigned int> convertInnerVectorCellIndexToGlobal(
      tarch::la::Vector<dim, unsigned int> cellIndexVectorMDContext) const;

  /** returns the local vector cell index from a linearised index. This function
   * just performs the inverse operation to
  ...
  /** returns the "average" number of local macroscopic cells. "Average" in this
   * case means
   *  the number of cells on all processes except for the processes on the final
   * coordinate range;
   *  we may have more cells there, due to the domain decomposition. The number
   * of ghost cells
   *  is not included in this vector.
   */
  tarch::la::Vector<dim, unsigned int>
      getAverageLocalNumberMacroscopicCells() const;
  /** returns the local number of macroscopic cells on this process. The ghost
   * layer is not
  ...
  /** returns the rank which corresponds to the linearised index of the vector
   * returned by
   *  getThisProcess().
   */
  ...
  /** returns the offset, i.e. the lower,left... corner coordinate, of the MD
   * domain.*/
  ...
  /** returns the global vector cell index of the macroscopic cell which
   * contains the vector "position". */
  tarch::la::Vector<dim, unsigned int>
  ...
  /** returns the process coordinates for the rank 'rank'. Forwards the call to
   * the ParallelTopology implementation. */
  tarch::la::Vector<dim, unsigned int>
      getProcessCoordinates(unsigned int rank) const;
  /** returns the linearised index (i.e. rank) of the given process coordinates.
  ...
  /** returns the unique rank for a macroscopic cell. If the cell is contained
   * in the MD volume, the rank is uniquely
   *  chosen by using the process which contains the macroscopic cell as
   * non-ghost (i.e. real inner) cell. If the cell
   *  is a global ghost cell, we choose the rank according to the
   * block-decomposition of the grid applied to the
   ...
   * macroscopic cell with this global
  ...
  /** checks if the global vector cell index is in a well-defined range. Should
   * only be used for debugging
   *  and validation purposes.
   */
  bool isValidGlobalVectorCellIndex(
      tarch::la::Vector<dim, unsigned int> globalCellIndex) const;
  /** checks if the local vector cell index is in a well-defined range. Should
   * only be used for debugging
   *  and validation purposes.
   */
  bool isValidLocalVectorCellIndex(
      tarch::la::Vector<dim, unsigned int> localCellIndex) const;
  /** checks if the global cell index is in a well-defined range. Should only be
   * used for debugging
   *  and validation purposes.
   */
  bool isValidGlobalCellIndex(unsigned int globalIndex) const;
  /** checks if the local cell index is in a well-defined range. Should only be
   * used for debugging
   *  and validation purposes.
   */
...
private:
  /** converts a vector cell index into a linearised cell index using a
  ...
  unsigned int getCellIndex(
      tarch::la::Vector<dim, unsigned int> vectorCellIndex,
  ...
  /** checks if the index is in a valid range. We add the ghost layer influence
  ...
  /** initialises _macroscopicCellSize using _globalMDDomainSize and
   * _globalNumberMacroscopicCells. */
  tarch::la::Vector<dim, double> initMacroscopicCellSize(
      ...
      const tarch::la::Vector<dim, unsigned int> &
          globalNumberMacroscopicCells) const;
   ...
   * is extended by 2 in order to account
