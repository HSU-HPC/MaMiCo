...
template <unsigned int dim> class BoxCellContainer;
} // namespace datastructures
} // namespace coupling
 ...
 */
template <unsigned int dim> class coupling::datastructures::BoxCellContainer {

public:
  BoxCellContainer(I01 lowerBound, I01 upperBound) : BoxCellContainer({}, lowerBound, upperBound) {}
  
  BoxCellContainer(I01 lowerBound, tarch::la::Vector<dim, unsigned int> shape) : BoxCellContainer({}, lowerBound, shape) {}

  BoxCellContainer(std::vector<coupling::datastructures::CouplingCell<dim>*> couplingCells, I01 lowerBound, I01 upperBound)
    : BoxCellContainer(couplingCells, lowerBound, upperBound.get() - lowerBound.get()) {}

  BoxCellContainer(std::vector<coupling::datastructures::CouplingCell<dim>*> couplingCells, I01 lowerBound, tarch::la::Vector<dim, unsigned int> shape) {
   ...
   */
  coupling::datastructures::CouplingCell<dim>* operator[](tarch::la::Vector<dim, unsigned int> index) const {
#if (COUPLING_MD_ERROR == COUPLING_MD_YES)
    // TODO implement bounds checking!
    if (_couplingCells.size() < linearNumberCellsInDomain()) {
      std::cout << "BoxCellContainer<" << dim << "> accessed but not full " << std::endl;
...
#if (COUPLING_MD_ERROR == COUPLING_MD_YES)
    if (_couplingCells.size() >= linearNumberCellsInDomain()) {
      std::cout << "BoxCellContainer<" << "," << dim << "> can only hold " << linearNumberCellsInDomain() << " coupling cells!"
                << std::endl;
      std::exit(EXIT_FAILURE);
    }
#endif
  ...
  tarch::la::Vector<dim, unsigned int> numberCellsInDomain() const {
    return _shape;
  }
...
//     */
//    const std::pair<coupling::datastructures::CouplingCell<dim>*, I01> operator*() const { return std::make_pair(*_itCouplingCells, *_itIdx); }
//
//    Iterator& operator++() {
//      ++_itCouplingCells;
//      ++_itIdx;
...
protected:
   ...
   */
  tarch::la::Vector<dim, unsigned int> _shape;
  /**
