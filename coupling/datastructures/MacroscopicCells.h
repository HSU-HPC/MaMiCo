...
public:
  ...
  /** returns vector-of-pointers to macroscopic cells without access to linked cells.
  ...
  /** apply the function apply(MacroscopicCell&,const unsigned int&) of a generic class A to all local non-ghost macroscopic cells.
   *  This functionality is used from the MacroscopicCellService to apply various functionalities (such as momentum transfer) to the
   *  macroscopic cells which cover the MD domain.
   */
   ...
   *  We can have the same traversal by applying both applyToLocalGhostMacroscopicCellsWithLinkedCells and applyToLocalNonGhostMacroscopicCellsWithLinkedCells.
   *  However, this implementation is more efficient.
   */
  template <class A> void applyToAllLocalMacroscopicCellsWithLinkedCells(A& a);
  /** apply the function apply(MacroscopicCell&,const unsigned int&) of a generic class A to the first layer of GLOBAL non-ghost cells.
  ...
  template <class A> void applyXLayersOfGlobalNonGhostCellsWithLinkedCellsAtBottom(A& a, unsigned int layers2Use);

  template <class A> void applyXLayersOfGlobalNonGhostCellsWithLinkedCellsAtTop(A& a, unsigned int layers2Use);
...
private:
  ...
  /** holds pointers to all macroscopic cells with linked cells, but without access to linked cells.
