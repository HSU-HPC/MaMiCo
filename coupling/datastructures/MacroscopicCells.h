...
public:
   ...
   * generic class A to all local non-ghost macroscopic cells.
   *  This functionality is used from the MacroscopicCellService to apply
   * various functionalities (such as momentum transfer) to the
...
generic class A to all local ghost macroscopic cells.
   ...
   *  We can have the same traversal by applying both
   * applyToLocalGhostMacroscopicCellsWithLinkedCells and
   * applyToLocalNonGhostMacroscopicCellsWithLinkedCells.
   *  However, this implementation is more efficient.
   */
  template <class A> void applyToAllLocalMacroscopicCellsWithLinkedCells(A &a);
  /** apply the function apply(MacroscopicCell&,const unsigned int&) of a
generic class A to the first layer of GLOBAL non-ghost cells.
     *  If a cell is a local ghost cell (due to parallelization, not due to
being located outside MD domain!), it will not be handled by this method.
     *  If a cell is a local non-ghost cell and is not located at the boundary
