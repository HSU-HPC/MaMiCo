...
namespace coupling {
  namespace solvers{
    /** interface for Couette solvers. advance(dt) advances the solver in time. getVelocity(pos) returns the velocity at position pos. */
...
class coupling::solvers::CouetteSolver: public coupling::solvers::AbstractCouetteSolver<dim> {
  public:
    ...
    /** returns the velocity vector at a certain channel height pos[dim-1], given by:
