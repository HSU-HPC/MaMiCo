 ...
 * set to 1: this yields that only the first non-ghost layer of coupling cells shall be sent to MD, and all other inner coupling cells are received from
...
public:
  ...
  virtual bool receiveMacroscopicQuantityFromMDSolver(tarch::la::Vector<dim, unsigned int> globalCellIndex) {
    ...
    // (globalCellIndex[d]<_globalNumberCouplingCells[d]+1-_outerRegion); }
    for (unsigned int d = 0; d < dim; d++) {
      recv = recv && (globalCellIndex[d] > _outerRegion - 1) && (globalCellIndex[d] < _globalNumberCouplingCells[d] + 2 - _outerRegion);
    }
    // for (unsigned int d = 0; d < dim; d++){ recv = recv && (globalCellIndex[d]>0) && (globalCellIndex[d]<_globalNumberCouplingCells[d]+1); }
    return recv;
  }

  /** send all coupling cell data within a boundary strip to MD. Only send data that are not in the ghost layer and not part of the inner region. */
...
private:
  ...
  const tarch::la::Vector<dim, unsigned int> _globalNumberCouplingCells; // global number of coupling cells
};
