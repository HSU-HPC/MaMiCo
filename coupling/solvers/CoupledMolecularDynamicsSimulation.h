...
/** simulation class for coupled MD simulations; thus, the implementation of one timestep slightly
 *  differs from the one of the base class.
 *  @author Philipp Neumann
 *  @todo since simpleMD is not part of the docu, somthing is missing here?!? How to deal with?
 */
...
public simplemd::MolecularDynamicsSimulation {
  public:
    ...
    /** @brief turns off the coupling parts within md, md runs according to the basic md method */
    void switchOffCoupling(){
      _couplingSwitchedOn = false;
    }
     ...
     *  in boundary regions and on different processes after mass insertion/ deletion.
  ...
  private:
    ...
    /** @brief bool holding the current state of the coupling: true - coupled simulation and false - independent md simulation */
