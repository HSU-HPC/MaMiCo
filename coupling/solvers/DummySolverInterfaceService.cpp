      ...
      tarch::la::Vector<3,double> macroscopicCellSize
        	...
        	for (unsigned int d = 0; d < 3; d++){
				...
				if (fabs(_globalNumberMacroscopicCells[d]*macroscopicCellSize[d]-globalMDDomainSize[d])>1e-13){
                    std::cout << "ERROR MacroscopicSolverInterfaceService::init(): globalNumberMacroscopicCells does not fit" << std::endl;
                    exit(EXIT_FAILURE);
                }
        	}
        	...
        	if (_dummySolverInterface==NULL){
                std::cout << "ERROR MacroscopicSolverInterfaceService::init(): Could not initialise _transferStrategy!" << std::endl;
                exit(EXIT_FAILURE);
        	}
...
bool DummySolverInterfaceService::addToSendBuffer(const double &density, const tarch::la::Vector<3,double> &velocity,const 			tarch::la::Vector<3,unsigned int> &index) {
        ...
        if (_dummySolverInterface->sendMacroscopicQuantityToMDSolver(globalVectorIndex)){
            // if we have to send this cell, scale mass and momentum and write them into cell buffer
...
bool DummySolverInterfaceService::getFromReceiveBuffer(double &density, tarch::la::Vector<3,double> &velocity,const tarch::la::Vector<3,unsigned int> &index) const {
        if (!isInsideTransferRegion(index)){
			return false;
		}
        ...
        if (_dummySolverInterface->receiveMacroscopicQuantityFromMDSolver(globalVectorIndex)){
        	...
        	std::cout << "ERROR MacroscopicSolverInterfaceService::getFromReceiveBuffer(): Cell information for global cell index " << globalVectorIndex << " not found in buffer!" << std::endl;
        	exit(EXIT_FAILURE);
		}
        return false;
}
...
void DummySolverInterfaceService::allocateBuffers(){
	...
	// loop over global MD domain and find out if we need to receive/send a cell
  	// store the cell indices for sending and receiving temporarily; we copy those into the arrays afterwards, but first need to
  	// determine the sizes of the arrays. For the send-buffer, this would typically not be necessary, but we do this only once at
	...
	for(unsigned int d=0; d<3; d++){
  		...
  		for (loop[1]=0; loop[1] < _transferDomainNumCells[1]; loop[1]++){
  			for (loop[0]=0; loop[0] < _transferDomainNumCells[0]; loop[0]++){
        		...
        		for (unsigned int i = 0; i < sizeRanks; i++){
           			...
           			if (ranks[i] == thisRank){cellOnThisProcess = true; break;}
        		}

        		// if the cell is on this process, find out if we need to receive/ send information and increment buffers
    			if (cellOnThisProcess){
      				if (_dummySolverInterface->receiveMacroscopicQuantityFromMDSolver(loop)){
        				...
        				if (_receiveBuffer[_receiveBufferSize]==NULL){
							std::cout << "ERROR DummySolverInterfaceService::allocateBuffers(): _receiveBuffer[i]==NULL!" << std::endl;
							exit(EXIT_FAILURE);
						}
        				_receiveBufferSize++;
      				}
      				...
      				if (_dummySolverInterface->sendMacroscopicQuantityToMDSolver(loop)){
        				...
        				if (_sendBuffer[_sendBufferSize]==NULL){
							std::cout << "ERROR DummySolverInterfaceService::allocateBuffers(): _sendBuffer[i]==NULL!" << std::endl;
							exit(EXIT_FAILURE);
						}
        				_sendBufferSize++;
      				}
  	...
  	if (_receiveBufferSize != 0){
    	...
    	if (_globalIndices4ReceiveBuffer==NULL){std::cout << "ERROR DummySolverInterfaceService::allocateBuffers(): _globalIndices4ReceiveBuffer==NULL" << std::endl; exit(EXIT_FAILURE);}
    	for (unsigned int i = 0; i < _receiveBufferSize; i++){ 
  	...
  	if (_sendBufferSize != 0){
   		...
   		if (_globalIndices4SendBuffer==NULL){std::cout << "ERROR DummySolverInterfaceService::allocateBuffers(): _globalIndices4SendBuffer==NULL" << std::endl; exit(EXIT_FAILURE);}
