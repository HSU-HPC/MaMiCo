...
namespace coupling {
namespace solvers { class FiniteDifferenceSolver; }
}

class coupling::solvers::FiniteDifferenceSolver
    : public coupling::solvers::NumericalSolver {
public:
  FiniteDifferenceSolver(const double channelheight,
      ...
      : coupling::solvers::NumericalSolver(channelheight, dx, dt, kinVisc,
                                           ...
                                           processes),
        _omega(dt * kinVisc / (dx * dx)), _wallVelocity(wallVelocity) {
    if (_processes[0] > 1 || _processes[1] > 1 || _processes[2] > 1) {
      ...
      std::cout << "The FiniteDifferenceSolver was requested in a parallel "
                   ...
                   "support parallel runs. " << std::endl;
    }
    ...
    _velold = new double[3 * (_domainSizeX + 2) * (_domainSizeY + 2) *
                         (_domainSizeZ + 2)];
  ...
  getVelocity(tarch::la::Vector<3, double> pos) const
      override { // same as with the function above
    ...
    if ((pos[0] < domainOffset[0]) ||
      ...
      std::cout << "ERROR FiniteDifferenceSolver::getVelocity(): Position "
                << pos << " out of range!" << std::endl;
      exit(EXIT_FAILURE);
    }
#endif
  ...
  double getDensity(tarch::la::Vector<3, double> pos) const override {
    ...
    if ((pos[0] < domainOffset[0]) ||
        ...
        (pos[2] < domainOffset[2]) ||
        (pos[2] > domainOffset[2] + _domainSizeZ * _dx)) {
      std::cout << "ERROR FiniteDifferenceSolver::getDensity(): Position "
                << pos << " out of range!" << std::endl;
      exit(EXIT_FAILURE);
    }
#endif
...
#pragma omp parallel for
    for (unsigned int i = 0; i < size; i++) {
      ...
      std::cout << indexConversion.getGlobalVectorCellIndex(recvIndices[i])
      ...
      if (_flag[index] != MD_BOUNDARY) {
        std::cout
            << "ERROR FiniteDifferenceSolver::setMDBoundaryValues(): Cell "
            << index << " is no MD boundary cell!" << std::endl;
        exit(EXIT_FAILURE);
      }
#endif
      // set velocity value in MD boundary cell (before streaming); the boundary
      // velocities are interpolated between the neighbouring and this cell.
      // This interpolation is valid for
      // FLUID-MD_BOUNDARY neighbouring relations only.
      // determine local velocity received from MaMiCo and convert it to LB
      // units; store the velocity in _vel
      tarch::la::Vector<3, double> localVel(
          (1.0 / recvBuffer[i]->getMacroscopicMass()) *
...
#pragma omp parallel for
    for (int i = _yO;
         i < (_domainSizeX + 2) * (_domainSizeY + 2) * (_domainSizeZ + 2) - _yO;
         i++) {
      if (_flag[i] == FLUID) {
        _vel[3 * i] = _omega * (_velold[3 * (i - _yO)] - 2 * _velold[3 * i] +
                                _velold[3 * (i + _yO)]) + _velold[3 * i];
      }
    }
  }
  ...
  tarch::la::Vector<3, double>
      _wallVelocity;   // velocity of moving wall of Couette flow
