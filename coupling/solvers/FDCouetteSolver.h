 ...
 *  The grid is just a simple cubic, equidistant mesh.
 *  @brief implements a simple one-dimensional finite-diffference solver for the Couette flow.
...
public:
	...
	FiniteDifferenceSolver(
		...
		_wallVelocity(wallVelocity)
		{
			if(_processes[0]>1 || _processes[1]>1 || _processes[2]>1){
				...
				_processes[1] = 1;
				_processes[2] = 1;
				std::cout << "The FiniteDifferenceSolver was requested in a parallel manner. It will be run sequentially, since it doesn't support parallel runs. " << std::endl;
			}
			// return if required
      if (skipRank()){return;}
	...
	~FiniteDifferenceSolver(){
		if (_velold){delete [] _velold; _velold=nullptr;}
	}
...
private:
	/** there is no grid point on the wall, since the wall is located directly between
	 *  the cells on the boundary. Therefore so set the boundary condition, the velocity
	 *  boundary condition is applied by setting the velocity in the outer cell
