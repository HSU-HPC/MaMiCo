...
/** dummy solver, implementing a flow solver with parabolic velocity profile and
 * constant density everywhere.
 *  @author Rahul Arora
 */
class DummySolver {
public:
  DummySolver(int nx, int ny, int nz, double vmax)
      : _Nx(nx), _Ny(ny), _Nz(nz), _Vmax(vmax) {
    _domain = new double[_Nx * _Ny * _Nz * 4];
    for (unsigned int x = 0; x < _Nx; x++) {
      for (unsigned int y = 0; y < _Ny; y++) {
        for (unsigned int z = 0; z < _Nz; z++) {
          _domain[x + _Nx * y + _Nx * _Ny * z + _Nx * _Ny * _Nz * 0] =
              0.6;                               // density=1.0
          _domain[x + _Nx * y + _Nx * _Ny * z + _Nx * _Ny * _Nz * 1] =
              (4.0 * _Vmax * ((double)(y) / (_Ny - 1)) *
               (1 - ((double)(y) / (_Ny - 1)))); // Vx=Vmax*y*(Ny-y)
          _domain[x + _Nx * y + _Nx * _Ny * z + _Nx * _Ny * _Nz * 2] =
              0.0;                               // Vy=0.0
          _domain[x + _Nx * y + _Nx * _Ny * z + _Nx * _Ny * _Nz * 3] =
              0.0;                               // Vz=0.0
        }
      }
    }
