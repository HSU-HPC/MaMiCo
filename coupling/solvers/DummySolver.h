...
public:
      ...
      for (unsigned int y = 0; y < _Ny; y++) {
        for (unsigned int z = 0; z < _Nz; z++) {
          ...
          _domain[x + _Nx * y + _Nx * _Ny * z + _Nx * _Ny * _Nz * 1] =
              (4.0 * _Vmax * ((double)(y) / (_Ny - 1)) * (1 - ((double)(y) / (_Ny - 1)))); // Vx=Vmax*y*(Ny-y)
  ...
  void setDensity(double density, int x, int y, int z) { _domain[x + _Nx * y + _Nx * _Ny * z + _Nx * _Ny * _Nz * 0] = density; }

  double getDensity(int x, int y, int z) { return _domain[x + _Nx * y + _Nx * _Ny * z + _Nx * _Ny * _Nz * 0]; }
