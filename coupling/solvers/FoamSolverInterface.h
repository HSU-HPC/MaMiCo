...
#ifndef _MOLECULARDYNAMICS_COUPLING_SOLVERS_FOAMSOLVERINTERFACE_H_
#define _MOLECULARDYNAMICS_COUPLING_SOLVERS_FOAMSOLVERINTERFACE_H_
...
/** interface to the Foam solver. It's almost the same as the
 * CouetteSolverInterface. That means, data from MD is received in the inner
 * region plus one cell layer from the outer region. Plus one is necessary cause
 * the Foam solver needs the boundary condition on the boundary. So it is
 * interpolated between the two cells beside. The Foam solver sends data in tha
 * outerRegion region, as defined by the parameters of the interface. . By
 * default, the argument outerRegion is set to 1: this yields that only the
 * first non-ghost layer of macroscopic cells shall be sent to MD, and all other
 * inner macroscopic cells are received from MD. The Foam solver is expected to
 * only run on rank 0.
 *  @author Philipp Neumann & Helene Wittenberg
 */
template <unsigned int dim>
class coupling::solvers::FoamSolverInterface
...
public:
  ...
  virtual bool receiveMacroscopicQuantityFromMDSolver(
    ...
    bool recv = true;
    // for (unsigned int d = 0; d < dim; d++){ recv = recv &&
    // (globalCellIndex[d]>_outerRegion) &&
    // (globalCellIndex[d]<_globalNumberMacroscopicCells[d]+1-_outerRegion); }
    for (unsigned int d = 0; d < dim; d++) {
      recv = recv && (globalCellIndex[d] > _outerRegion - 1) &&
             (globalCellIndex[d] <
              _globalNumberMacroscopicCells[d] + 2 - _outerRegion);
    }
    // for (unsigned int d = 0; d < dim; d++){ recv = recv &&
    // (globalCellIndex[d]>0) &&
    // (globalCellIndex[d]<_globalNumberMacroscopicCells[d]+1); }
    return recv;
  }
...
private:
  ...
  const tarch::la::Vector<dim, unsigned int>
      _globalNumberMacroscopicCells; // global number of macroscopic cells
};

#endif // _MOLECULARDYNAMICS_COUPLING_SOLVERS_FOAMSOLVERINTERFACE_H_
