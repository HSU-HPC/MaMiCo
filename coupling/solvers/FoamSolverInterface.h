...
namespace coupling {
namespace solvers { template <unsigned int dim> class FoamSolverInterface; }
}
 ...
 * region plus one cell layer from the outer region.
 *  Plus one is necessary cause the Foam solver needs the boundary condition on
 * the boundary. So it is interpolated between the two cells beside. The Foam
 * solver sends data in tha outerRegion
