...
namespace coupling {
namespace solvers { class NumericalSolver; }
}
...
public:
  ...
  /** flags the domain boundary cells. mdDomainOffset and mdDomainSize
   * correspond to lower/left/front corner of the MD domain and the size of the
   * domain. overlapStrip is the number of cells
   *  that the LB and MD domain shall overlap, i.e. the number of "MD cells"
   * that lie within the LB domain but which should not be flagged (and thus be
   * handled by both solvers).*/
    ...
    for (int z = 0; z < _domainSizeZ + 2; z++) {
      for (int y = 0; y < _domainSizeY + 2; y++) {
        for (int x = 0; x < _domainSizeX + 2; x++) {
          ...
          bool isMDCell = true;
          for (int d = 0; d < 3; d++) {
            isMDCell = isMDCell && (globalCoords[d] > _offset[d] + (int)
                                                          overlapStrip - 1) &&
                       (globalCoords[d] <
                        _offset[d] + _globalNumberMacroscopicCells[d] - (int)
                            overlapStrip);
          }
          if (isMDCell) {
  ...
  /** Function applies the values received from the MD-solver within the
   * conntinuum solver */
...
private:
  tarch::la::Vector<3, unsigned int> getProcessCoordinates() const {
    ...
    coords[1] = (((unsigned int) rank) -
                 coords[2] * _processes[0] * _processes[1]) / _processes[0];
    coords[0] =
  ...
  void determineParallelNeighbours() {
    ...
    if (_processes[0] * _processes[1] * _processes[2] > (unsigned int) size) {
      std::cout << "ERROR NumericalSolver::determineParallelNeighbours(): Not "
                   "enough ranks available!" << std::endl;
      exit(EXIT_FAILURE);
    }
...
// _parallelNeighbours << std::endl;
  ...
  /** determines the local domain size on this rank where channelheight is the
   * domain length in direction d. */
  int getDomainSize(double channelheight, double dx,
    ...
    coords[1] = (((unsigned int) rank) -
                 coords[2] * processes[0] * processes[1]) / processes[0];
    coords[0] =
...
protected:
  ...
  /** returns index in 2D parallel buffer with buffer dimensions
  ...
  void plot() const {
    ...
    file << "POINTS " << (_domainSizeX + 3) * (_domainSizeY + 3) *
                             (_domainSizeZ + 3) << " float" << std::endl;

    flag << "CELL_DATA " << (_domainSizeX + 2) * (_domainSizeY + 2) *
                                (_domainSizeZ + 2) << std::endl;
...
#endif
    return ((unsigned int)
            rank > _processes[0] * _processes[1] * _processes[2] - 1);
  }
  ...
  tarch::la::Vector<3, unsigned int>
      _processes;               // domain decomposition on MPI rank basis
