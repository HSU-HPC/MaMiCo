...
namespace coupling {
namespace solvers { class IcoFoam; }
}
...
public:
  IcoFoam(int rank, int plotEveryTimestep, double channelheight,
          std::string dict, std::string folder,
          tarch::la::Vector<12, unsigned int> boundariesWithMD)
      : AbstractCouetteSolver<3>(),
        runTime(Foam::Time::controlDictName, dict, folder),
        mesh(Foam::IOobject(Foam::fvMesh::defaultRegion, runTime.timeName(),
                            runTime, Foam::IOobject::MUST_READ)),
        transportProperties(Foam::IOobject(
        ...
        p(Foam::IOobject("p", runTime.timeName(), mesh,
                         Foam::IOobject::MUST_READ, Foam::IOobject::AUTO_WRITE),
          mesh),
        U(Foam::IOobject("U", runTime.timeName(), mesh,
                         Foam::IOobject::MUST_READ, Foam::IOobject::AUTO_WRITE),
          mesh),
        phi(Foam::IOobject("phi", runTime.timeName(), mesh,
                           Foam::IOobject::READ_IF_PRESENT,
                           Foam::IOobject::AUTO_WRITE),
            Foam::fvc::flux(U)),
        piso(mesh), _boundariesWithMD(boundariesWithMD),
        _dx(std::cbrt(Foam::max(mesh.cellVolumes()))),
        _channelheight(channelheight),
        _boundary2RecvBufferIndicesOuter(
            new unsigned int[_numberBoundaryPoints]),
        _boundary2RecvBufferIndicesInner(
            new unsigned int[_numberBoundaryPoints]),
        _boundaryIndices(new Foam::vector *[_numberBoundaryPoints]),
        _rank(rank), _plotEveryTimestep(plotEveryTimestep) {
    if (skipRank()) {
      return;
    }
    ...
    _numberBoundaryPoints =
        6 * U.boundaryFieldRef()[innerMDBoundaryIndex].size();
    Foam::setRefCell(p, mesh.solutionDict().subDict("PISO"), pRefCell,
                     pRefValue);
  ...
  getVelocity(tarch::la::Vector<3, double> pos) const override {
    ...
    if (foamIndice > 0) {
      return tarch::la::Vector<3, double>(U[foamIndice][0], U[foamIndice][1],
                                          U[foamIndice][2]);
    } else {
  ...
  // Gets the next cell center beside the boundary, necessary to set the
  // boundary condition from MD data
  const tarch::la::Vector<3, double>
  getOuterPointFromBoundary(const int layer, const int index) {
    const Foam::vectorField FoamCoord =
        U.boundaryFieldRef()[layer].patch().Cf()[index] +
        (U.boundaryFieldRef()[layer].patch().nf() * _dx * 0.5);
    const tarch::la::Vector<3, double> FoamCoordVector(
        FoamCoord[0][0], FoamCoord[0][1], FoamCoord[0][2]);
  ...
  // boundary condition from MD data
  const tarch::la::Vector<3, double>
  getInnerPointFromBoundary(const int layer, const int index) {
    const Foam::vectorField FoamCoord =
        U.boundaryFieldRef()[layer].patch().Cf()[index] -
        (U.boundaryFieldRef()[layer].patch().nf() * _dx * 0.5);
    const tarch::la::Vector<3, double> FoamCoordVector(
  ...
  void setMDBoundaryValues(
    ...
    for (unsigned int i = 0; i < _numberBoundaryPoints; i++) {
      ...
      tarch::la::Vector<3, double> localOuterVel(
          (1.0 / recvBuffer[outer]->getMacroscopicMass()) *
          recvBuffer[outer]->getMacroscopicMomentum());
      tarch::la::Vector<3, double> localInnerVel(
          (1.0 / recvBuffer[inner]->getMacroscopicMass()) *
          recvBuffer[inner]->getMacroscopicMomentum());
  ...
  // Settup for the mapping from MD to continuum data. Velocity values are
  // necessary directly on the boundary. Therefore the MD values from the two
  // cells beside
  // the boundary are interpolated. The function looks for the index of every
  // cell that data is necessary from. This indices will be stored in two
  // arrays.
    ...
    for (unsigned int boundary = 0; boundary < 12; boundary++) {
      if (_boundariesWithMD[boundary] == 1) {
        ...
        for (unsigned int j = 0; j < MDPointsPerBoundary; j++) {
          ...
          const unsigned int globalIndexOuter =
              indexConversion.getGlobalCellIndex(
                  indexConversion.getGlobalVectorCellIndex(
                      getOuterPointFromBoundary(boundary, j)));
          const unsigned int globalIndexInner =
              indexConversion.getGlobalCellIndex(
                  indexConversion.getGlobalVectorCellIndex(
                      getInnerPointFromBoundary(boundary, j)));
          ...
          std::cout << "IcoFoam: Within the mapping of the FoamBoundary and "
                       "the SimpleMD cells there was an error" << std::endl;
        endloop:
          for (unsigned int k = 0; k < size; k++) {
...
private:
  ...
  void plottxt() {
    ...
    if (!file.is_open()) {
      std::cout << "ERROR NumericalSolver::plottxt(): Could not open file "
                << ss.str() << "!" << std::endl;
      exit(EXIT_FAILURE);
    }
    ...
    for (double z = _dx / 2; z < _channelheight; z = z + _dx) {
      ...
      if (foamIndice > 0) {
        velocity << U[foamIndice][0] << ", " << U[foamIndice][1] << ", "
  ...
  //
  //   std::stringstream ss; ss << "Continuum_Velocity_IcoFoam_" << _rank << "_"
  // << _timestepCounter << ".vtk";
  //   std::ofstream file(ss.str().c_str());
  //   if (!file.is_open()){std::cout << "ERROR NumericalSolver::plot(): Could
  // not open file " << ss.str() << "!" << std::endl; exit(EXIT_FAILURE);}
  //   std::stringstream velocity;
  //
  //   file << "# vtk DataFile Version 2.0" << std::endl;
  //   file << "MaMiCo FoamSolver" << std::endl;
  //   file << "ASCII" << std::endl << std::endl;
  //   file << "DATASET STRUCTURED_GRID" << std::endl; It is not a structured
  // grid, the md domain in the middle is missing. ToDo check which other type
  // is fitting
  //   int pointsPerDimension = _channelheight/_dx+2;
  //   file << "DIMENSIONS " << pointsPerDimension << " " << pointsPerDimension
  // << " " << pointsPerDimension << std::endl; // everything +1 cause of change
  // in index
  //   file << "POINTS " <<
  // (pointsPerDimension)*(pointsPerDimension)*(pointsPerDimension) << " float"
  // << std::endl;
  //
  ...
  //   //   // write information to streams;
  //   //   file << U.mesh()[i][0] << ", " << U.mesh()[i][1] << ", " <<
  // U.mesh()[i][2] << std::endl; // boundary is missing, how to include? Just
  // not do?
  //   //   velocity << U[i][0] << ", " << U[i][1] << ", " << U[i][2] <<
  // std::endl;
  //   // }
  //
  //   file << std::endl;
  ...
  // The solver runs sequentially on rank 0. Therefore the function checks if
  // the acutal rank is zero.
  bool skipRank() { return !(_rank == 0); }

  // the following are original OpenFOAM variables, their names shall not be
  // changed
  Foam::Time runTime;
  Foam::fvMesh mesh;
