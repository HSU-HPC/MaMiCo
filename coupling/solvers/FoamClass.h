...
public:
  ...
  // cells beside the boundary are interpolated. The function looks for the
      ...
      if (_boundariesWithMD[boundary] == 1) {
        ...
        for (unsigned int j = 0; j < MDPointsPerBoundary; j++) {
          ...
          std::cout << "IcoFoam: Within the mapping of the FoamBoundary and "
                       "the SimpleMD cells there was an error"
                    << std::endl;
        endloop:
          for (unsigned int k = 0; k < size; k++) {
...
private:
  unsigned int getNumBoundaryPoints() {
    if (_boundariesWithMD == tarch::la::Vector<12, unsigned int>{0})
      return 0;
  ...
  void plot() const {
    ...
    std::stringstream ss;
    ss << "Continuum_Velocity_IcoFoam_" << _rank << "_" << _timestepCounter
       << ".vtk";
    std::ofstream file(ss.str().c_str());
    if (!file.is_open()) {
    ...
    // << " " << pointsPerDimension << std::endl; // everything +1 cause of
    // change in index
    ...
    for (int i = 0; i < size; i++) {
      // write information to streams;
      file << U.mesh().C()[i][0] << " " << U.mesh().C()[i][1] << " "
           << U.mesh().C()[i][2]
           << std::endl; // boundary is missing, how to include? Just not do?
