...
public:
  IcoFoam(int rank, int plotEveryTimestep, double channelheight, std::string dict, std::string folder, tarch::la::Vector<12, unsigned int> boundariesWithMD)
      : AbstractCouetteSolver<3>(), runTime(Foam::Time::controlDictName, dict, folder),
        ...
        _boundary2RecvBufferIndicesOuter(new unsigned int[_numberBoundaryPoints]), _boundary2RecvBufferIndicesInner(new unsigned int[_numberBoundaryPoints]),
        _boundaryIndices(new Foam::vector *[_numberBoundaryPoints]), _rank(rank), _plotEveryTimestep(plotEveryTimestep) {
    if (skipRank()) {
      return;
    }
      ...
      if (_boundariesWithMD[boundary] == 1) {
        ...
        for (unsigned int j = 0; j < MDPointsPerBoundary; j++) {
          ...
          const unsigned int globalIndexOuter =
              indexConversion.getGlobalCellIndex(indexConversion.getGlobalVectorCellIndex(getOuterPointFromBoundary(boundary, j)));
          const unsigned int globalIndexInner =
              indexConversion.getGlobalCellIndex(indexConversion.getGlobalVectorCellIndex(getInnerPointFromBoundary(boundary, j)));
