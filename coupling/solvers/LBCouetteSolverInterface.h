...
namespace coupling {
namespace solvers { class LBCouetteSolverInterface; }
}
...
public:
  LBCouetteSolverInterface(
      tarch::la::Vector<3, unsigned int>
          avgNumberLBCells, // avg number of LB cells per process (obtained from
                            // LBCouetteSolver)
      tarch::la::Vector<3, unsigned int>
          numberProcesses, // total number of processes used by the LB Couette
                           // solver (obtained from LBCouetteSolver)
      tarch::la::Vector<3, unsigned int>
          offsetMDDomain, // offset (measured in cell units) of the MD domain
      ...
      : // see CouetteSolverInterface
        ...
        _numberProcesses(numberProcesses), _offsetMDDomain(offsetMDDomain),
...
private:
  const tarch::la::Vector<3, unsigned int>
      _avgNumberLBCells; // avg. number of LB cells per LB process (must be same
                         // for Interface and LBCouetteSolver)
  const tarch::la::Vector<3, unsigned int>
      _numberProcesses;              // number of processes used by LB solver
  const tarch::la::Vector<3, unsigned int>
      _offsetMDDomain; // offset of MD domain (excl. any ghost layers on MD or
                       // LB side)
