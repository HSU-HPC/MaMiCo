...
void riemannSolverNonlinear(
  ...
  for (int i = 0; i < basisSize; i++) {
    for (int j = 0; j < basisSize; j++) {
      ...
      solver.viscousEigenvalues(&QL[idx_QLR(i, j, 0)], direction, LL);
      solver.viscousEigenvalues(&QR[idx_QLR(i, j, 0)], direction, LR);
      std::transform(LL, LL + numberOfVariables, LL, std::abs<double>);
      std::transform(LR, LR + numberOfVariables, LR, std::abs<double>);
  ...
  for (int i = 0; i < basisSize; i++) {
    for (int j = 0; j < basisSize; j++) {
      ...
      for (int k = 0; k < numberOfVariables; k++) {
        ...
        assertion7(std::isfinite(FL[idx_FLR(i, j, k)]), i, j, k, factor,
                   penalty, maxHyperbolicEigenvalue, maxDiffusiveEigenvalue);
