 ...
 *  z*nx*ny + y*nx + x=x + nx*(y+ny*z), where nx,ny,nz are the numbers of
 * processes in x,y,z-direction.
 *  topologyOffset can be used to shift the whole topology by an offset of
 * ranks. Derived class from the class ParallelTopology.
 *  E.g. assuming ParallelTopologyType = XYZ and there is a cubic domain,
 * splitted into 8 sub-domains (2 sub-domains in each dimension). Then the
