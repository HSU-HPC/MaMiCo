...
public:
  /** this state is returned by the insertDeleteMass() function. To tell the program,
   *  which action happened through the application of the function*/
  enum Action {
  ...
  /**
   *  @brief adds or removes particles to the macroscopic cell, simulates a mass flow.
   ...
   *  @param boundaryForceController an instance of the boundaryForceController for the simulation
   ...
   *  @brief returns true, if a potential energy landscape is needed for the insertion/removal.
   ...
   *  @param insertDeleteMassAtTimestep interval of time steps for the insertion/removal of particles*/
  ParticleInsertion(unsigned int insertDeleteMassEveryTimestep) : _insertDeleteMassEveryTimestep(insertDeleteMassEveryTimestep) {
    if (_insertDeleteMassEveryTimestep == 0) {
      std::cout << "ERROR ParticleInsertion::ParticleInsertion(..): _insertDeleteMassEveryTimestep=0!" << std::endl;
      exit(EXIT_FAILURE);
    }
  }
  ...
  /** @brief returns true if mass needs to be inserted or removed in a time step t
   *  @param t time step to check for the insertion/removal of mass
   *  @returns a bool that indicates if mass insertion/deletion shall be applied in this
