...
class TestLammpsCalculateForceEnergy: public TestLammps<dim> {
  public:
    ...
    virtual void run(){
      ...
      LAMMPS_NS::MamicoLammpsMDSolverInterface<dim>* mdSolverInterface = (LAMMPS_NS::MamicoLammpsMDSolverInterface<dim>*)
        coupling::interface::MamicoInterfaceProvider<LAMMPS_NS::MamicoCell,dim>::getInstance().getMDSolverInterface();
      if (mdSolverInterface==NULL){ std::cout << "ERROR TestLammpsGhost: could not cast MD Solver interface!" << std::endl; exit(EXIT_FAILURE); }
      ...
      if (indexConversion.getThisRank()==0){
        ...
        if (fabs(potEnergy-potEnergyRef)>tolerance){
          std::cout << "ERROR TestLammpsCalculateForceEnergy: potential Energies do not match1! Potential energy ref.=" << potEnergyRef << ", potential energy=" << potEnergy << std::endl; exit(EXIT_FAILURE);
        }
        if (fabs(tarch::la::norm2(force-forceRef)>tolerance)){
          std::cout << "ERROR TestLammpsCalculcateForceEnergy: forces do not match1! Force ref.=" << forceRef << ", force=" << force << std::endl; exit(EXIT_FAILURE);
        }
      ...
      if (indexConversion.getThisRank()==0){
        ...
        for (int i = 0; i < nlocal; i++){
          bool found = true; for (int d = 0; d < dim; d++){ found = found && (thisPosition[d]==lammps->atom->x[i][d]); }
          if (found){ n=i; break;}
        }
        if (n<0){ std::cout << "ERROR TestLammpsCalculateForceEnergy: Could not find molecule after adding it!" << std::endl; exit(EXIT_FAILURE); }
        else {
        ...
        if (fabs(potEnergy-potEnergyRef)>tolerance){
          std::cout << "ERROR TestLammpsCalculateForceEnergy: potential Energies do not match2! Potential energy ref.=" << potEnergyRef << ", potential energy=" << potEnergy << std::endl; exit(EXIT_FAILURE);
        }
      }
    }
