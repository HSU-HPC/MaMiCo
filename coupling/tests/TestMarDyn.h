 ...
 * 	initializes all parts of the coupling and tests the get methods of the MDSolverInterface
 *	@author Hanno Flohr
 */
...
public:
	...
	virtual ~TestMarDyn() {
		...
		if(_marDynMDsolver!=NULL) {delete _marDynMDsolver; _marDynMDsolver=NULL;}
		if(_marDyn!=NULL) {delete _marDyn; _marDyn=NULL;}
	};
	...
	virtual void run() {
		...
		if(testValue!=1.0) std::cout << "Wrong mass, should be 1.0!" << std::endl;

		//test getKB()
		testValue = mdsi->getKB();
		std::cout << "kB: " << testValue << std::endl;
		if(testValue!=1.0) std::cout << "Wrong Boltzmann's constant, should be 1.0!" << std::endl;

		//test getMoleculeSigma()
		testValue = mdsi->getMoleculeSigma();
		std::cout << "Molecule sigma: " << testValue << std::endl;
		if(testValue!=1.0) std::cout << "Wrong molecule sigma value, should be 1.0!" << std::endl;

		//test getMoleculeEpsilon()
		testValue = mdsi->getMoleculeEpsilon();
		std::cout << "Molecule epsilon: " << testValue << std::endl;
		if(testValue!=1.0) std::cout << "Wrong molecule epsilon value, should be 1.0!" << std::endl;

		//test getDt
		testValue = mdsi->getDt();
		std::cout << "Timestep: " << testValue << std::endl;
		if(testValue!=0.002) std::cout << "Wrong timestep size, should be 0.002!" << std::endl;

		//test getGlobalMDDomainSize()
		testVec = mdsi->getGlobalMDDomainSize();
		std::cout << "GlobalMDDomainSize: " << testVec << std::endl;
		validResult = (testVec[0]==30.0); for(int d=0;d<3;d++) {validResult = validResult && (testVec[d]==30.0);}
		if(!validResult) std::cout << "Wrong global MD domain size! Should be 30.0 in each component!" << std::endl;

		//test getGlobalMDDomainOffset()
		testVec = mdsi->getGlobalMDDomainOffset();
		std::cout << "GlobalMDDomainOffset: " << testVec << std::endl;
		validResult = (testVec[0]==0.0); for(int d=0;d<3;d++) {validResult = validResult && (testVec[d]==0.0);}
		if(!validResult) std::cout << "Wrong global MD domain offset! Should be 0.0 in each component!" << std::endl;
...
protected:
	...
	void loadMacroscopicSolverConfiguration() {
		...
		if(_macroSolverInterface==NULL) {std::cout << "ERROR in TestMarDyn: macroSolverInterface==NULL!" << std::endl; exit(EXIT_FAILURE);}
	...
	void loadMamicoTestConfiguration() {
		...
		tarch::configuration::ParseConfiguration::parseConfiguration<coupling::configurations::MaMiCoConfiguration<3> >(
                  mamicoTestConfig,"mamico",config);
		...
		_macroscopicCellService = new coupling::services::MacroscopicCellServiceImpl<MarDynCell,3> (
			...
			coupling::interface::MamicoInterfaceProvider<MarDynCell,3>::getInstance().getMacroscopicSolverInterface(),
			numberProcesses,
			rank,
		...
		if(_macroscopicCellService==NULL) {std::cout << "ERROR TestMarDyn: _macroscopicCellService==NULL!" << std::endl; exit(EXIT_FAILURE);}
