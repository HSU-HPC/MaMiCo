...
/*	Test class. Provides an implementation of the coupling of Mardyn with input values created by a Dummy solver.
 * 	@author Hanno Flohr
 */
class TestMarDyn2DummyCoupling: public Test {
public:
	TestMarDyn2DummyCoupling(std::string name, int argc, char **argv):
		Test(name), _marDynMDSolverInterface(NULL), _dummySolver(NULL), _macroscopicCellService(NULL), _argc(argc), _argv(argv) {}

	virtual ~TestMarDyn2DummyCoupling() {}

	virtual void run() {
		std::cout << "Testing MarDyn coupled with Dummy Macroscopic solver: " << std::endl;
		...
		global_log->info() << "The coupling step took: " << mamicoTimer.get_etime() << " sec." << std::endl;
...
private:
	...
	void simulate() {
		...
		if(rank == 0) storeDummySolverDataInSendBuffer();
		...
		for(unsigned int t=0; t<timesteps; t++) _marDynMDSolverInterface->getSimulation()->simulateOneMDTimestep(t);
		...
		if(rank == 0) writeReceiveBufferDataToDummySolver();

#ifdef ENABLE_MPI
		MPI_Barrier(MPI_COMM_WORLD);
#endif

		if(rank==0) writeDummySolverInfo(timesteps);
	...
	//parse mamico config file
		...
		tarch::configuration::ParseConfiguration::parseConfiguration<coupling::configurations::MaMiCoConfiguration<3> >(
	...
	//initialize mardyn simulation
		...
		MarDynCoupledSimulation* marDynSimulation =
				new MarDynCoupledSimulation(mamicoConfig.getMacroscopicCellConfiguration().getMacroscopicCellSize(),
		...
		if(fileExists(marDynConfigFile.c_str())) {
			...
			std::cout << "ERROR: Cannot open MarDyn input file: '" << marDynConfigFile << "'" << std::endl;
			exit(EXIT_FAILURE);
		}
	...
	//initialize mardyn md solver interface
		if(_marDynMDSolverInterface!=NULL) { delete _marDynMDSolverInterface; _marDynMDSolverInterface=NULL; }
		_marDynMDSolverInterface = new MarDynMDSolverInterface(marDynSimulation);

		coupling::interface::MamicoInterfaceProvider<MarDynCell,3>::getInstance().setMDSolverInterface(_marDynMDSolverInterface);
 	...
 	//initialize dummy macroscopic solver
		if(_dummySolver!=NULL) { delete _dummySolver; _dummySolver=NULL; }
...
//
//		//set the number of processes using the same method as done in MarDyn (MPI_Dims_create)
//		int gridSize [3];
...
//#endif
        ...
        tarch::la::Vector<3,double> globalMDDomainSize = _marDynMDSolverInterface->getGlobalMDDomainSize();
        tarch::la::Vector<3,double> globalMDDomainOffset = _marDynMDSolverInterface->getGlobalMDDomainOffset();
        tarch::la::Vector<3,double> macroscopicCellSize = mamicoConfig.getMacroscopicCellConfiguration().getMacroscopicCellSize();
        //tarch::la::Vector<3,unsigned int> linkedCellsPerMacroscopicCell = mamicoConfig.getMacroscopicCellConfiguration().getNumberLinkedCellsPerMacroscopicCell();
        ...
        for (loop[2]=2; loop[2] < 16; loop[2]++) {
            for (loop[1]=2; loop[1] < 16; loop[1]++) {
                for (loop[0]=2; loop[0] < 16; loop[0]++) {
                    ...
                    const double density=_dummySolver->getDensity(index[0],index[1],index[2]);
                    const tarch::la::Vector<3,double> velocity=_dummySolver->getVelocity(index[0],index[1],index[2]);

                    bool send = DummySolverInterfaceService::getInstance().addToSendBuffer(density, velocity, index);
                    if(send==true) {
                    	sendCounter++;
                    }
                }
	   ...
	   for (loop[2]=2; loop[2] < 16; loop[2]++){
		   for (loop[1]=2; loop[1] < 16; loop[1]++){
			   for (loop[0]=2; loop[0] < 16; loop[0]++){
				   ...
				   bool receive = DummySolverInterfaceService::getInstance().getFromReceiveBuffer(density, velocity, index);
				   if(receive==true) {
					   receiveCounter++;
   ...
   void writeDummySolverInfo(unsigned int timestep) {
	   std::stringstream momentum;
		...
		for(loop[2]=0;loop[2]<18;loop[2]++) {
			for(loop[1]=0;loop[1]<18;loop[1]++) {
				for(loop[0]=0;loop[0]<18;loop[0]++) {
					tarch::la::Vector<3,double> vel = _dummySolver->getVelocity(loop[0],loop[1],loop[2]);
		...
		if(!output.is_open()) {
			std::cout << "ERROR writeDummySolverInfo: could not open file '" << filename.str() << "'!" << std::endl;
			exit(EXIT_FAILURE);
		}

		output << momentum.str() << std::endl;
		momentum.clear(); momentum.str("");
   }
   ...
   void meanInputVelocity() {
	   ...
	   for(loop[1]=3;loop[1]<15;loop[1]++) {
		   tarch::la::Vector<3,double> velocity = _dummySolver->getVelocity(loop[0],loop[1],loop[2]);
	   ...
	   std::cout << "computed mean input velocity is: " << meanInputVelocity << std::endl;
   }
