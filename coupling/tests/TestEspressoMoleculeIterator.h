 ...
 *  I used the same input file as for LAMMPS inputpositions3D_moleculeiterator.xyz
 *  @author Rahul Arora
 */
...
public:
	...
	if(this_node == 0) {
        	...
        	|| Tcl_Eval(interp, "setmd box_l 10 10 10;") || Tcl_Eval(interp, "setmd max_num_cells 64;");
			...
			if(interflag == 1 || interflag2 == 1) { std::cout << "ERROR: interactions definition " << std::endl; }
    ...
    void testMolecules(){
		...
		for(unsigned int i=0; i< local_cells.n; i++) {
			...
			if(numberOfMoleculesFound != numberOfMoleculesInCurrentCell) {
				flag=0;
				std::cout << "ERROR: The number of molecules in the current cell "<< i << " are expected to be " << numberOfMoleculesInCurrentCell << " while our implementation of molecule iterator returns " << numberOfMoleculesFound << std::endl;
			}
		}
		if(flag==1) { 
