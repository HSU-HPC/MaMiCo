 ...
 * indices in the macroscopic cells and print the values. We loop over both
...
private:
  ...
  class TestMDSolverInterface
      : public coupling::interface::MDSolverInterface<simplemd::LinkedCell,
                                                      dim> {
  public:
    TestMDSolverInterface()
        : coupling::interface::MDSolverInterface<simplemd::LinkedCell, dim>() {
    } virtual ~TestMDSolverInterface() {
    }

    virtual simplemd::LinkedCell &getLinkedCell(
    ...
    virtual tarch::la::Vector<dim, double> getGlobalMDDomainSize() const {
      return tarch::la::Vector<dim, double>(1.0);
    }
    ...
    virtual coupling::interface::MoleculeIterator<simplemd::LinkedCell, dim> *
  ...
  public:
    WriteIndex(const coupling::IndexConversion<dim> &indexConversion)
        : _indexConversion(indexConversion) {}
    ~WriteIndex() {}
    void beginCellIteration() {}
    void endCellIteration() {}
    void apply(coupling::datastructures::MacroscopicCellWithLinkedCells<
                   simplemd::LinkedCell, dim> &cell,
               const unsigned int &index) {
      tarch::la::Vector<dim, unsigned int> localIndex =
          _indexConversion.getLocalVectorCellIndex(index);
  ...
  template <unsigned int dim> void testMacroscopicCells() {
    TestMDSolverInterface<dim> *testInterface =
        new TestMDSolverInterface<dim>();
    ...
    const coupling::IndexConversion<dim> indexConversion(
        globalNumberCells, numberProcesses, rank,
        testInterface->getGlobalMDDomainSize(),
        testInterface->getGlobalMDDomainOffset(),
        coupling::paralleltopology::XYZ);
    const tarch::la::Vector<dim, unsigned int>
        numberLinkedCellsPerMacroscopicCell(2);

    coupling::datastructures::MacroscopicCells<simplemd::LinkedCell, dim>
        macroscopicCells(numberLinkedCellsPerMacroscopicCell, indexConversion,
                         testInterface);
    WriteIndex<dim> writeIndex(indexConversion);
    PrintIndex<dim> printIndex;

    // write indices to inner macroscopic cells
    macroscopicCells.applyToLocalNonGhostMacroscopicCellsWithLinkedCells(
        writeIndex);
    macroscopicCells.applyToLocalNonGhostMacroscopicCellsWithLinkedCells(
        printIndex);

    // write indices to outer macroscopic cells
    macroscopicCells.applyToLocalGhostMacroscopicCellsWithLinkedCells(
        writeIndex);
    macroscopicCells.applyToLocalGhostMacroscopicCellsWithLinkedCells(
        printIndex);
