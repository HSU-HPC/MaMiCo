 ...
 *  Therefore, we define two classes which just set the local vector cell indices in the macroscopic cells
...
class MacroscopicCellsTest: public Test {
  ...
  private:
    ...
    class TestMDSolverInterface: public coupling::interface::MDSolverInterface<simplemd::LinkedCell,dim>{
      public:
        ...
        virtual void getInitialVelocity(const tarch::la::Vector<dim,double>& meanVelocity, const double &kB, const double& temperature, tarch::la::Vector<dim,double>& initialVelocity) const {}
    ...
    class WriteIndex {
      public:
        ...
        void apply(coupling::datastructures::MacroscopicCellWithLinkedCells<simplemd::LinkedCell,dim> &cell,const unsigned int &index){
          ...
          for (unsigned int d = 0; d < dim; d++){convertLocal[d] = localIndex[d];}
    ...
    template<unsigned int dim>
    void testMacroscopicCells(){
      TestMDSolverInterface<dim> *testInterface = new TestMDSolverInterface<dim>();
      if (testInterface==NULL){std::cout << "ERROR MacroscopicCellsTest::testMacroscopicCells(): testInterface==NULL!" << std::endl; exit(EXIT_FAILURE);}
      const tarch::la::Vector<dim,unsigned int> globalNumberCells(3);
      const tarch::la::Vector<dim,unsigned int> numberProcesses(1);
      const unsigned int rank = 0;
      const coupling::IndexConversion<dim> indexConversion(
        globalNumberCells,numberProcesses,rank,testInterface->getGlobalMDDomainSize(),testInterface->getGlobalMDDomainOffset(),coupling::paralleltopology::XYZ
      );
      const tarch::la::Vector<dim,unsigned int> numberLinkedCellsPerMacroscopicCell(2);

      coupling::datastructures::MacroscopicCells<simplemd::LinkedCell,dim> macroscopicCells(
        numberLinkedCellsPerMacroscopicCell,indexConversion, testInterface
      );
      ...
      delete testInterface; testInterface = NULL;
    }
};
