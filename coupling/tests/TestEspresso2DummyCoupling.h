   ...
   thus we have to break this loop in order!!*/
    ...
    if (this_node == 0) {
      ...
      int globalvarsflag = Tcl_Eval(interp, "setmd time_step 0.002;") ||
                           ...
                           Tcl_Eval(interp, "setmd box_l 30 30 30;") ||
                           Tcl_Eval(interp, "setmd max_num_cells 3456;") ||
      ...
      in.close();
      std::cout << "The cell grid is " << dd.cell_grid[0] << " "
                << dd.cell_grid[1] << " " << dd.cell_grid[2] << std::endl;
      mpi_stop();
    } else {
      mpi_loop();
    }
    ...
    std::cout << "Rank " << rank << " Total number of particles is " << n_part
              << " and no of particles on this node are " << count
              << " and the number of cells on this node are " << local_cells.n
              << std::endl;

    std::cout << "Node Grid " << node_grid[0] << " " << node_grid[1] << " "
              << node_grid[2] << " My node is " << this_node
              << " Node position " << node_pos[0] << " " << node_pos[1] << " "
              << node_pos[2] << std::endl;
#endif
    ...
    if (this_node == 0) {
      ...
      for (loop0[1] = 0; loop0[1] < 18; loop0[1]++) {
        tarch::la::Vector<3, double> tempVel =
            _dummySolver.getVelocity(loop0[0], loop0[1], loop0[2]);
        std::cout << loop0[1] << " " << tempVel[0] << " "
                  << _dummySolver.getDensity(loop0[0], loop0[1], loop0[2])
                  << std::endl;
      }
    ...
    for (unsigned int i = 0; i < local_cells.n; i++) {
      ...
      for (unsigned int j = 0; j < list->n; j++) {
        outfile << part[j].p.identity << "  " << part[j].r.p[0] << " "
                << part[j].r.p[1] << " " << part[j].r.p[2] << "  "
                << part[j].m.v[0] << " " << part[j].m.v[1] << " "
                << part[j].m.v[2] << "\n";
      }
    }
    outfile.close();
    ...
    if (this_node == 0) {
      ...
      for (loop[1] = 0; loop[1] < 18; loop[1]++) {
        ...
        std::cout << loop[1] << " " << tempVel[0] << " "
                  << _dummySolver.getDensity(loop[0], loop[1], loop[2])
                  << std::endl;
      }
    }
