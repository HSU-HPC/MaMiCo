 ...
 * 	Tests the deletion and then insertion of a molecule into the ParticleContainer
 *	@author Hanno Flohr
 */
...
private:
	void deleteMoleculeTest() {
		...
		MarDynMoleculeWrapper testMolecule;
 		...
 		for(int d=0;d<3;d++) macroCellIndex[d] = 1 + (int) floor(position[d] / 2.5);
 		...
 		if(moleculeCountBefore != (moleculeCountAfter+1)) std::cout << "Molecule deletion test failed!" << std::endl;
 		else std::cout << "Molecule deletion test successful!" << std::endl;
 		...
 		if(moleculeCountAfter==moleculeCountAfterSync) std::cout << "synchronize test successfull!" << std::endl;
 		else std::cout << "synchronize test failed: " << moleculeCountAfterSync << "in particleContainer after synchronize." <<  std::endl;
	}
	...
	void addMoleculeTest() {
		...
		for(int d=0; d<3; d++) position[d] = 15.0;
		Molecule internalMolecule;
                for (int d = 0; d < 3; d++){ internalMolecule.setr(d,position[d]); internalMolecule.setv(d,dummy[d]);}
		...
		if(moleculesBefore != (moleculesAfter-1)) std::cout << "Molecule insertion test failed!" << std::endl;
		else std::cout << "Molecule insertion test successful!" << std::endl;
	}
};
