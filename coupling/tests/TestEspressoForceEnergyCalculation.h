 ...
 *  I add a single particle at the center of the domain. In this simple scenario, the net force acting on the new particle should be
 *  zero, while its potential energy should be greater than zero.
 *  Also I provide a very simple test to the method getLinkedCellIndexForMoleculePositionTest
 *  @author Rahul Arora
 */
...
private:
  /* This test function is used on a test domain ,which consists of 8 cells and each cell conatin a molecule in the center of the cell, in this function first
   * we add an molecule at the center of the domain , equidistant from all the molecules at time t=0. Then we compute the force acting on that molecule as well
   * as its potential energy. The force should be zero ( the forces are equal in magnitude and opposite in direction), while the potential energy has to be non
   * zero */
  ...
  void calculateForceandEnergyTest() {
    ...
    // Call to the method calculateForceAndEnergy, provided in EspressoMDSolverInterface
  ...
  /* This function calcultes the linked cell index for each molecule in the domain (for local cells only, no ghosts) and prints it out */
  void getLinkedCellIndexForMoleculePositionTest() {
    ...
    // Call to the method getLinkedCellIndexForMoleculePosition, provided in EspressoMDSolverInterface
