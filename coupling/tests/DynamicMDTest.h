...
public:
  DynamicMDTest(int argc, char **argv)
      : Test("DynamicMDTest"), _generator(std::chrono::system_clock::now().time_since_epoch().count()), _generatorForVaryMD(0),
...
private:
  ...
  void parseConfigurations() {
    tarch::configuration::ParseConfiguration::parseConfiguration<simplemd::configurations::MolecularDynamicsConfiguration>("couette.xml", "molecular-dynamics",
                                                                                                                           _simpleMDConfig);
  ...
  void initSolvers() {
    ...
    if (_cfg.miSolverType == coupling::configurations::CouetteConfig::MicroSolverType::SYNTHETIC) {
      if (_cfg.twsLoop) {
        _noiseReduction =
            _mamicoConfig.getNoiseReductionConfiguration().interpreteConfiguration<3>(_multiMDCellService->getIndexConversion(), *_multiMDService, _tws);
      } else {
        _noiseReduction =
            _mamicoConfig.getNoiseReductionConfiguration().interpreteConfiguration<3>(_multiMDCellService->getIndexConversion(), *_multiMDService);
      }
    }
  ...
  void twoWayCoupling(int cycle) {
    if ((_cfg.maSolverType == coupling::configurations::CouetteConfig::MacroSolverType::COUETTE_LB) && _cfg.twoWayCoupling && cycle == _cfg.filterInitCycles) {
    ...
    if ((_cfg.maSolverType == coupling::configurations::CouetteConfig::MacroSolverType::COUETTE_LB) && _cfg.twoWayCoupling && cycle >= _cfg.filterInitCycles) {
      static_cast<coupling::solvers::LBCouetteSolver *>(_couetteSolver)
          ->setMDBoundaryValues(_buf.recvBuffer, _buf.globalCellIndices4RecvBuffer, _multiMDCellService->getIndexConversion());
    }
  }
  ...
  void shutdown() {
    ...
    if (_rank == 0) {
      ...
      std::cout << "Time taken for MD removel: " << (_tv.rmEnd.tv_sec - _tv.rmStart.tv_sec) + (_tv.rmEnd.tv_usec - _tv.rmStart.tv_usec) / 1000000 << " s"
                << std::endl;
    }
  ...
  void allocateSendBuffer(const coupling::IndexConversion<3> &indexConversion, coupling::interface::MacroscopicSolverInterface<3> &couetteSolverInterface) {
    ...
    if (indices == NULL) {
      std::cout << "ERROR CouetteTest::allocateSendBuffer(): indices==NULL!" << std::endl;
      exit(EXIT_FAILURE);
    }
      ...
      if (couetteSolverInterface.sendMacroscopicQuantityToMDSolver(indexConversion.getGlobalVectorCellIndex(i))) {
        ...
        if (containsThisRank) {
          ...
          if (_buf.sendBuffer[_buf.sendBuffer.size() - 1] == NULL) {
            std::cout << "ERROR CouetteTest::allocateSendBuffer: sendBuffer[" << _buf.sendBuffer.size() - 1 << "]==NULL!" << std::endl;
            exit(EXIT_FAILURE);
          }
  ...
  void allocateRecvBuffer(const coupling::IndexConversion<3> &indexConversion, coupling::interface::MacroscopicSolverInterface<3> &couetteSolverInterface) {
    ...
    if (indices == NULL) {
      std::cout << "ERROR CouetteTest::allocateRecvBuffer(): indices==NULL!" << std::endl;
      exit(EXIT_FAILURE);
    }
      ...
      if (couetteSolverInterface.receiveMacroscopicQuantityFromMDSolver(indexConversion.getGlobalVectorCellIndex(i))) {
        ...
        if (containsThisRank) {
          ...
          if (_buf.recvBuffer[_buf.recvBuffer.size() - 1] == NULL) {
            std::cout << "ERROR CouetteTest::allocateRecvBuffer: recvBuffer[" << _buf.recvBuffer.size() - 1 << "]==NULL!" << std::endl;
            exit(EXIT_FAILURE);
          }
  ...
  void write2CSV(std::vector<coupling::datastructures::MacroscopicCell<3> *> &recvBuffer, const unsigned int *const recvIndices,
    ...
    if (_cfg.csvEveryTimestep < 1 || couplingCycle % _cfg.csvEveryTimestep > 0 || recvBuffer.size() == 0)
      return;
    ...
    if (!file.is_open()) {
      std::cout << "ERROR CouetteTest::write2CSV(): Could not open file " << ss.str() << "!" << std::endl;
      exit(EXIT_FAILURE);
    }
    ...
    for (unsigned int i = 0; i < numCellsRecv; i++) {
      ...
      coupling::error::ErrorEstimation ErEs(vel[0], _cfg.temp, recvBuffer[i]->getMacroscopicMass(), _simpleMDConfig.getMoleculeConfiguration().getMass(),
                                            mamicoSoundSpeed, _multiMDService->getTotalNumberOfMDSimulations(), mamicoCellVolume);
      ...
      file << counter[0] << " ; " << counter[1] << " ; " << counter[2] << " ; " << vel[0] << " ; " << vel[1] << " ; " << vel[2] << " ; "
           << recvBuffer[i]->getMacroscopicMass() << " ; " << error << " ; " << error2 << " ; " << recvBuffer[i]->getTemperature() << ";";
      file << std::endl;
    }
  ...
  coupling::interface::MacroscopicSolverInterface<3> *getCouetteSolverInterface(coupling::solvers::AbstractCouetteSolver<3> *couetteSolver,
    ...
    if (_cfg.maSolverType == coupling::configurations::CouetteConfig::MacroSolverType::COUETTE_ANALYTICAL) {
      ...
      if (lbSolver == NULL) {
        std::cout << "ERROR CouetteTest::getCouetteSolverInterface(...), rank=" << _rank << ": Could not convert abstract to LB solver!" << std::endl;
        exit(EXIT_FAILURE);
      }
      ...
      for (unsigned int d = 0; d < 3; d++) {
        if (mdOffset[d] < 0.0) {
          std::cout << "ERROR CouetteTest::getCouetteSolverInterface(...): mdOffset[" << d << "]<0.0!" << std::endl;
          exit(EXIT_FAILURE);
        }
    ...
    if (interface == NULL) {
      std::cout << "ERROR CouetteTest::getCouetteSolverInterface(...), rank=" << _rank << ": interface==NULL!" << std::endl;
      exit(EXIT_FAILURE);
    }
    return interface;
  }
