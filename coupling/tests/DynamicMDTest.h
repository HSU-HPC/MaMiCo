...
public:
  DynamicMDTest(int argc, char **argv)
      : Test("DynamicMDTest"),
        ...
        _generatorForVaryMD(0), _distributionForVaryMD(-100, 100) {
    if (argc >= 1) {
      ...
      if (argc == 2) {
        try {
          ...
          std::cout << "ERROR DynamicMDTest::DynamicMDTest() : cannot parse "
                    << argv[1] << " into an integer." << std::endl;
  ...
  virtual void run() {
    ...
    for (int cycle = 0; cycle < _cfg.couplingCycles; cycle++) {
      ...
      //_multiMDMediator->getNumberOfActiveMDSimulations();
      //auto start = std::chrono::system_clock::now();
      runOneCouplingCycle(cycle);
      varyMD(cycle);
      //auto end = std::chrono::system_clock::now();
      //std::chrono::duration<double> diff = (end-start);
      //if(_rank == 0) std::cout << "Cycle " << cycle << ", MD instances before
      //" << N_instances
      //  << ", MD instances after " <<
      // _multiMDMediator->getNumberOfActiveMDSimulations()
      //  << ", time " << diff.count()
...
private:
  ...
  void varyMD(int cycle) {
    ...
    if (_rank == 0) {
      ...
      tarch::la::Vector<3, double> vel(_buf.recvBuffer[cellIdx]
                                           ->getMacroscopicMomentum());

      double mamicoSoundSpeed = (1 / std::sqrt(3)) * (2.5 / (50 * 0.005));
      double mamicoCellVolume = 2.5 * 2.5 * 2.5;

      coupling::error::ErrorEstimation errorControl(
          vel[0], _cfg.temp, _buf.recvBuffer[cellIdx]->getMacroscopicMass(),
          _simpleMDConfig.getMoleculeConfiguration().getMass(),
          mamicoSoundSpeed, _multiMDService->getTotalNumberOfMDSimulations(),
          mamicoCellVolume);

      errorControl.setAbsVelocityError(0.05);

      double NoMD = errorControl.getCorrectorNumberOfSamples(
          coupling::error::ErrorEstimation::Velocity,
          coupling::error::ErrorEstimation::Absolute);
...
#endif
    if (target < 0) {
      if ((int) _multiMDMediator->getNumberOfActiveMDSimulations() + target <
          _lowerBoundForVaryMD) {
        target = (_lowerBoundForVaryMD - (int)
                  _multiMDMediator->getNumberOfActiveMDSimulations());
      }
    }
    if (_rank == 0)
      std::cout << "Insert/Remove Simulations : " << target << std::endl;
    if (target < 0) {
  ...
  void runOneCouplingCycle(int cycle) {
    ...
    write2CSV(_buf.recvBuffer, _buf.globalCellIndices4RecvBuffer,
              _multiMDCellService->getIndexConversion(), cycle);
  ...
  void parseConfigurations() {
    ...
    _cfg = coupling::configurations::CouetteConfig::parseCouetteConfiguration(
        "couette.xml");
  }
  ...
  void initSolvers() {
    ...
    _instanceHandling = new coupling::InstanceHandling<MY_LINKEDCELL, 3>(
        _simpleMDConfig, _mamicoConfig, *_multiMDService);
    if (_instanceHandling == nullptr) {
      std::cout
          << "ERROR DynamicMDTest::initSolvers() : _instanceHandling == NULL!"
          << std::endl;
      std::exit(EXIT_FAILURE);
    }
    ...
    if (_cfg.miSolverType ==
        coupling::configurations::CouetteConfig::MicroSolverType::SIMPLEMD) {
      // equilibrate MD
    ...
    _multiMDMediator = new coupling::MultiMDMediator<MY_LINKEDCELL, 3>(
        *_multiMDCellService, *_instanceHandling, *_multiMDService,
    ...
    allocateSendBuffer(_multiMDCellService->getIndexConversion(),
                       *couetteSolverInterface);
    allocateRecvBuffer(_multiMDCellService->getIndexConversion(),
                       *couetteSolverInterface);

    // manually allocate noise reduction if necessary
    if (_cfg.miSolverType ==
        coupling::configurations::CouetteConfig::MicroSolverType::SYNTHETIC) {
      if (_cfg.twsLoop) {
  ...
  void advanceMicro(int cycle) {
    ...
    if (_cfg.miSolverType ==
      ...
      fillRecvBuffer(_cfg.density, *_couetteSolver,
                     _multiMDCellService->getIndexConversion(), _buf.recvBuffer,
                     _buf.globalCellIndices4RecvBuffer);
  ...
  void computeSNR(int cycle) {
    if (_cfg.computeSNR && cycle >= _cfg.filterInitCycles) {
      ...
      const coupling::IndexConversion<3> &indexConversion =
          _multiMDCellService->getIndexConversion();
  ...
  void twoWayCoupling(int cycle) {
    if ((_cfg.maSolverType == coupling::configurations::CouetteConfig::
                                  MacroSolverType::COUETTE_LB) &&
    ...
    if ((_cfg.maSolverType == coupling::configurations::CouetteConfig::
        ...
        _cfg.twoWayCoupling && cycle >= _cfg.filterInitCycles) {
      static_cast<coupling::solvers::LBCouetteSolver *>(_couetteSolver)
          ->setMDBoundaryValues(_buf.recvBuffer,
                                _buf.globalCellIndices4RecvBuffer,
                                _multiMDCellService->getIndexConversion());
    }
  ...
  void shutdown() {
    ...
    if (_rank == 0) {
      ...
      std::cout << _tv.micro / time_total * 100 << ", "
                ...
                << std::endl;
      std::cout << "Time taken for MD removel: "
                << (_tv.rmEnd.tv_sec - _tv.rmStart.tv_sec) +
                       (_tv.rmEnd.tv_usec - _tv.rmStart.tv_usec) / 1000000
                << " s" << std::endl;
    }
  ...
  void write2CSV(
    ...
    if (_cfg.csvEveryTimestep < 1 ||
        couplingCycle % _cfg.csvEveryTimestep > 0 || recvBuffer.size() == 0)
      return;
    // form file name and open file
    std::stringstream ss;
    ss << "out/CouetteAvgMultiMDCells_" << _rank << "_" << couplingCycle
       << ".csv";
    ...
    for (unsigned int i = 0; i < numCellsRecv; i++) {
      ...
      coupling::error::ErrorEstimation ErEs(
          vel[0], _cfg.temp, recvBuffer[i]->getMacroscopicMass(),
          _simpleMDConfig.getMoleculeConfiguration().getMass(),
          mamicoSoundSpeed, _multiMDService->getTotalNumberOfMDSimulations(),
          mamicoCellVolume);
      ...
      double error = ErEs.getError(coupling::error::ErrorEstimation::Density,
                                   coupling::error::ErrorEstimation::Absolute);

      //       coupling::error::ErrorEstimation ErEs2(1, _cfg.temp,
      // ((int)recvBuffer[i]->getMacroscopicMass()),
      //                                             _simpleMDConfig.getMoleculeConfiguration().getMass(),
      // mamicoSoundSpeed,1, mamicoCellVolume);

      double error2 = ErEs.getError(coupling::error::ErrorEstimation::Density,
                                    coupling::error::ErrorEstimation::Relative);

      // std::cout << recvBuffer[i]->getTemperature() << error << " ; " <<
      // error2 << " ; " << recvBuffer[i]->getMacroscopicMass() << std::endl;
      file << counter[0] << " ; " << counter[1] << " ; " << counter[2] << " ; "
           << vel[0] << " ; " << vel[1] << " ; " << vel[2] << " ; "
           << recvBuffer[i]->getMacroscopicMass() << " ; " << error << " ; "
           << error2 << " ; " << recvBuffer[i]->getTemperature() << ";";
      file << std::endl;
    }
  ...
  void fillSendBuffer(
    ...
    for (unsigned int i = 0; i < size; i++) {
      ...
      if (_cfg.maSolverType ==
          coupling::configurations::CouetteConfig::MacroSolverType::COUETTE_LB)
  ...
  getCouetteSolver(const double dx, const double dt) {
    ...
    if (_cfg.maSolverType == coupling::configurations::CouetteConfig::
                                 MacroSolverType::COUETTE_ANALYTICAL) {
      if (_rank == 0) {
    ...
    } else if (_cfg.maSolverType == coupling::configurations::CouetteConfig::
                                        MacroSolverType::COUETTE_LB) {
