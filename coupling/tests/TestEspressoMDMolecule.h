...
public:
	...
	/* compareParticleList() compares the list of particles in each cell, one directly read from Espresso Simulation and other from the list 	sent to the coupling tool (EspressoMDMolecule) */
    ...
    void compareParticleLists(int c, int &flag) {
		...
		if(listEspresso.size() != (unsigned int) 2*number){ std::cout << "ERROR: particle list has wrong size!" << std::endl; flag=0;}	
