  ...
  private:
    ...
    void test(){
      ...
      for (unsigned int i = 0; i < numberCellsInclBoundary; i++){
        ...
        if (cellsMamico[i]==NULL){std::cout << "ERROR FromMD2Macro::test(): cellsMamico[i]==NULL!" << std::endl; exit(EXIT_FAILURE);}
      ...
      fromMD2Macro.sendFromMD2Macro(
        indexConversion, testDataExchangeFromMD2Macro, cellsMamico,
        receivedCells, receivedGlobalIndices
      );
      ...
      // free memory
      for (unsigned int i = 0; i < (unsigned int) receivedCells.size(); i++){if (receivedCells[i]!=NULL){delete receivedCells[i]; receivedCells[i] = NULL;} }
      if (receivedGlobalIndices!=NULL){delete [] receivedGlobalIndices; receivedGlobalIndices = NULL;}
