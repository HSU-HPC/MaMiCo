...
private:
  ...
  void initSolvers() {
    ...
    if (_cfg.twsLoop) {
      ...
      _multiMDCellService = new coupling::services::MultiMDCellService<MY_LINKEDCELL, 3>(
          ...
          _mamicoConfig.getBoundaryForceConfiguration(), _mamicoConfig.getTransferStrategyConfiguration(), _mamicoConfig.getNoiseReductionConfiguration(),
          _mamicoConfig.getParallelTopologyConfiguration(), _simpleMDConfig.getSimulationConfiguration().getNumberOfTimesteps(),
  ...
  void advanceMicro(int cycle) {
    ...
    if (_cfg.miSolverType == SYNTHETIC) {
      fillRecvBuffer(_cfg.density, *_couetteSolver, _multiMDCellService->getMacroscopicCellService(0).getIndexConversion(), _buf.recvBuffer,
