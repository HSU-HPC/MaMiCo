...
private:
  ...
  void parseCouetteTestConfiguration() {
    ...
    if (type == "lb") {
      ...
      if (!_cfg.twoWayCoupling && _foam.boundariesWithMD != tarch::la::Vector<12, unsigned int>{0}) {
  ...
  void initSolvers() {
    ...
    if (_cfg.twsLoop) {
      ...
      _multiMDCellService = new coupling::services::MultiMDCellService<MY_LINKEDCELL, 3>(
          _mdSolverInterface, couetteSolverInterface, _simpleMDConfig.getMPIConfiguration().getNumberOfProcesses(), (unsigned int)_rank,
          _cfg.totalNumberMDSimulations, _mamicoConfig.getParticleInsertionConfiguration(), _mamicoConfig.getMomentumInsertionConfiguration(),
          _mamicoConfig.getBoundaryForceConfiguration(), _mamicoConfig.getTransferStrategyConfiguration(), _mamicoConfig.getParallelTopologyConfiguration(),
          _mamicoConfig.getThermostatConfiguration(), _simpleMDConfig.getSimulationConfiguration().getNumberOfTimesteps(),
  ...
  void advanceMacro(int cycle) {
    if (_couetteSolver != NULL) {
      ...
      fillSendBuffer(_cfg.density, *_couetteSolver, _multiMDCellService->getMacroscopicCellService(0).getIndexConversion(), _buf.sendBuffer,
...
#if (BUILD_WITH_OPENFOAM)
    else if ((_cfg.maSolverType == COUETTE_FOAM) && _cfg.twoWayCoupling && cycle == _cfg.filterInitCycles && _couetteSolver != NULL) {
