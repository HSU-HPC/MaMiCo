 ...
 * 	Loops through all ParticleCells of the simulation, sets an MarDynMoleculeIterator interface for each one,
 * 	then compares the number of molecules of each ParticleCell with the number of molecules obtained through the interface.
 * 	@author Hanno Flohr
 */
...
protected:
	void testIterator() {
		...
		for(int i=0; i<2744; i++) {
			...
			if(moleculeCount!=0) std::cout << "Test: " << moleculeCount << " =?= " << moleculesInInterface << std::endl;

			if(moleculeCount != moleculesInInterface) {
				errors++;
				std::cout << "ERROR: number of molecules in current cell " << i << "should be " << moleculeCount << ". The interface instead has: " << moleculesInInterface << std::endl;
			}
		}
