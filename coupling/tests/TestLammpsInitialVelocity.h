...
class TestLammpsInitialVelocity: public TestLammps<dim> {
  public:
    ...
    virtual void run(){
      ...
      const tarch::la::Vector<dim,double> meanVelocity(1.0); // value for avg velocity
      const double stdDev = std::sqrt(dim*kB*temperature/mdInterface->getMoleculeMass());
      ...
      // compute avg
      for (int i = 0; i < numberExperiments; i++){ expAvg = expAvg + velocities[i]; }
      expAvg = (1.0/numberExperiments)*expAvg;
      // compute std dev
      for (int i = 0; i < numberExperiments; i++){ expStdDev = expStdDev + tarch::la::dot( velocities[i] - expAvg, velocities[i] - expAvg); }
      expStdDev = std::sqrt(expStdDev/numberExperiments);


      std::cout << "Average: " << meanVelocity << " measured: " << expAvg <<          ", ||. - .||_2: " << tarch::la::norm2(meanVelocity-expAvg) << std::endl;
      std::cout << "Std-dev: " << stdDev             << " measured: " << expStdDev << ", ||. - .||_2: " << fabs(stdDev-expStdDev) << std::endl;
      if (tarch::la::norm2(meanVelocity-expAvg)>tol){ std::cout << "ERROR TestLammpsInitialVelocity: Avg. velocity deviation exceeds " << tol << "!" << std::endl; exit(EXIT_FAILURE); }
      if (fabs(stdDev-expStdDev)>tol)               { std::cout << "ERROR TestLammpsInitialVelocity: Std. dev. deviation exceeds " << tol << "!" << std::endl; exit(EXIT_FAILURE); }
    }
