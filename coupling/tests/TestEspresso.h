...
void loadEspressoTestConfiguration(){
	...
	if(this_node == 0) {
        ...
        || Tcl_Eval(interp, "setmd box_l 2 2 2;") || Tcl_Eval(interp, "setmd max_num_cells 8;");
	    ...
	    if (partflag == 1) { std::cout << " ERROR: setting up particle properties" << std::endl; }
		
		// Integrate command 
		int integrateflag = Tcl_Eval(interp, "integrate 100");
        if ( integrateflag == 1) { std::cout << "ERROR: integrate command " << std::endl; }
        ...
        for(unsigned int i=0; i<8; i++) {
        	ss.str(""); ss << "puts $trajectory \"" << i << " [part " << i << " print pos] [part " << i << " print v]\"";
        	outflag = Tcl_Eval(interp, ss.str().c_str());
        }
        outflag = Tcl_Eval(interp, "close $trajectory");
        if (outflag == 1) { std::cout << "ERROR: integrate command " << std::endl; }
	}
	else {
		mpi_loop();
	}	 
