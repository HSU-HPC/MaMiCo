...
class TestDataExchangeFromMD2Macro: public coupling::sendrecv::DataExchange<TestCell<dim>,dim > {
  ...
  public:
    ...
    virtual void readFromCell(double * const buffer, const TestCell<dim>& cell){
      ...
      for (unsigned int d = 0; d < dim; d++){buffer[d] = b1[d];}
    ...
    virtual void getBuffer4MacroscopicSolverCells(unsigned int &numberCells, std::vector<TestCell<dim>* > &cells, unsigned int * &globalCellIndices){
      ...
      if (_indexConversion.getThisRank() == 0){
        ...
        const tarch::la::Vector<dim,unsigned int> start =
          ...
          (_indexConversion.getNumberProcesses()-tarch::la::Vector<dim,unsigned int>(1))*
           _indexConversion.getAverageLocalNumberMacroscopicCells()[0];

        numberCells = (_indexConversion.getGlobalNumberMacroscopicCells()
                       - (_indexConversion.getNumberProcesses()-tarch::la::Vector<dim,unsigned int>(1))*
        ...
        for (unsigned int i = 0; i < numberCells; i++){
          ...
          if (cells[i]==NULL){std::cout << "ERROR getBuffer4MacroscopicSolverCells: cells[i]==NULL!" << std::endl; exit(EXIT_FAILURE);}
        }
