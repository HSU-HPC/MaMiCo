 ...
 * MaMiCo and a dummy macroscopic solver.
 *  For N processes, this class sends data from process n to n+1. The only data
 * that are sent
 *  correspond to the points on the main "diagonal" of a field, that is cell
 * data for cells at coordinates (n,n,n) (in global ordering).
 *  @author Philipp Neumann
 */
template <unsigned int dim>
class TestDataExchangeFromMD2Macro
    : public coupling::sendrecv::DataExchange<TestCell<dim>, dim> {

public:
  TestDataExchangeFromMD2Macro(
      unsigned int tag, const coupling::IndexConversion<dim> &indexConversion)
      : coupling::sendrecv::DataExchange<TestCell<dim>, dim>(tag),
  ...
  getTargetRanks(tarch::la::Vector<dim, unsigned int> globalCellIndex) {
    ...
    if ((globalCellIndex == tarch::la::Vector<dim, unsigned int>(0)) ||
        (globalCellIndex == _indexConversion.getGlobalNumberMacroscopicCells() +
                                tarch::la::Vector<dim, unsigned int>(1))) {
      return vec;
    }
    const unsigned int rank =
        _indexConversion.getUniqueRankForMacroscopicCell(globalCellIndex);
   ...
   * global coordinates correspond to
   *  (n,n,n).
   */
  virtual std::vector<unsigned int>
   ...
   */
  virtual void
  getBuffer4MacroscopicSolverCells(unsigned int &numberCells,
                                   std::vector<TestCell<dim> *> &cells,
    ...
    if (_indexConversion.getThisRank() == 0) {
      ...
      for (unsigned int i = 0; i < numberCells; i++) {
        ...
        if (cells[i] == NULL) {
          std::cout << "ERROR getBuffer4MacroscopicSolverCells: cells[i]==NULL!"
                    << std::endl;
          exit(EXIT_FAILURE);
        }
      }
      ...
      if (globalCellIndices == NULL) {
        std::cout
            << "ERROR TestDataExchangeFromMD2Macro::getBuffer4ReceivedCells: "
               "NULL ptr returned!" << std::endl;
        exit(EXIT_FAILURE);
      }

      for (unsigned int i = 0; i < numberCells; i++) {
        globalCellIndices[i] = _indexConversion.getGlobalCellIndex(
            start + i * tarch::la::Vector<dim, unsigned int>(1));
      }
      ...
      const tarch::la::Vector<dim, unsigned int> neighbourRankCoords =
          _indexConversion.getProcessCoordinates(neighbourRank);
      bool isSource = true;
      for (unsigned int d = 1; d < dim; d++) {
        isSource =
            isSource && (neighbourRankCoords[0] == neighbourRankCoords[d]);
      }

      if (isSource) {
        // we only plan with cubic sub-domains on the processes...
        const unsigned int numberLocalCells =
            _indexConversion.getAverageLocalNumberMacroscopicCells()[0];
        ...
        for (unsigned int i = 0; i < numberLocalCells; i++) {
          ...
          if (cells[i] == NULL) {
            std::cout
                << "ERROR getBuffer4MacroscopicSolverCells: cells[i]==NULL!"
                << std::endl;
            exit(EXIT_FAILURE);
          }
        }
 ...
 * ranks, i.e. the original target ranks are
 *  the current source ranks. Since we hold certain macroscopic cells on several
 * Mamico-blocks (due to the ghost cells),
 *  we use a modified target ranks-vector (based on the indexConversion).
 */
template <unsigned int dim>
class TestDataExchangeFromMacro2MD : public TestDataExchangeFromMD2Macro<dim> {
public:
  TestDataExchangeFromMacro2MD(
      unsigned int tag, const coupling::IndexConversion<dim> &indexConversion)
  ...
  virtual std::vector<unsigned int>
  getTargetRanks(tarch::la::Vector<dim, unsigned int> globalCellIndex) {
    return TestDataExchangeFromMD2Macro<dim>::_indexConversion
        .getRanksForMacroscopicCell(globalCellIndex);
  }
  virtual std::vector<unsigned int>
  getSourceRanks(tarch::la::Vector<dim, unsigned int> globalCellIndex) {
