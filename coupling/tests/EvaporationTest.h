...
private:
  ...
  enum MacroSolverType {
    COUETTE_LB = 0, ///< the Lattice-Bolzmann solver is used (coupling::solvers::LBCouetteSolver)
  };
  ...
  void initSolvers() {
    ...
    if (_couetteSolver != NULL)
      std::cout << "Couette solver not null on rank: " << _rank << std::endl; // TODO: remove debug
  ...
  coupling::solvers::AbstractCouetteSolver<3>* getCouetteSolver(const double dx, const double dt) {
    ...
    solver = new coupling::solvers::LBCouetteSolver(_cfg.channelheight, _cfg.wallVelocity, _cfg.kinVisc, dx, dt, -1, "LBCouette",
