...
/** general test class for lammps. Here, we provide tests for most get..(..) methods of the MDSolverInterface
 *  as well as initialisation routines which can be re-used by other inherited test cases.
 *  @author Philipp Neumann
 */
template<unsigned int dim>
class TestLammps: public Test {
  public:
    ...
    virtual ~TestLammps(){
      if (_macroscopicCellService != NULL){delete _macroscopicCellService; _macroscopicCellService = NULL; }
      if (_macroSolverInterface != NULL){delete _macroSolverInterface; _macroSolverInterface = NULL; }
      ...
      if (_lammps!=NULL){delete _lammps; _lammps=NULL;}
    }
    ...
    virtual void run(){
      ...
      if (dim==2) { loadLammpsTestConfiguration("inputpositionsonly2D.xyz",4); }
      else {        loadLammpsTestConfiguration("inputpositionsonly3D.xyz",8); }
      ...
      if (tmp != 2.0){std::cout << "Wrong mass value, mass=" << tmp << ", instead of 2.0!" << std::endl; exit(EXIT_FAILURE); }
      ...
      if (tmp != 1.0){std::cout << "Wrong boltzmann constant, kB=" << tmp <<", should be 1.0!" << std::endl; exit(EXIT_FAILURE); }
      ...
      if (tmp != 1.0e-14){std::cout << "Wrong time step, time step=" << tmp << ", should be 1.0e-14!" << std::endl; exit(EXIT_FAILURE);}
      // test domain size
      ...
      valid = (tmpVec[0]==10.0); for (int d = 1; d < dim; d++){valid = valid && (tmpVec[d]==10.0); }
      if (!valid){std::cout << "Wrong domain size " << tmpVec << ", should be 10.0 in each component!" << std::endl; exit(EXIT_FAILURE);}
      // test domain offset
      ...
      valid = (tmpVec[0]==1.0); for (int d = 1; d < dim; d++){valid = valid && (tmpVec[d]==1.0); }
      if (!valid){std::cout << "Wrong domain offset " << tmpVec << ", should be 1.0 in each component!" << std::endl; exit(EXIT_FAILURE); }
    }
  ...
  protected:
    ...
    void loadLammpsTestConfiguration(std::string inputmoleculeconfiguration, int numberAtoms){
      ...
      if ( (dim != 2 ) && (dim != 3) ){std::cout << "ERROR TestLammps::loadLammpsTestConfiguration: only dim=2 or dim=3 supported!" << std::endl; exit(EXIT_FAILURE); }
      // allocate lammps
      if (_lammps!=NULL){delete _lammps; _lammps = NULL;}
      _lammps = new LAMMPS_NS::LAMMPS(_argc,_argv,MPI_COMM_WORLD);
      if (_lammps==NULL){ std::cout << "ERROR TestLammps::loadLammpsTestConfiguration: Could not allocate _lammps!" << std::endl; exit(EXIT_FAILURE); }
      ...
      if (dim==2){ _lammps->input->one("dimension 2"); }
      if (dim==3){ _lammps->input->one("region box block 1.0 11.0 1.0 11.0 1.0 11.0"); }
      else       { _lammps->input->one("region box block 1.0 11.0 1.0 11.0 0.0 10.0"); }
      _lammps->input->one("create_box 1 box");
      if (dim==2){ std::stringstream ss;
                   ss << "create_atoms 1 random " << numberAtoms << " 123 box";
                   _lammps->input->one(ss.str().c_str());
                   ss.str(""); ss.clear(); ss << "read_dump " << inputmoleculeconfiguration << " 0 x y replace yes box no format xyz";
                   _lammps->input->one(ss.str().c_str()); }
      else {       std::stringstream ss;
                   ss << "create_atoms 1 random " << numberAtoms << " 123 box";
                   _lammps->input->one(ss.str().c_str());
                   ss.str(""); ss.clear(); ss << "read_dump " << inputmoleculeconfiguration << " 0 x y z replace yes box no format xyz";
                   _lammps->input->one(ss.str().c_str()); }
      ...
      if (dim==2){
        switch(size){
          case 1: 
            _lammps->input->one("fix 2 all mamico 36 123456 2.5"); break;
          case 4:
            _lammps->input->one("fix 2 all mamico 16 123456 2.5"); break;
          case 16:
            _lammps->input->one("fix 2 all mamico  9 123456 2.5"); break;
          default:
            std::cout << "ERROR TestLammps: only 1,4,16 procs supported in 2D!" << std::endl; exit(EXIT_FAILURE); break;
        }
      } else if (dim==3){
        switch(size){
          case 1:
            _lammps->input->one("fix 2 all mamico 216 123456 2.5"); break;
          case 8:
            _lammps->input->one("fix 2 all mamico  64 123456 2.5");  break;
          case 64:
            _lammps->input->one("fix 2 all mamico  27 123456 2.5"); break;
          default:
            std::cout << "ERROR TestLammps: only 1,8,64 procs supported in 3D!" << std::endl; exit(EXIT_FAILURE); break;
        }
      }
    }
    ...
    void loadMamicoTestConfiguration(){
      ...
      if (dim==2) { mamicoLammpsTestConfiguration="mamico_lammps_test_configuration2D.xml";
      } else if (dim==3){ mamicoLammpsTestConfiguration="mamico_lammps_test_configuration3D.xml";}
      ...
      if (dim==2){
        switch(size){
          case 1:
            for (int d = 0; d < dim; d++){ numberProcesses[d] = 1; }
            break;
          case 4:
            for (int d = 0; d < dim; d++){ numberProcesses[d] = 2; }
            break;
          case 16:
            for (int d = 0; d < dim; d++){ numberProcesses[d] = 4; }
            break;
           default:
            std::cout << "ERROR TestLammps: only 1,4,16 procs supported in 2D!" << std::endl; exit(EXIT_FAILURE); break;
        }
        ...
        std::cout << "ERROR TestLammps::loadMamicoTestConfiguration: only 2D and 3D supported!" << std::endl; exit(EXIT_FAILURE);
      }
      ...
      tarch::configuration::ParseConfiguration::parseConfiguration<coupling::configurations::MaMiCoConfiguration<dim> > (mamicoLammpsTestConfiguration,"mamico", config);
      ...
      _macroscopicCellService = new coupling::services::MacroscopicCellServiceImpl<LAMMPS_NS::MamicoCell,dim>(
        ...
        numberProcesses,
        rank,
        config.getParticleInsertionConfiguration(),
      ...
      if (_macroscopicCellService==NULL){std::cout << "ERROR TestLammps: _macroscopicCellService==NULL!" << std::endl; exit(EXIT_FAILURE); }
    ...
    void loadMacroscopicSolverConfiguration(){
      ...
      if (_macroSolverInterface==NULL){std::cout << "ERROR TestLammps: macroSolverInterface==NULL!" << std::endl; exit(EXIT_FAILURE); }
    ...
    void printMolecules(bool printGhostMolecules=false){
      ...
      if (printGhostMolecules) numberAtoms += _lammps->atom->nghost;

      if (rank==0) std::cout << "Print molecule information: " << std::endl;
      for (int r = 0; r < size; r++){
        if (r==rank){
          for (int i = 0; i < numberAtoms; i++){
            std::cout << "Rank " << rank << ": molecule " << i << ": " << _lammps->atom->x[i][0] << ","  << _lammps->atom->x[i][1] << "," << _lammps->atom->x[i][2] << std::endl;
          }
        }
        MPI_Barrier(MPI_COMM_WORLD);
      }
