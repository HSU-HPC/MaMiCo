 ...
 *  We use two input files for molecule coordinates inputpositionsdD_moleculeiterator.xyz
 *  where d=2 or d=3, respectively. In 2D, the molecules are sorted into the cells as follows
 *  (remember, we use a domain that starts at (1,1,1) and goes to (11,11,11) with cells with dx=2.5):
 *  -------------------------
 *  |  x  | xxx |  x  | xxx |
 *  |     |     |     |     |
 *  -------------------------
 ...
 *  |     | x x |     | x x |
 *  -------------------------
 *  Each x marks a molecule (with approx. positions ;-), but correct cell indexing). For 3D, we use the
 *  same grid on each z-layer of cells (i.e. we have a 4x4x4 grid where each 4x4x(z=Z) plane corresponds
 *  to this picture).
 *  We now use the molecule iterator to traverse the atoms and check if we obtain the correct particle
...
class TestLammpsMoleculeIterator: public TestLammps<dim> {
    ...
    virtual void run(){
      ...
      coupling::services::MacroscopicCellServiceImpl<LAMMPS_NS::MamicoCell,dim> *macroscopicCellService
        = (coupling::services::MacroscopicCellServiceImpl<LAMMPS_NS::MamicoCell,dim>*) 
          coupling::interface::MamicoInterfaceProvider<LAMMPS_NS::MamicoCell,dim>::getInstance().getMacroscopicCellService();
      if (macroscopicCellService == NULL){
        std::cout << "ERROR: Could not cast pointer to MacroscopicCellServiceImpl!" << std::endl; exit(EXIT_FAILURE);
      }
  ...
  private:
    /** returns the pairs (global macroscopic cell index, number molecules in this cell) in a vector. The number is hard-coded, oriented at the setup described above. */
    std::vector<tarch::la::Vector<2,unsigned int> > initNumberMoleculesPerMacroscopicCell(const coupling::IndexConversion<dim> &indexConversion)const {
      ...
      if (dim==2){
       buffer[0] =  7; buffer[1] = 0; moleculesPerMacroscopicCell.push_back(buffer);
       buffer[0] =  9; moleculesPerMacroscopicCell.push_back(buffer);
       buffer[0] = 19; moleculesPerMacroscopicCell.push_back(buffer);
       buffer[0] = 21; moleculesPerMacroscopicCell.push_back(buffer);

       buffer[0] =  8; buffer[1] = 2; moleculesPerMacroscopicCell.push_back(buffer);
       buffer[0] = 10; moleculesPerMacroscopicCell.push_back(buffer);
       buffer[0] = 20; moleculesPerMacroscopicCell.push_back(buffer);
       buffer[0] = 22; moleculesPerMacroscopicCell.push_back(buffer);

       buffer[0] = 13; buffer[1] = 1; moleculesPerMacroscopicCell.push_back(buffer);
       buffer[0] = 15; moleculesPerMacroscopicCell.push_back(buffer);
       buffer[0] = 25; moleculesPerMacroscopicCell.push_back(buffer);
       buffer[0] = 27; moleculesPerMacroscopicCell.push_back(buffer);

       buffer[0] = 14; buffer[1] = 3; moleculesPerMacroscopicCell.push_back(buffer);
       buffer[0] = 16; moleculesPerMacroscopicCell.push_back(buffer);
       buffer[0] = 26; moleculesPerMacroscopicCell.push_back(buffer);
       buffer[0] = 28; moleculesPerMacroscopicCell.push_back(buffer);
      } else {
        for (int layer=0; layer<4; layer++){
          buffer[0] = 43+layer*36; buffer[1] = 0; moleculesPerMacroscopicCell.push_back(buffer);
          buffer[0] = 45+layer*36; moleculesPerMacroscopicCell.push_back(buffer);
          buffer[0] = 55+layer*36; moleculesPerMacroscopicCell.push_back(buffer);
          buffer[0] = 57+layer*36; moleculesPerMacroscopicCell.push_back(buffer);

          buffer[0] = 44+layer*36; buffer[1] = 2; moleculesPerMacroscopicCell.push_back(buffer);
          buffer[0] = 46+layer*36; moleculesPerMacroscopicCell.push_back(buffer);
          buffer[0] = 56+layer*36; moleculesPerMacroscopicCell.push_back(buffer);
          buffer[0] = 58+layer*36; moleculesPerMacroscopicCell.push_back(buffer);

          buffer[0] = 49+layer*36; buffer[1] = 1; moleculesPerMacroscopicCell.push_back(buffer);
          buffer[0] = 51+layer*36; moleculesPerMacroscopicCell.push_back(buffer);
          buffer[0] = 61+layer*36; moleculesPerMacroscopicCell.push_back(buffer);
          buffer[0] = 63+layer*36; moleculesPerMacroscopicCell.push_back(buffer);

          buffer[0] = 50+layer*36; buffer[1] = 3; moleculesPerMacroscopicCell.push_back(buffer);
          buffer[0] = 52+layer*36; moleculesPerMacroscopicCell.push_back(buffer);
          buffer[0] = 62+layer*36; moleculesPerMacroscopicCell.push_back(buffer);
          buffer[0] = 64+layer*36; moleculesPerMacroscopicCell.push_back(buffer);
        }
      }
    ...
    /** loops over all macroscopic cells on this process and checks for the number of molecules in each cell. Further, we plot the molecule positions of each cell. */
    void testMolecules(
      coupling::datastructures::MacroscopicCells<LAMMPS_NS::MamicoCell,dim>& macroscopicCells,
      coupling::cellmappings::MoleculeExtractor<LAMMPS_NS::MamicoCell,dim>& moleculeExtractor,
     ...
     */
    unsigned int findNumberMolecules(const unsigned int &globalIndex, const std::vector<tarch::la::Vector<2,unsigned int> > &numberMoleculesPerMacroscopicCell) const {
      const unsigned int size = (unsigned int) numberMoleculesPerMacroscopicCell.size();
      for (unsigned int i = 0; i < size; i++){
        if (globalIndex==numberMoleculesPerMacroscopicCell[i][0]){ return numberMoleculesPerMacroscopicCell[i][1]; }
      }
