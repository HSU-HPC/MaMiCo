 ...
 * inputpositionsdD_moleculeiterator.xyz
 *  where d=2 or d=3, respectively. In 2D, the molecules are sorted into the
 * cells as follows
 *  (remember, we use a domain that starts at (1,1,1) and goes to (11,11,11)
 * with cells with dx=2.5):
 *  -------------------------
 *  |  x  | xxx |  x  | xxx |
 *  |     |     |     |     |
 *  -------------------------
 ...
 * indexing). For 3D, we use the
 *  same grid on each z-layer of cells (i.e. we have a 4x4x4 grid where each
 * 4x4x(z=Z) plane corresponds
 *  to this picture).
 *  We now use the molecule iterator to traverse the atoms and check if we
 * obtain the correct particle
