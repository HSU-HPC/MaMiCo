 ...
 * the MarDynMoleculeWrapper. Then a test using a whole cell (and the
 * MarDynMoleculeIterator) is executed, in which the values of the molecules
 * (position, velocity, force) obtained from the MarDyn simulation directly are
 * compared with the values obtained through the interfaces. The third test
 * checks the results of the getPotentialEnergy() and
