...
public:
  ...
  virtual void run() {
    ...
    const unsigned int totalNumberMDSimulations =
        8; //total number of MD simulations; MD/DPD: 64 (DPD), 128 (MD
           //scalability), 144 (MD)
    ...
    //coupling::noisereduction::NoiseReduction<3>*
    //noiseReduction(mamicoConfig.getNoiseReductionConfiguration().interpreteConfiguration<3>(multiMDCellService.getMacroscopicCellService(0).getIndexConversion(),
    //multiMDService, tws));
...
private:
  ...
  void fillSendBuffer(
    ...
    for (unsigned int i = 0; i < size; i++) {
      ...
      //*static_cast<const
      //coupling::solvers::LBCouetteSolver*>(&couetteSolver)->getDensity(cellMidPoint);
