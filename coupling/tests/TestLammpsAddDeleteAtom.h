...
public:
  ...
  virtual void run() {
    ...
    const bool performDeletion =
        (size == 1) || ((size == 4) && (rank == 0)) ||
        ((size == 16) && (rank == 1)) || ((size == 8) && (rank == 0)) ||
        ((size == 64) && (rank == 1));
    ...
    if (performDeletion) {
      ...
      coupling::interface::MoleculeIterator<LAMMPS_NS::MamicoCell, dim> *
          iterator = coupling::interface::MamicoInterfaceProvider<
              LAMMPS_NS::MamicoCell, dim>::getInstance()
              .getMDSolverInterface()->getMoleculeIterator(deleteCell);
      ...
      if (numberAtoms - 1 != TestLammps<dim>::_lammps->atom->nlocal) {
        std::cout << "ERROR TestLammpsAddDeleteAtom::run(): Same number of "
                     "atoms after deletion as before!" << std::endl;
        exit(EXIT_FAILURE);
      }
    ...
    if (performAdd) {
      ...
      coupling::interface::MoleculeIterator<LAMMPS_NS::MamicoCell, dim> *
          iterator = coupling::interface::MamicoInterfaceProvider<
              LAMMPS_NS::MamicoCell, dim>::getInstance()
              .getMDSolverInterface()->getMoleculeIterator(addCell);
      ...
      if (numberAtoms + 1 != TestLammps<dim>::_lammps->atom->nlocal) {
        std::cout << "ERROR TestLammpsAddDeleteAtom::run(): Same number of "
                     "atoms after adding new molecule as before!" << std::endl;
        exit(EXIT_FAILURE);
      }
      // check if deleted atom can be found in mamico cell
