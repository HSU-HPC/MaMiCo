...
class TestLammpsAddDeleteAtom: public TestLammps<dim> {
  public:
    ...
    virtual void run(){
      ...
      if (dim==2) { TestLammps<dim>::loadLammpsTestConfiguration("inputpositionsonly2D.xyz",4); }
      else {        TestLammps<dim>::loadLammpsTestConfiguration("inputpositionsonly3D.xyz",8); }
      ...
      tarch::la::Vector<dim,double> posDeleteAtom(0.0); posDeleteAtom[0] = 4.0; posDeleteAtom[1] = 2.0; if (dim!=2) posDeleteAtom[2] = 2.0;
      const tarch::la::Vector<dim,double> dummyVec(0.0); const double dummyEnergy = 0.0;
      const coupling::datastructures::Molecule<dim> deleteMolecule(posDeleteAtom,dummyVec,dummyVec,dummyEnergy);
      tarch::la::Vector<dim,unsigned int> deleteCellIndex(1); deleteCellIndex[0] = 2; // global cell index for deletion
      // true, if this is the respective rank that should contain this molecule
      const bool performDeletion = (size==1) || ((size==4) && (rank==0)) || ((size==16) && (rank==1))
                                             || ((size==8) && (rank==0)) || ((size==64) && (rank==1));
      ...
      const bool performAdd = (size==1) || ((size==4) && (rank==0)) || ((size==16) && (rank==5))
                                        || ((size==8) && (rank==0)) || ((size==64) && (rank==21));

      // set a very small time step (so that molecules hardly move and positions thus remain quasi identical) and run one time step to sort molecules into mamico cells
      ...
      if (performDeletion){
        ...
        LAMMPS_NS::MamicoCell &deleteCell = coupling::interface::MamicoInterfaceProvider<LAMMPS_NS::MamicoCell,dim>::getInstance().
                                            getMDSolverInterface()->getLinkedCell(
                                              deleteCellIndex,linkedCellInMacroscopicCell,linkedCellsPerMacroscopicCell,indexConversion
                                            );
        ...
        getMDSolverInterface()->deleteMoleculeFromMDSimulation(deleteMolecule,deleteCell);

        // check if number of atoms has been decremented
        if (numberAtoms-1 != TestLammps<dim>::_lammps->atom->nlocal){
          std::cout << "ERROR TestLammpsAddDeleteAtom::run(): Same number of atoms after deletion as before!" << std::endl; exit(EXIT_FAILURE);
        }
        ...
        if (cellCounterAfterDeletion != cellCounterBeforeDeletion-1){
          ...
          std::cout << "Rank " << rank << ": ERROR TestLammpsAddDeleteAtom::run(): Did not reduce number of molecules in the respective mamico cell!" << std::endl; exit(EXIT_FAILURE);
        }
        ...
        for (int i = 0; i < TestLammps<dim>::_lammps->atom->nlocal; i++){
          bool found = true;
          for (unsigned int d = 0; d < dim; d++){ found = found && tarch::la::equals(TestLammps<dim>::_lammps->atom->x[i][d],posDeleteAtom[d],tolerance); }
          if (found){
            std::cout << "ERROR TestLammpsAddDeleteAtom::run(): Found atom in LAMMPS though it should have been deleted!" << std::endl; exit(EXIT_FAILURE);
          }
        }
      ...
      if (rank==0){std::cout << "Print molecules after deletion test..." << std::endl;}
      TestLammps<dim>::printMolecules();
      ...
      if (performAdd){
        ...
        LAMMPS_NS::MamicoCell &addCell = coupling::interface::MamicoInterfaceProvider<LAMMPS_NS::MamicoCell,dim>::getInstance().
                                         getMDSolverInterface()->getLinkedCell(
                                           addCellIndex,linkedCellInMacroscopicCell,linkedCellsPerMacroscopicCell,indexConversion
                                         );
        coupling::interface::MoleculeIterator<LAMMPS_NS::MamicoCell,dim> *iterator =
        ...
        // add molecule 
        coupling::interface::MamicoInterfaceProvider<LAMMPS_NS::MamicoCell,dim>::getInstance().
        getMDSolverInterface()->addMoleculeToMDSimulation(addMolecule);

        // check if number of atoms is now equal again
        if (numberAtoms+1 != TestLammps<dim>::_lammps->atom->nlocal){
          std::cout << "ERROR TestLammpsAddDeleteAtom::run(): Same number of atoms after adding new molecule as before!" << std::endl; exit(EXIT_FAILURE);
        }
        // check if deleted atom can be found in mamico cell
        ...
        if (cellCounterAfterAdding != cellCounterBeforeAdding+1){
          int rank; MPI_Comm_rank(MPI_COMM_WORLD,&rank);
          std::cout << "Rank " << rank << ": ERROR TestLammpsAddDeleteAtom::run(): Did not increase number of molecules in the respective mamico cell!" << std::endl; exit(EXIT_FAILURE);
        }
        ...
        for (int i = 0; i < TestLammps<dim>::_lammps->atom->nlocal; i++){
          ...
          if (found){ foundMolecule=true; break; }
        }
        if (!foundMolecule){
          std::cout << "ERROR TestLammpsAddDeleteAtom::run(): Could not find new atom in LAMMPS!" << std::endl; exit(EXIT_FAILURE);
        }
      ...
      // test synchronisation of mass across processes
      {
        ...
        if (dim==2){ if (totalNumberAtoms != 4) {
          std::cout << "ERROR TestLammpsAddDeleteAtom: total number of atoms=" << totalNumberAtoms << " instead of 4" << std::endl; exit(EXIT_FAILURE);
        }}
        if (dim==3){ if (totalNumberAtoms != 8) {
          std::cout << "ERROR TestLammpsAddDeleteAtom: total number of atoms=" << totalNumberAtoms << " instead of 8" << std::endl; exit(EXIT_FAILURE);
        }}
        coupling::interface::MamicoInterfaceProvider<LAMMPS_NS::MamicoCell,dim>::getInstance().
        getMDSolverInterface()->synchronizeMoleculesAfterMassModification();
        ...
        if (totalNumberAtomsAfterInsertion != totalNumberAtoms){
          std::cout << "ERROR TestLammpsAddDeleteAtom: total number of atoms has changed; new total number of atoms: " << totalNumberAtomsAfterInsertion << "!" << std::endl; exit(EXIT_FAILURE);
        }
      }

      if (rank==0){std::cout << "Print molecules after insertion test..." << std::endl;}
      TestLammps<dim>::printMolecules();
