...
class NieTest: public Test {
  ...
  public:
    ...
    virtual void run(){
      ...
      const double density = 0.813037037;  // density of particle system; DPD value: 3.0
      ...
      tarch::configuration::ParseConfiguration::parseConfiguration<simplemd::configurations::MolecularDynamicsConfiguration>("test_nie_simplemd.xml","molecular-dynamics",simpleMDConfig);
      if (!simpleMDConfig.isValid()){std::cout << "ERROR NieTest: Invalid SimpleMD config!" << std::endl; exit(EXIT_FAILURE);}
      ...
      if (!mamicoConfig.isValid()){ std::cout << "ERROR NieTest: Invalid MaMiCo config!" << std::endl; exit(EXIT_FAILURE); }
      ...
      //std::cout << "Rank: " << rank << " # total MD instances: " << totalNumberMDSimulations << ", # local MD instances: " << mdInstances << std::endl;
      ...
      for (unsigned int i = 0; i < mdInstances; i++){
        ...
        if (simpleMD[i]==NULL){std::cout << "ERROR NieTest: simpleMD[" << i << "]==NULL!" << std::endl; exit(EXIT_FAILURE);}
      }
      ...
      for (unsigned int i = 0; i < mdInstances; i++){
        ...
        if (i<mdInstances-1){mdStepCounter=0;}
      }
      ...
      for (unsigned int i = 0; i < mdInstances; i++){
        // switch on coupling; in case of LAMMPS: initialse MDSolverInterface and buffer result in MamicoInterfaceProvider; it will be delivered to mdSolverInterface from there
        simpleMD[i]->switchOnCoupling();
        mdSolverInterface.push_back(coupling::interface::SimulationAndInterfaceFactory::getInstance().getMDSolverInterface(simpleMDConfig,mamicoConfig,simpleMD[i]));
        if (mdSolverInterface[i]==NULL){std::cout << "ERROR main_nie: mdSolverInterface[" << i << "]==NULL!" << std::endl; exit(EXIT_FAILURE);}
      }
      coupling::interface::MacroscopicSolverInterface<3>* couetteSolverInterface = getCouetteSolverInterface(
        couetteSolver, simpleMDConfig.getDomainConfiguration().getGlobalDomainOffset(),
        mamicoConfig.getMacroscopicCellConfiguration().getMacroscopicCellSize(),
        getGlobalNumberMacroscopicCells(simpleMDConfig,mamicoConfig),mamicoConfig.getMomentumInsertionConfiguration()._innerOverlap,
        solverType,rank
      );
      ...
      unsigned int *globalCellIndices4SendBuffer = allocateSendBuffer(sendBuffer,multiMDCellService.getMacroscopicCellService(0).getIndexConversion(),rank,*couetteSolverInterface);
      ...
      for (unsigned int cycles = 0; cycles < couplingCycles; cycles++){
        ...
        if(cycles >= initCycles) gettimeofday(&start,NULL);
        ...
        if(cycles >= initCycles){
          gettimeofday(&end,NULL);
          POD_total_time_usec += (end.tv_sec-start.tv_sec)*1000000 + (end.tv_usec-start.tv_usec);
      ...
      std::cout << tws << ", " << 10*log10(real_vx/noise_x) << ", " << POD_total_time_usec/1000 << " ms" << std::endl;
  ...
  private:
    ...
    coupling::interface::MacroscopicSolverInterface<3> &couetteSolverInterface) const {
      ...
      if (indices==NULL){std::cout << "ERROR NieTest::allocateSendBuffer(): indices==NULL!" << std::endl; exit(EXIT_FAILURE); }
      ...
      for (unsigned int i = 0; i < num; i++){
        if (couetteSolverInterface.sendMacroscopicQuantityToMDSolver(indexConversion.getGlobalVectorCellIndex(i))){
           ...
           if (containsThisRank){
             ...
             if (sendBuffer[sendBuffer.size()-1]==NULL){std::cout << "ERROR NieTest::allocateSendBuffer: sendBuffer[" << sendBuffer.size()-1 << "]==NULL!" << std::endl; exit(EXIT_FAILURE);}
    ...
    coupling::interface::MacroscopicSolverInterface<3> &couetteSolverInterface) const {
      ...
      if (indices==NULL){std::cout << "ERROR NieTest::allocateRecvBuffer(): indices==NULL!" << std::endl; exit(EXIT_FAILURE); }
      ...
      for (unsigned int i = 0; i < num; i++){
        if (couetteSolverInterface.receiveMacroscopicQuantityFromMDSolver(indexConversion.getGlobalVectorCellIndex(i))){
          ...
          if (containsThisRank){
            ...
            if (recvBuffer[recvBuffer.size()-1]==NULL){std::cout << "ERROR NieTest::allocateRecvBuffer: recvBuffer[" << recvBuffer.size()-1 << "]==NULL!" << std::endl; exit(EXIT_FAILURE);}
    ...
    void write2CSV(
      ...
      if (!file.is_open()){std::cout << "ERROR NieTest::write2CSV(): Could not open file " << ss.str() << "!" << std::endl; exit(EXIT_FAILURE);}
    ...
    coupling::solvers::AbstractCouetteSolver<3>* getCouetteSolver(
      const double channelheight, tarch::la::Vector<3,double> wallVelocity, const double kinVisc, const double dx, const double dt,
      ...
      if     (solverType==COUETTE_ANALYTICAL){
        if (rank==0){
          ...
          if (solver==NULL){std::cout << "ERROR NieTest::getCouetteSolver(): Analytic solver==NULL!" << std::endl; exit(EXIT_FAILURE);}
        }
      // LB solver: active on "processes"
        ...
        if (solver==NULL){std::cout << "ERROR NieTest::getCouetteSolver(): LB solver==NULL!" << std::endl;exit(EXIT_FAILURE);}
      }
      else {std::cout << "ERROR NieTest::getCouetteSolver(): Unknown solver type!" << std::endl; exit(EXIT_FAILURE); return NULL;}
      return solver;
    }
    ...
    coupling::interface::MacroscopicSolverInterface<3>* getCouetteSolverInterface(
      ...
      if (solverType == COUETTE_ANALYTICAL){
        ...
        if (lbSolver==NULL){std::cout << "ERROR NieTest::getCouetteSolverInterface(...), rank=" << rank << ": Could not convert abstract to LB solver!" << std::endl; exit(EXIT_FAILURE);}
        // compute number of cells of MD offset; detect any mismatches!
        tarch::la::Vector<3,unsigned int> offsetMDDomain(0);
        for (unsigned int d = 0; d < 3; d++){
          if (mdOffset[d] < 0.0){std::cout << "ERROR NieTest::getCouetteSolverInterface(...): mdOffset[" << d << "]<0.0!" << std::endl; exit(EXIT_FAILURE);}
          ...
          if (fabs( (offsetMDDomain[d]*mamicoMeshsize[d] - mdOffset[d])/mamicoMeshsize[d])>1.0e-8){
            std::cout << "ERROR NieTest::getCouetteSolverInterface: MD offset and mesh size mismatch!" << std::endl;
            exit(EXIT_FAILURE);
          }
        }
      ...
      if (interface==NULL){std::cout << "ERROR NieTest::getCouetteSolverInterface(...), rank=" << rank << ": interface==NULL!" << std::endl; exit(EXIT_FAILURE);}
      return interface;
    }
