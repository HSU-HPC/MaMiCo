  ...
  virtual void run() {
    for (int tws = 10; tws <= 100; tws += 10) {
      ...
      couetteSolver = getCouetteSolver(
          ...
          simpleMDConfig.getSimulationConfiguration().getDt() * simpleMDConfig
      ...
      for (unsigned int cycles = 0; cycles < couplingCycles; cycles++) {
        ...
        if (cycles >= initCycles) {
          gettimeofday(&end, NULL);
          POD_total_time_usec += (end.tv_sec - start.tv_sec) * 1000000 +
                                 (end.tv_usec - start.tv_usec);
      ...
      std::cout << tws << ", " << 10 * log10(real_vx / noise_x) << ", "
                << POD_total_time_usec / 1000 << " ms" << std::endl;
