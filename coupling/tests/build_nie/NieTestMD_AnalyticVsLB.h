...
public:
  ...
  virtual void run() {
    ...
    const unsigned int couplingCycles =
        105; // number of coupling cycles, that is continuum time steps; MD/DPD:
             // 1000
    ...
    couetteSolver = getCouetteSolver(
        channelheight, tarch::la::Vector<3, double>(0.0, 0.0, 0.0), kinVisc,
    ...
    for (unsigned int cycles = 0; cycles < couplingCycles; cycles++) {
      ...
      fillRecvBuffer(
          ...
          multiMDCellService.getMacroscopicCellService(0).getIndexConversion(),
          recvBuffer, globalCellIndices4RecvBuffer, generator);
