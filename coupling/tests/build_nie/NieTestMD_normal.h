...
public:
  ...
  virtual void run() {
    ...
    const int plotEveryTimestep =
        50;       // only for LB couette solver: VTK plotting per time step
    ...
    const unsigned int couplingCycles =
        401; // number of coupling cycles, that is continuum time steps; MD/DPD:
             // 1000
    ...
    for (unsigned int cycles = 0; cycles < couplingCycles; cycles++) {
      ...
      if ((solverType == COUETTE_LB) && twoWayCoupling &&
          cycles == twoWayCouplingInitCycles) {
        static_cast<coupling::solvers::LBCouetteSolver *>(couetteSolver)
            ->setMDBoundary(
                simpleMDConfig.getDomainConfiguration().getGlobalDomainOffset(),
...
private:
  ...
  void fillRecvBuffer(
    ...
    for (unsigned int i = 0; i < size; i++) {
      ...
      // compute momentum
      const tarch::la::Vector<3, double> noise(distribution(generator),
