...
public:
  ...
  virtual void run() {
    ...
    const double kinVisc = 0.54 / density; // kinematic viscosity (cf. paper by Nie et al.); DPD
                                           // value: 0.54/density
    const double temp = 1.0;               // temperature of MD system; DPD value: 1.0
    const int equSteps = 100;              // number of equilibration steps (we start from a particle
    ...
    const tarch::la::Vector<3, double> wallVelocity(1.5, 0.0, 0.0); // velocity of moving wall (lower boundary moves);
    ...
    const unsigned int couplingCycles = 10;           // number of coupling cycles, that is continuum time steps; MD/DPD:
                                                      // 1000
    const unsigned int totalNumberMDSimulations = 10; // total number of MD simulations; MD/DPD: 64 (DPD), 128 (MD
                                                      // scalability), 144 (MD)
    ...
    coupling::services::MultiMDCellService<MY_LINKEDCELL, 3> multiMDCellService(
        ...
        mamicoConfig.getTransferStrategyConfiguration(), mamicoConfig.getParallelTopologyConfiguration(),
        simpleMDConfig.getSimulationConfiguration().getNumberOfTimesteps(), mamicoConfig.getMacroscopicCellConfiguration());
