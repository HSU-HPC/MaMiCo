...
public:
  ...
  virtual void run() {
    ...
    const double kinVisc =
        0.54 / density; // kinematic viscosity (cf. paper by Nie et al.); DPD
                        // value: 0.54/density
    const double temp = 1.0;    // temperature of MD system; DPD value: 1.0
    const int equSteps =
        100; // number of equilibration steps (we start from a particle
             // configuration where all molecules are placed on a grid); DPD
             // value: 1000, MD value: 10000
    unsigned int mdStepCounter = 0; // time step counter for MD

    const double channelheight =
        50.0; // channel is always expected to have origin at (0.0,0.0,0.0) and
              // to be cubic (MD 30: 50.0, MD 60: 100.0, MD 120: 200.0)
    const tarch::la::Vector<3, double> wallVelocity(
        1.5, 0.0, 0.0); // velocity of moving wall (lower boundary moves);
    ...
    const unsigned int couplingCycles =
        10; // number of coupling cycles, that is continuum time steps; MD/DPD:
            // 1000
    const unsigned int totalNumberMDSimulations =
        10; //total number of MD simulations; MD/DPD: 64 (DPD), 128 (MD
            //scalability), 144 (MD)
    const SolverType solverType =
        COUETTE_ANALYTICAL; // LB or analytical couette solver
    ...
    const unsigned int mdInstances = multiMDService
        .getLocalNumberOfMDSimulations(); // local number of MD simulations
    ...
    for (unsigned int i = 0; i < mdInstances; i++) {
      ...
      mdSolverInterface.push_back(
          coupling::interface::SimulationAndInterfaceFactory::getInstance()
              .getMDSolverInterface(simpleMDConfig, mamicoConfig, simpleMD[i]));
    ...
    for (unsigned int cycles = 0; cycles < couplingCycles; cycles++) {
      ...
      fillSendBuffer(
          density, *couetteSolver,
          multiMDCellService.getMacroscopicCellService(0).getIndexConversion(),
          sendBuffer, globalCellIndices4SendBuffer);
      multiMDCellService.sendFromMacro2MD(sendBuffer,
                                          globalCellIndices4SendBuffer);
      // run MD instances
      for (unsigned int i = 0; i < mdInstances; i++) {
        ...
        if (i < mdInstances - 1) {
          mdStepCounter -= simpleMDConfig.getSimulationConfiguration()
              .getNumberOfTimesteps();
        }

        // plot macroscopic time step info in multi md service
        multiMDCellService.getMacroscopicCellService(i)
            .plotEveryMacroscopicTimestep(cycles);
      }
      // send back data from MD instances and merge it
      multiMDCellService.sendFromMD2Macro(recvBuffer,
                                          globalCellIndices4RecvBuffer);

      if ((solverType == COUETTE_LB) && twoWayCoupling) {
        static_cast<coupling::solvers::LBCouetteSolver *>(couetteSolver)
            ->setMDBoundaryValues(recvBuffer, globalCellIndices4RecvBuffer,
                                  multiMDCellService.getMacroscopicCellService(
                                      0).getIndexConversion());
      }
      // write data to csv-compatible file for evaluation
      write2CSV(
          recvBuffer, globalCellIndices4RecvBuffer,
          multiMDCellService.getMacroscopicCellService(0).getIndexConversion(),
          rank, cycles);
      if (rank == 0) {
    ...
    if (rank == 0) {
      ...
      std::cout << "NieTest-Coupling: " << (int)(runtime / 1000) << "ms for "
                << couplingCycles << " coupling cycles and "
                << totalNumberMDSimulations << " MD simulations" << std::endl;
      gettimeofday(&start, NULL);
    }
    ...
    for (unsigned int i = 0; i < mdInstances; i++) {
      ...
      delete simpleMD[i];
      simpleMD[i] = NULL;
      mdSolverInterface[i] = coupling::interface::MamicoInterfaceProvider<
    ...
    if (rank == 0) {
      ...
      double runtime =
          (end.tv_sec - start.tv_sec) * 1000000 + (end.tv_usec - start.tv_usec);
      std::cout << "NieTest-Shutdown: " << (int)(runtime / 1000) << "ms"
                << std::endl;
    }
...
private:
  ...
  coupling::interface::MacroscopicSolverInterface<3> *getCouetteSolverInterface(
    ...
    if (solverType == COUETTE_ANALYTICAL) {
      ...
      if (lbSolver == NULL) {
        std::cout
            << "ERROR NieTest::getCouetteSolverInterface(...), rank=" << rank
            << ": Could not convert abstract to LB solver!" << std::endl;
        exit(EXIT_FAILURE);
      }
      ...
      for (unsigned int d = 0; d < 3; d++) {
        ...
        if (fabs((offsetMDDomain[d] * mamicoMeshsize[d] - mdOffset[d]) /
          ...
          std::cout << "ERROR NieTest::getCouetteSolverInterface: MD offset "
                       "and mesh size mismatch!" << std::endl;
          exit(EXIT_FAILURE);
        }
      }
