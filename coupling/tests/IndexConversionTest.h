...
/** tests several properties of the IndexConversion. Currently, only XYZ is tested in this scope.
...
class IndexConversionTest: public Test {
  ...
  private:
    void test3D(
      const tarch::la::Vector<3,unsigned int> &testGlobalNumberCells,
      ...
      for (unsigned int xg = 1 ; xg < testGlobalNumberCells[0]; xg++){
        ...
        for (unsigned int xnp = 1; xnp < testNumberProcesses[0]; xnp++){
          ...
          for (unsigned int rank = 0; rank < numberRanks; rank++){
            ...
            const tarch::la::Vector<3,unsigned int> wholeLocalDomain = conversion.getLocalNumberMacroscopicCells() + tarch::la::Vector<3,unsigned int>(2);
    ...
    void test2D(
      const tarch::la::Vector<2,unsigned int> &testGlobalNumberCells,
    ...
    void testUniqueRanks(
      ...
      for (loop[2] = 0; loop[2] < end[2]; loop[2]++){ for (loop[1]=0; loop[1] < end[1]; loop[1]++){ for (loop[0]=0; loop[0]<end[0]; loop[0]++){
        std::cout << "Unique rank for cell " << coupling::initDimVector<dim>(loop) << ": " << indexConversion.getUniqueRankForMacroscopicCell(coupling::initDimVector<dim>(loop)) << std::endl;
      }}}
    }
