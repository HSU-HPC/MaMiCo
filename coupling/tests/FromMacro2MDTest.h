...
private:
  ...
  template <unsigned int dim> void test() {
    ...
    const unsigned int oneDir = (unsigned int)
        floor(pow(((double) size), (1.0 / ((double) dim))) + 0.5);
    ...
    if (rank == 0) {
      std::cout
          << "Global number macroscopic cells: " << globalNumberMacroscopicCells
          << std::endl;
    ...
    TestDataExchangeFromMacro2MD<dim> testDataExchangeFromMacro2MD(
        10, indexConversion);
    ...
    testDataExchangeFromMacro2MD.getBuffer4MacroscopicSolverCells(
        numberSentCells, sentCells, sentGlobalIndices);
    for (unsigned int i = 0; i < numberSentCells; i++) {
      sentCells[i]
          ->setBuffer1(tarch::la::Vector<dim, double>(sentGlobalIndices[i]));
      sentCells[i]->setBuffer2(sentGlobalIndices[i]);
    }
    // INCLUDE THE FOLLOWING LINES FOR BETTER OUTPUT
    sleep(indexConversion.getThisRank());
    std::cout << "Send cells from rank " << indexConversion.getThisRank()
              << ": " << std::endl;
    for (unsigned int i = 0; i < numberSentCells; i++) {
      std::cout
          << "Cell " << sentGlobalIndices[i] << "("
          << indexConversion.getGlobalVectorCellIndex(sentGlobalIndices[i])
          << "): " << sentCells[i]->getBuffer1() << " , "
          << sentCells[i]->getBuffer2() << std::endl;
    }
#if (COUPLING_MD_PARALLEL == COUPLING_MD_YES)
    ...
    fromMacro2MD.sendFromMacro2MD(indexConversion, testDataExchangeFromMacro2MD,
                                  cellsMamico, sentCells, sentGlobalIndices);
      ...
      if (cellsMamico[i]->getBuffer2() > 0.0) {
        std::cout << "Cell " << indexConversion.convertLocalToGlobalCellIndex(i)
                  ...
                  << indexConversion.getGlobalVectorCellIndex(
                         indexConversion.convertLocalToGlobalCellIndex(i))
                  << "): " << cellsMamico[i]->getBuffer1() << " , "
