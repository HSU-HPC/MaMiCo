...
public:
  ...
  MacroscopicCellPlotter(
      unsigned int ID, std::string filename, unsigned int rank, unsigned int t,
    ...
    plotMacroscopicCellFile(ID, filename, rank, t, indexConversion, cells,
                            mdSolverInterface);
  }
...
private:
  ...
  void plotMoleculeFile(
    ...
    Writer4Molecules writer(moleculeVelocities, moleculePositions,
                            moleculePotentials, appendFloatZeros,
                            mdSolverInterface);
    ...
    moleculeFile << "POINTS " << writer.getMoleculeCounter() << " float"
                 << std::endl;
  ...
  void plotMacroscopicCellFile(
    ...
    microscopicMomenta << "VECTORS microscopicMomentumBuffer float"
                       << std::endl;
    std::stringstream macroscopicMomenta;
    macroscopicMomenta << "VECTORS macroscopicMomentumBuffer float"
                       << std::endl;
    ...
    for (unsigned int d = 0; d < dim; d++) {
      original[d] = indexConversion.getGlobalMDDomainOffset()[d] -
                    0.5 * indexConversion.getMacroscopicCellSize()[d] +
                    indexConversion.getAverageLocalNumberMacroscopicCells()[d] *
                        indexConversion.getThisProcess()[d] *
                        indexConversion.getMacroscopicCellSize()[d];
    }
    // open file
    open(ID, filename, "MacroscopicCells", rank, t, ofCell);
    Writer4Cells writer(microscopicMasses, macroscopicMasses,
                        microscopicMomenta, macroscopicMomenta, meanVelocities,
                        masses, energies, temperatures, appendFloatZeros,
                        indexConversion, mdSolverInterface);
    ...
    ofCell << "DIMENSIONS " << numberCells[0] << " " << numberCells[1] << " "
           << numberCells[2] << std::endl;
    ofCell << "ORIGIN ";
    ...
    ofCell << std::endl;
    ofCell << "POINT_DATA " << numberCells[0] * numberCells[1] * numberCells[2]
           << std::endl
           << std::endl;
  ...
  void open(unsigned int ID, std::string filename, std::string fileType,
            unsigned int rank, unsigned int t, std::ofstream &of) {
    std::stringstream ss;
    ss << filename << "_" << fileType << "_" << ID << "_" << rank << "_" << t
       << ".vtk";
    of.open(ss.str().c_str());
    if (!of.is_open()) {
      std::cout
          << "ERROR coupling::MacroscopicCellPlotter: Could not open file "
          << ss.str() << "!" << std::endl;
      exit(EXIT_FAILURE);
    }
  }
  ...
  public:
     ...
     *  @param mdSolverInterface interface of the md solver */
    Writer4Cells(std::stringstream &microscopicMasses,
                 std::stringstream &macroscopicMasses,
                 std::stringstream &microscopicMomenta,
                 std::stringstream &macroscopicMomenta,
                 ...
                 coupling::interface::MDSolverInterface<LinkedCell, dim>
                     *const mdSolverInterface)
        : _microscopicMasses(microscopicMasses),
          _macroscopicMasses(macroscopicMasses),
          _microscopicMomenta(microscopicMomenta),
          _macroscopicMomenta(macroscopicMomenta),
          _meanVelocities(meanVelocities), _masses(masses), _energies(energies),
          _temperatures(temperatures), _appendFloatZeros(appendFloatZeros),
    ...
    void
    apply(coupling::datastructures::MacroscopicCellWithLinkedCells<LinkedCell,
                                                                   dim> &cell,
          const unsigned int &index) {
      // compute local quantities from molecules, if this is an inner cell
      const tarch::la::Vector<dim, unsigned int> end =
          _indexConversion.getLocalNumberMacroscopicCells() +
          tarch::la::Vector<dim, unsigned int>(1);
      const tarch::la::Vector<dim, unsigned int> localIndex =
          _indexConversion.getLocalVectorCellIndex(index);
      bool isInnerCell = true;
      for (unsigned int d = 0; d < dim; d++) {
        isInnerCell =
            isInnerCell && (localIndex[d] > 0) && (localIndex[d] < end[d]);
      }
      double mass = 0.0;
  ...
  public:
     ...
     * or 2d
     *  @param mdSolverInterface interface of the md solver */
    Writer4Molecules(std::stringstream &moleculeVelocities,
                     std::stringstream &moleculePositions,
                     std::stringstream &moleculePotentials,
                     const std::string &appendFloatZeros,
                     coupling::interface::MDSolverInterface<LinkedCell, dim>
        ...
        : _moleculeVelocities(moleculeVelocities),
          ...
          _mdSolverInterface(mdSolverInterface), _moleculeCounter(0) {}
    ...
    apply(coupling::datastructures::MacroscopicCellWithLinkedCells<LinkedCell,
                                                                   dim> &cell,
          const unsigned int &index) {
      coupling::cellmappings::VTKMoleculePlotter<LinkedCell, dim>
          vtkMoleculePlotter(_moleculeVelocities, _moleculePositions,
                             _moleculePotentials, _appendFloatZeros,
                             _mdSolverInterface);
