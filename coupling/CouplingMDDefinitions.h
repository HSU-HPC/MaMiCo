...
#endif
// message tags for MPI parallelisation. For each MacroscopicCellService, we add the ID of the cell service to the tags from below to yield a unique tag for each operation.
// this currently supports a maximum of <10M instances of MacroscopicCellService.
...
#include "tarch/la/Vector.h"
  ...
  namespace coupling {
    ...
    tarch::la::Vector<dim,unsigned int> initDimVector(tarch::la::Vector<3,unsigned int> vec){
      ...
      for (unsigned int d = 0; d < dim; d++){smallVec[d] = vec[d];}
      return smallVec;
    }
