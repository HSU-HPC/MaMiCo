...
private:
  ...
  tarch::la::Vector<dim, double> *_oldSolution; // solution of velocity field at the beginning of coupling
  ...
  const double _shiftTimestep;                  // offset by which evaluation of time interval is shifted
  ...
  const tarch::la::Vector<2 * dim, bool> _massFluxBoundary; // true in each entry if west/east, south/north,
