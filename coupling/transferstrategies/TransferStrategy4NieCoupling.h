 ...
 */
template <class LinkedCell, unsigned int dim>
class coupling::transferstrategies::TransferStrategy4NieCoupling
...
public:
  ...
  /** store old microscopic mass in excess-mass buffer and reset microscopic
   * mass buffer */
  ...
  /** converts momentum into velocity values; stores velocity values in new
   * cont.-velocity field solution and sets correct velocity value for first MD
   * time step in
  ...
  /** computes current velocity (linear time interpolation) in this cell and
   * accumulates mass/momentum for sampling */
...
private:
  ...
  unsigned int getLocalNumberMacroscopicCells(
  ...
  double computeMassFlux(const double &mass,
  ...
  tarch::la::Vector<dim, double> *
      _oldSolution; // solution of velocity field at the beginning of coupling
                    // cycle (=end of old coupling cycle)
  tarch::la::Vector<dim, double> *
      _newSolution;   // solution of velocity field at the end of coupling cycle
  const unsigned int
      _numberMDSteps; // number of MD time steps per coupling cycle
  const double
      _shiftTimestep; // offset by which evaluation of time interval is shifted
                      // compared to continuum solver. A value of zero implies
                      // interpolation between new and old solution,
                      // corresponding to a time interval t->t+dt.
  // A value of one implies extrapolation between the times t+dt (newSolution)
  // and t+2dt using oldSolution and newSolution. All inter-/extrapolations are
  // linear.
  const unsigned int
      _numberLocalCells; // local number of cells (incl. ghost layers)
  unsigned int _timestepCounter; // time step counter within a coupling cycle
                                 // (should run from 0 to _numberMDSteps)
  double *_excessMass;   // mass that was transferred in an earlier coupled step
  const tarch::la::Vector<2 * dim, bool>
      _massFluxBoundary; // true in each entry if west/east, south/north,
                         // bottom/top boundary is a mass flux boundary
