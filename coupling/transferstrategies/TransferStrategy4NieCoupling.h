...
class coupling::transferstrategies::TransferStrategy4NieCoupling: public coupling::transferstrategies::TransferStrategy<LinkedCell,dim> {
  public:
     ...
     *  @param shiftTimestep parameter for the inter- and extrapolation of the time steps refers to the Nie algorithm
     *  @param massFluxBoundary indicates at which boundary mass shall be transferred (true) or not (false);
     *                          in their order the entries refer to: west, east, south, north, bottom, top boundary  */
    ...
    /** @brief stores the old cont.-velocity field solution and resets time step counter */
    virtual void beginProcessInnerMacroscopicCellsBeforeReceivingMacroscopicSolverData();

    /** @brief store old microscopic mass in excess-mass buffer and reset microscopic mass buffer
    ...
    virtual void processInnerMacroscopicCellBeforeReceivingMacroscopicSolverData(
     ...
     /** stores velocity values in new cont.-velocity field solution and sets correct
      *  velocity value for first MD time step in microscopic momentum buffer.
    ...
    /** @brief computes current velocity (linear time interpolation) in this cell and accumulates mass/momentum for sampling
     *  @param cell the macroscopic cell to be processed
     *  @param index the index of the macroscopic cell */
  ...
  private:
    ...
    /** offset by which evaluation of time interval is shifted compared to continuum solver.
     *  A value of zero implies interpolation between new and old solution, corresponding to
     *  a time interval t->t+dt. A value of one implies extrapolation between the times t+dt
     * (newSolution) and t+2dt using oldSolution and newSolution. All inter-/extrapolations are linear. */
