...
public:
   ...
   *  Afterwards, all values from the send/receive-buffer are added to the
   * mass/momentum buffers of the macroscopic
   *  cells.
   ...
   * momentum have been
   *  transferred from the macroscopic solver to MD, but only the difference
   * between MD and macroscopic
   *  solver shall be introduced to MD. Then, this function determines this
   * difference and stores
   ...
   * need time-averaged
   *  data within a macroscopic cell, we can compute mass and momentum in each
   * timestep and add
