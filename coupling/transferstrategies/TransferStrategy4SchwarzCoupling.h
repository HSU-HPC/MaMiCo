...
public:
  ...
  /** the momentum is converted to velocity (velocity = momentum/mass) and stored in the microscopicMomentum
   *  the mass is not applied, therefore the macroscopicMass is set to 0
  ...
  /** @brief the momentum is converted to velocity (velocity = momentum/mass) and stored in the microscopicMomentum
   *  the mass is not applied, therefore the macroscopicMass is set to 0
   *  the macroscopic quantities are reseted (0)
   *  @brief the data received from the macro solver is processed
   *  @param cell the macroscopic cell to be processed
  ...
  /** @brief the data collected during the md time steps is averaged (/numberMDSteps)
   *  @param cell the macroscopic cell to be processed
   *  @param index the index of the macroscopic cell */
  ...
  /** @brief the _timestepCounter is incremented and if sample()==true the _sampleCounter too */
  virtual void beginProcessInnerMacroscopicCellsAfterMDTimestep();

  /** @brief the mass and momentum are evaluated in the cell and stored in the macroscopic quantities
   *  @param cell the macroscopic cell to be processed
   *  @param index the index of the macroscopic cell */
