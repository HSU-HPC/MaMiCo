...
private:
  void init() {
    ...
    if (!_mdConfig.isValid()) {
      std::cout << "ERROR CouetteScenario: Invalid molecular dynamics config." << std::endl;
      exit(EXIT_FAILURE);
    }
    ...
    // init solvers
    _macroSolver = new coupling::solvers::PreciceAdapter<dim>(_cfg.channelheight, dx, dt, _cfg.plotEveryTimestep, "preCICECouette",
    ...
    if (_instanceHandling == nullptr) {
      std::cout << "ERROR CouetteScenario::init() : _instanceHandling == NULL!" << std::endl;
    ...
    for (unsigned int d = 0; d < 3; d++) {
      if (mdOffset[d] < 0.0) {
        std::cout << "ERROR CouetteScenario::getMacroSolverInterface(...): mdOffset[" << d << "]<0.0!" << std::endl;
        exit(EXIT_FAILURE);
      }
    ...
    if (macroSolverInterface == NULL) {
      std::cout << "ERROR CouetteScenario: rank=" << _rank << ": macroSolverInterface==NULL!" << std::endl;
      exit(EXIT_FAILURE);
    }
  ...
  void write2CSV(std::vector<coupling::datastructures::MacroscopicCell<3>*>& recvBuffer, const unsigned int* const recvIndices,
    ...
    if (recvBuffer.size() == 0)
      return;
    if (couplingCycle % _scenarioConfig.csvEveryTimestep > 0)
      return;
    std::stringstream ss;
  ...
  void allocateCFDToMDBuffer(const coupling::IndexConversion<3>& indexConversion, coupling::interface::MacroscopicSolverInterface<3>& macroSolverInterface) {
    ...
    if (indices == NULL) {
      std::cout << "ERROR CouetteScenario::allocateCFDToMDBuffer(): indices==NULL!" << std::endl;
      exit(EXIT_FAILURE);
    }
    for (unsigned int i = 0; i < num; i++) {
      ...
      if (_buf.CFDToMDBuffer[_buf.CFDToMDBuffer.size() - 1] == NULL) {
        std::cout << "ERROR CouetteScenario::allocateCFDToMDBuffer: CFDToMDBuffer[" << _buf.CFDToMDBuffer.size() - 1 << "]==NULL!" << std::endl;
        exit(EXIT_FAILURE);
      }
    }
  ...
  void allocateMDToCFDBuffer(const coupling::IndexConversion<3>& indexConversion, coupling::interface::MacroscopicSolverInterface<3>& macroSolverInterface) {
    ...
    if (indices == NULL) {
      std::cout << "ERROR CouetteScenario::allocateMDToCFDBuffer(): indices==NULL!" << std::endl;
      exit(EXIT_FAILURE);
    }
    for (unsigned int i = 0; i < num; i++) {
      ...
      if (_buf.MDToCFDBuffer[_buf.MDToCFDBuffer.size() - 1] == NULL) {
        std::cout << "ERROR CouetteScenario::allocateMDToCFDBuffer: MDToCFDBuffer[" << _buf.MDToCFDBuffer.size() - 1 << "]==NULL!" << std::endl;
        exit(EXIT_FAILURE);
      }
    }
