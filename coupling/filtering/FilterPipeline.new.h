...
template<unsigned int dim>//template für LinkedCell?
...
class coupling::rilterPipeline{
    public:
        ...
        void apply(coupling::services::MacroscopicCellServiceImpl<dim> macro_service);

        template<class LinkedCell>
        void apply(coupling::services::MultiMDCellServiceImpl<LinkedCell, dim> multimd_service);
...
/**
 * TODO:
 * std::vector von Filtern, InitializeFilters methode
 * WriteToFile
 *
 * Fragen:
 * Warum sind cell indices const unsigned int* const und nicht einfach ein std::vector<unsigned int>?
 * Sollen Konstruktoren überhaupt in den Header?
 * */
