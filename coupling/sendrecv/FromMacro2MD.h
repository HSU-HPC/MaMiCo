 ...
 */
template <class MacroscopicCell, unsigned int dim> class coupling::sendrecv::FromMacro2MD : public coupling::sendrecv::SendReceiveBuffer<MacroscopicCell, dim> {
...
to the macroscopic
     *  cells of MaMico (ghost cells are also allowed). Since the macroscopic
solver can have an arbitrary distribution of cells on the processes,
     *  the buffer for sending the cell data is provided to this function in
terms of an array of macroscopic cells including the
     *  respective global cell indices.
     *  This function calls sendFromMacro2MDNonBlocking(..) and immediately
subsequently wait4SendFromMacro2MD(...). Those two methods may alternatively be
used, e.g., for "non-blocking" communication.
...
wait4SendFromMacro2MD(...) to guarantee
...
sendFromMacro2MDNonBlocking(..)--to be finished and fills the information into
the macroscopic cells from Mamico.
         * 	@param indexConversion
         * 	@param dataExchange
         * 	@param macroscopicCellsFromMamico
         */
  void wait4SendFromMacro2MD(const coupling::IndexConversion<dim> &indexConversion, coupling::sendrecv::DataExchange<MacroscopicCell, dim> &dataExchange,
                             const std::vector<MacroscopicCell *> &macroscopicCellsFromMamico);

private:
  /** given a list of macroscopic cells (from the macroscopic solver), the data
from these cells are written
     *  to the send buffer.
         * 	@param indexConversion
         * 	@param dataExchange
...
obtain data on the side of MaMiCo,
     *  we can just loop over all local macroscopic cells of the coupling tool
and call allocateReceiveBuffers(...) of the
...
macroscopic cells. Since this is a receive
     *  for the macroscopic cells on the side of the coupling tool, we can
consider a Cartesian grid topology for the
     *  macroscopic cells. For each cell, readFromReceiveBuffer(...) of
