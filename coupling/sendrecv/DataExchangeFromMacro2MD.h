...
namespace coupling {
  namespace sendrecv{
    ...
    class DataExchangeFromMacro2MD;
  }
}


/** data exchange from the macroscopic solver to the MD solver, that is to the coupling tool. We transfer the buffers
 *  microscopicMass and microscopicMomentum of the MacroscopicCell.
 *  The target ranks are determined by the getRanksForMacroscopicCell() method of IndexConversion: since macroscopic cells
 *  may exist on different ranks (due to ghost layers), this method determines all ranks with a particular macroscopic cell
 *  and returns a vector with all required ranks (see also documentation of IndexConversion). The source ranks
 *  are determined via the macroscopic solver interface's method getRanks()
 *	@brief data exchange from the macroscopic solver to the MD solver. Derived from the class coupling::sendrecv::DataExchange
 *	@tparam dim Number of dimensions; it can be 1, 2 or 3
 *	@sa  DataExchangeFromMD2Macro
 *  @author Philipp Neumann
 */
template<unsigned int dim>
class coupling::sendrecv::DataExchangeFromMacro2MD:
public coupling::sendrecv::DataExchange<coupling::datastructures::MacroscopicCell<dim>,dim> {
  ...
  public:
    ...
    coupling::sendrecv::DataExchange<coupling::datastructures::MacroscopicCell<dim>,dim>(TAG_FROM_MACRO2MD+tagoffset),_interface(interface), _indexConversion(indexConversion)
    {
	 ...
	 *	@return the corresponding ranks via IndexConversion, if we need information on MD side, otherwise empty vector
	 */
    virtual std::vector<unsigned int> getTargetRanks(tarch::la::Vector<dim,unsigned int> globalCellIndex){
      // if we need information on MD side, return the respective ranks via IndexConversion
	 ...
	 * 	@param  globalCellIndex 
	 *	@return the corresponding ranks via MacroscopicSolverInterface, if we need information on MD side, otherwise empty vector
	 */
    ...
    virtual void readFromCell(double * const buffer, const coupling::datastructures::MacroscopicCell<dim> &cell){
      ...
      for (unsigned int d = 0; d < dim; d++){ buffer[d+1] = cell.getMicroscopicMomentum()[d]; }
    }
    ...
    virtual void writeToCell(const double * const buffer, coupling::datastructures::MacroscopicCell<dim>& cell){
      ...
      for (unsigned int d = 0; d < dim; d++){ microscopicMomentum[d] = buffer[1+d]; }
      cell.setMicroscopicMomentum(microscopicMomentum);
      cell.setMicroscopicMass(buffer[0]);
    }


    /** returns the number of doubles that are sent per macroscopic cell. @return 1+dim  */
