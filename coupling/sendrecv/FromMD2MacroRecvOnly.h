 ...
 * a macroscopic solver process.
 *  This is used for multi-MD simulations where the macroscopic solver process
 * may run on a completely different process than a single MD simulation.
 *	@brief sends macroscopic cell information from MaMiCo to the macroscopic
 * solver. Expects to only receive data from, but will not send any data.
 * Derived from the class coupling::sendrecv::SendReceiveBuffer
 *	@tparam MacroscopicCell cell type
 *	@tparam dim Number of dimensions; it can be 1, 2 or 3
 *  @author Philipp Neumann
 */
template <class MacroscopicCell, unsigned int dim>
class coupling::sendrecv::FromMD2MacroRecvOnly
...
non-ghost cells of this process) to the macroscopic
     *  solver. Since the macroscopic solver can have an arbitrary distribution
of cells on the processes,
     *  the buffer for receiving the cell data is provided to this function in
terms of an array of macroscopic cells including the
     *  respective global cell indices.
     *  Basically calls sendFromMD2MacroNonBlocking(...) and
...
know anything about the macroscopic solver,
     *  we only have a list of global vector cell indices available for possible
cells to be received on this rank.
     *  For each global cell which is in the list, allocateReceiveBuffers(...)
of SendReceiveBuffer is called.
	 * 	@param indexConversion
	 * 	@param dataExchange
...
list of macroscopic cells. Since this is a receive
     *  for the macroscopic cells on the side of the macroscopic solver, we just
have a list of global cell indices and corresponding
     *  macrocsopic cell buffers. For each cell in this list,
readFromReceiveBuffer(...) of SendReceiveBuffer is called.
	 * 	@param indexConversion
	 * 	@param dataExchange
