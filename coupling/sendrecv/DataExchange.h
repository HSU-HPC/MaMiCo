...
/** This class holds information on the data exchange between processes. In contrast to the SendReceiveBuffer which solely
 *  handles the technical part of sending and receiving, this class needs to guarantee the validity in communication, e.g.
...
class coupling::sendrecv::DataExchange {
  public:
    ...
    /** returns the tag associated to messages for this DataExchange. @return _tag*/
     ...
     *  This method should globally define the target ranks for each global cell index of a macroscopic cell.
     ...
     *  This method should globally define the source ranks for each global cell index of a macroscopic cell.
	 * @param globalCellIndex unique global cell index
     */
    virtual std::vector<unsigned int> getSourceRanks(tarch::la::Vector<dim,unsigned int> globalCellIndex) = 0;


    /** local rule to read from a macroscopic cell and write data to (e.g. send) buffer 
	 * 	@param buffer 
	 * 	@param cell 
	 */
