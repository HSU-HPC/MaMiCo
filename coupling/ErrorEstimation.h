...
class coupling::error::ErrorEstimation {
  public:
	 ...
	 * 	@param cellVolume
	 */
    ErrorEstimation(double velocity,double temperature,
			double numberOfParticle, double particleMass,
			double soundSpeed, int numberOfSamples, double cellVolume):
			_velocity(velocity), _temperature(temperature),
			_numberOfParticle(numberOfParticle), _particleMass(particleMass),
			_soundSpeed(soundSpeed), _numberOfSamples(numberOfSamples),_cellVolume(cellVolume),
			_desiredAbsErrorVelocity(0.08), _desiredRelErrorVelocity(0.1),
			_desiredAbsErrorDensity(0.05),  _desiredRelErrorDensity(0.05),
			_desiredAbsErrorTemperature(0.05),  _desiredRelErrorTemperature(0.05),
			_desiredAbsErrorPerssure(0.05),  _desiredRelErrorPerssure(0.05),
			_gamma(1.667), _C_v(1.4973), _k(1){}
	 ...
	 */
	enum errorBaseQuantity{Velocity=0/**< error in Velocity*/, Density=1/**< error in Density*/, Temperature=2/**< error in Temperature*/, Perssure=3/**< error in Perssure*/};
	...
	enum errorType{Relative=0/**< Relative error */, Absolute=1/**< Absolute error */};
	 ...
	 /** This function predict the required number of MD instances to keep the relative or absolute error of the quantites veocty, density, tempetature or pressure based on its arguments under a certain value.
	 ...
	 double getCorrectorNumberOfSamples(errorBaseQuantity BaseQuantity, errorType Type){
        ...
        if (BaseQuantity==Velocity){
			...
			return requiredSamplesV(Type==Relative ? _desiredErrorVelocity : _desiredAbsoluteErrorVelocity, _temperature, _soundSpeed, Type==Relative ? _velocity : 1, _numberOfParticle, _particleMass);

        }else if (BaseQuantity==Density){
			
			return requiredSamplesD(_desiredErrorDensity, _temperature, _soundSpeed, _velocity,  _particleMass, _numberOfParticle);
...
//	    double deltaT = k*temperature*temperature/_C_v/numberOfParticle;
	  ...
	  */
	 double getErrorPressure(int numberOfSamples, double numberOfParticle, double temperature,
							double soundSpeed, double particleMass, double cellVolume, double pressure) {
	  ...
	  */
	 double requiredSamplesV(double desiredError, double temperature, double soundSpeed,
							 double velocity, double numberOfParticle, double particleMass){ 
//	double refSP= referenceSoundSpeed( temperature, particleMass);
//	double Ac = acousticNumber(soundSpeed, refSP);
...
//------------------------- Reference Properties ---------------------------------------------------
	 ...
	 */
	 double referencePressure(double temperature, double numberOfParticle, double cellVolume){return std::sqrt(numberOfParticle*_k*temperature/cellVolume);}
