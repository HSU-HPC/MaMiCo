...
namespace coupling {
namespace error { class ErrorEstimation; }
}
...
public:
  ...
  ErrorEstimation(double velocity, double temperature, double numberOfParticle,
      ...
      : _velocity(velocity), _temperature(temperature),
        ...
        _desiredRelErrorTemperature(0.05), _desiredAbsErrorPressure(0.05),
        _desiredRelErrorPressure(0.05), _gamma(1.667), _C_v(1.4973), _k(1) {}
...
veocty, density, tempetature or pressure based on its arguments.
  ...
  double getError(errorBaseQuantity BaseQuantity, errorType Type) {
    ...
    if (BaseQuantity == Velocity) {
      ...
      error = getErrorDensity(
          _numberOfSamples, _soundSpeed, _temperature,
          Type == Relative ? _numberOfParticle : 1 / _numberOfParticle,
          _particleMass);
...
pressure based on its arguments under a certain value.
  ...
  double getCorrectorNumberOfSamples(errorBaseQuantity BaseQuantity,
    ...
    if (BaseQuantity == Velocity) {
      ...
      return requiredSamplesV(Type == Relative ? _desiredRelErrorVelocity
                                               : _desiredAbsErrorVelocity,
      ...
      return requiredSamplesD(_desiredAbsErrorDensity, _temperature,
                              _soundSpeed, _velocity, _particleMass,
                              _numberOfParticle);
    ...
    std::cout
        << "Error Estimation::getCorrectorNumberOfSamples not a Valid quantity"
        << std::endl;
  ...
  double getErrorVelocity(int numberOfSamples, double velocity,
    ...
    double error = 1 / (velocitySNR(velocity, temperature, numberOfParticle,
                                    particleMass) * std::sqrt(numberOfSamples));
    return error;
  }
  ...
  double getErrorPressure(int numberOfSamples, double numberOfParticle,
    ...
    double error =
        referenceP / pressure * Ac *
        std::sqrt(_gamma / (numberOfParticle * numberOfSamples)) * temperature;
    return error;
  }

  /** This function estimates the number of MD instances required to keep the
error of the veloctiy under _desiredAbsErrorVelocity or _desiredrelErrorVelocity
	  *	 @param desiredError
	  *	 @param temperature
	  *	 @param soundSpeed
	  *	 @param velocity
  ...
  /** This function estimates the number of MD instances required to keep the
error of the density under _desiredAbsErrorDensity or _desiredrelErrorDensity
	  *	 @param desiredError
	  *	 @param temperature
	  *	 @param soundSpeed
	  *	 @param velocity
