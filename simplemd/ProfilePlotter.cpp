...
simplemd::ProfilePlotter::ProfilePlotter(
  const std::vector<simplemd::configurations::ProfilePlotterConfiguration>& configurations,
  ...
  for (unsigned int i = 0; i < _plotters.size(); i++){
    if (_plotters[i] != NULL){ delete _plotters[i]; _plotters[i] = NULL;}
  }
  _plotters.clear();

  for (unsigned int i = 0; i< configurations.size(); i++){
    _plotters.push_back(
      new simplemd::cellmappings::ProfilePlotterMapping(
        parallelTopologyService,
        linkedCellService,
        configurations[i].getWriteEveryTimestep(),configurations[i].getSampleEveryTimestep(),
        configurations[i].getStartAtTimestep(),
        linkedCellVolume,
        localMDSimulation
      )
    );
...
void simplemd::ProfilePlotter::adjustProfilesInParallel(const simplemd::services::ParallelTopologyService &parallelTopologyService){
  ...
  for (unsigned int i = 0; i < size; i++){
    _isActive.push_back(
      parallelTopologyService.globalToLocalRegionOfInterest(
...
void simplemd::ProfilePlotter::accumulateAndPlotInformation(const unsigned int& t){
  ...
  for (unsigned int i = 0; i < size; i++){
    #if (MD_PARALLEL == MD_YES)
      if(!_isActive[i]) continue; // skip profiles which have no intersection with local domain
    #endif
    _plotters[i]->setCurrentTimestep(t);
    _linkedCellService.iterateCells<simplemd::cellmappings::ProfilePlotterMapping>(
      *_plotters[i],
      _startCells[i],
      _ranges[i],
      false
