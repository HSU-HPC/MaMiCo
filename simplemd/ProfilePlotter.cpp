...
simplemd::ProfilePlotter::ProfilePlotter(
  ...
  for (unsigned int i = 0; i < configurations.size(); i++) {
    _plotters.push_back(new simplemd::cellmappings::ProfilePlotterMapping(
        parallelTopologyService, linkedCellService,
        configurations[i].getWriteEveryTimestep(),
        configurations[i].getSampleEveryTimestep(),
        configurations[i].getStartAtTimestep(), linkedCellVolume,
        localMDSimulation));
...
#if (MD_PARALLEL == MD_YES)
void simplemd::ProfilePlotter::adjustProfilesInParallel(
  ...
  for (unsigned int i = 0; i < size; i++) {
    _isActive.push_back(parallelTopologyService.globalToLocalRegionOfInterest(
        _startCells[i], _ranges[i], localStartCell, localRange));
...
void
simplemd::ProfilePlotter::accumulateAndPlotInformation(const unsigned int &t) {
  const unsigned int size = (unsigned int) _plotters.size();
  for (unsigned int i = 0; i < size; i++) {
#if (MD_PARALLEL == MD_YES)
    if (!_isActive[i])
    ...
    _linkedCellService
        .iterateCells<simplemd::cellmappings::ProfilePlotterMapping>(
            *_plotters[i], _startCells[i], _ranges[i], false);
  }
}
