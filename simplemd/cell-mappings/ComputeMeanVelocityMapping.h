...
class simplemd::cellmappings::ComputeMeanVelocityMapping {
  public:
    ComputeMeanVelocityMapping(
      simplemd::services::ParallelTopologyService& parallelTopologyService, const unsigned int &localMDSimulation, const bool & writeToFile = true
    ): _parallelTopologyService(parallelTopologyService),_particleCounter(0),_meanVelocity(0.0),_localMDSimulation(localMDSimulation),_writeToFile(writeToFile){}
    ~ComputeMeanVelocityMapping(){}
    ...
    void endCellIteration(){
      ...
      if (_writeToFile == false) return;

      #if (MD_PARALLEL == MD_YES)
      if(_parallelTopologyService.getProcessCoordinates()==tarch::la::Vector<MD_DIM,unsigned int>(0)) {
      #endif
        std::stringstream ss; ss << "MeanVelocity_" << _localMDSimulation << "_" << _parallelTopologyService.getRank() << ".dat";
