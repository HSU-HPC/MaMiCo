...
namespace simplemd {
namespace cellmappings { class ParallelBoundaryEmptyCellsMapping; }
}

/** shall only be applied to PARALLEL_BOUNDARY ghost cells. All particles of
 * each cell are sent to the
 *  respective neighbour with the inner cell corresponding to these ghost
 * boundary cells.
 *  Afterwards, the list of the ghost cell is cleared and the molecules are
 * deleted from the ghost cell.
