...
simplemd::cellmappings::LennardJonesForceMapping::LennardJonesForceMapping(
...
void simplemd::cellmappings::LennardJonesForceMapping::handleCell(
    const LinkedCell &cell, const unsigned int &cellIndex) const {
  // force buffer
  ...
  for (std::list<Molecule *>::const_iterator m1 = begin; m1 != end; m1++) {
    ...
    while (m2 != end) {
#if (MD_DEBUG == MD_YES)
      std::cout << "Compute force " << (*m1)->getID() << " <-> "
                << (*m2)->getID() << std::endl;
#endif
...
void simplemd::cellmappings::LennardJonesForceMapping::handleCellPair(
    const LinkedCell &cell1, const LinkedCell &cell2,
    const unsigned int &cellIndex1, const unsigned int &cellIndex2) const {
  ...
  for (std::list<Molecule *>::const_iterator m1 = beginCell1; m1 != endCell1;
       m1++) {
    tarch::la::Vector<MD_DIM, double> &force1 = (*m1)->getForce();
    const tarch::la::Vector<MD_DIM, double> &position1 =
        (*m1)->getConstPosition();

    for (std::list<Molecule *>::const_iterator m2 = beginCell2; m2 != endCell2;
         m2++) {
#if (MD_DEBUG == MD_YES)
      std::cout << "Compute force " << (*m1)->getID() << " <-> "
                << (*m2)->getID() << std::endl;
...
simplemd::cellmappings::LennardJonesForceMapping::getLennardJonesForce(
    const tarch::la::Vector<MD_DIM, double> &position1,
  ...
  if (tarch::la::equals(rij2, 0.0, 1e-4)) {
    std::cout << "ERROR "
                 "simplemd::cellmappings::LennardJonesForceMapping::getLennardJ"
                 "onesForce(): Particle positions are identical!" << std::endl;
