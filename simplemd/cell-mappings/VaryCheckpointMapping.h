...
namespace simplemd {
namespace cellmappings { class VaryCheckpointMapping; }
}

class simplemd::cellmappings::VaryCheckpointMapping {
public:
  VaryCheckpointMapping(const double &molecularMass, const double &kB,
                        const double &temperature, const double &sigma,
                        const tarch::la::Vector<MD_DIM, double> &meshSize)
      : _molecularMass(molecularMass), _kB(kB), _temperature(temperature),
        _sigma(sigma), _meshSize(meshSize) {}
  ...
  void handleCell(LinkedCell &cell, const unsigned int &cellIndex) {
    ...
    double stdDeviation =
        std::sqrt(MD_DIM * _kB * _temperature / _molecularMass);

    tarch::la::Vector<MD_DIM, double> randomNumbers(0.0);

    for (std::list<Molecule *>::iterator molecule = cell.begin();
         molecule != cell.end(); molecule++) {
      meanVelocityForCell += (*molecule)->getVelocity();
    }
    meanVelocityForCell = meanVelocityForCell / (double)
                          cell.getConstList().size() / _molecularMass;
