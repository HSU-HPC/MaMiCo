...
_filename(filename), _timestep(0){}


simplemd::moleculemappings::VTKMoleculeWriter::~VTKMoleculeWriter(){}


void simplemd::moleculemappings::VTKMoleculeWriter::setTimestep(const unsigned int &timestep){
  _timestep = timestep;
}
...
void simplemd::moleculemappings::VTKMoleculeWriter::beginMoleculeIteration(){
  ...
  if (!_file.is_open()){
    std::cout << "ERROR simplemd::moleculemappings::VTKMoleculeWriter: Could not open file " << ss.str() << "!" << std::endl;
    exit(EXIT_FAILURE);
  }
  ...
  _id << "SCALARS particleID int" << std::endl;
  _id << "LOOKUP_TABLE default" << std::endl;
}
...
void simplemd::moleculemappings::VTKMoleculeWriter::handleMolecule(Molecule &molecule){
  ...
  _id << molecule.getID() << std::endl;
}



void simplemd::moleculemappings::VTKMoleculeWriter::endMoleculeIteration(){
  _file << _positions.str() << std::endl << std::endl << _velocities.str() << std::endl << std::endl << _forces.str() << std::endl << _fix.str() << std::endl << _id.str() << std::endl<< std::endl;
  _positions.str("");
  _forces.str("");
  _velocities.str("");
  _fix.str("");
