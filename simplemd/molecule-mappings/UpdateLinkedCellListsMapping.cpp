...
void simplemd::moleculemappings::UpdateLinkedCellListsMapping::
...
void simplemd::moleculemappings::UpdateLinkedCellListsMapping::handleMolecule(
    Molecule &molecule) {
  ...
  for (unsigned int d = 0; d < MD_DIM; d++) {
#if (MD_ERROR == MD_YES)
    if ((position[d] < _domainOffset[d] - _meshWidth[d]) ||
        (position[d] > _domainOffset[d] + _domainSize[d] + _meshWidth[d])) {
      std::cout << "ERROR "
                   "simplemd::moleculemappings::UpdateLinkedCellListsMapping::h"
                   "andleMolecule: Position ";
  ...
  for (unsigned int d = 0; d < MD_DIM; d++) {
    ...
    if (index < 0) {
      std::cout
          << "ERROR simplemd::moleculemappings::UpdateLinkedCellListsMapping: "
             "index < 0: index=";
      std::cout << index << std::endl;
      std::cout << "Dimension : " << d << "," << _globalIndexOfFirstCell[d]
                << "," << _localIndexOfFirstCell[d] << std::endl;
      std::cout
      ...
      std::cout << "Position: " << position << ", offset: " << _domainOffset
                << ", meshwidth: " << _meshWidth << std::endl;
      exit(EXIT_FAILURE);
    }
#endif
...
// into the other cell
#if (MD_DEBUG == MD_YES)
  std::cout << "Put molecule " << molecule.getID() << ", pos. "
            << molecule.getConstPosition()
            << ", vel: " << molecule.getConstVelocity()
            << " , force: " << molecule.getConstForceOld() << " into cell "
            << std::endl;
