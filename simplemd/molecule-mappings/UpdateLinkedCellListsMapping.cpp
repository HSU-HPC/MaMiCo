...
void simplemd::moleculemappings::UpdateLinkedCellListsMapping::handleMolecule(Molecule& molecule){
  ...
  for (unsigned int d = 0; d < MD_DIM; d++){
    ...
    if (index < 0){
      ...
      for (unsigned int e = 0; e < d; e++){std::cout << cellVectorIndex[e] << std::endl;}
  ...
  #if (MD_DEBUG==MD_YES)
  std::cout << "Put molecule " << molecule.getID() << ", pos. " << molecule.getConstPosition() << ", vel: " << molecule.getConstVelocity() << " , force: " << molecule.getConstForceOld() << " into cell " << std::endl;
