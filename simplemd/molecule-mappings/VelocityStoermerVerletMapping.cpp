...
tarch::la::Vector<2 * MD_DIM, bool>
simplemd::moleculemappings::VelocityStoermerVerletMapping::
    initReflectingBoundary(const tarch::la::Vector<
        MD_LINKED_CELL_NEIGHBOURS, simplemd::BoundaryType> &boundary) const {
...
// check 6 sides for reflecting boundaries
  ...
  if (boundary[4] == simplemd::REFLECTING_BOUNDARY) {
    ...
    if ((boundary[0] != simplemd::REFLECTING_BOUNDARY) ||
      ...
      std::cout
          << "ERROR "
             "simplemd::moleculemappings::VelocityStoermerVerletMapping::initRe"
             "flectingBoundary: Boundaries 0,1,2,3,5,6,7,8 are not reflecting!"
          << std::endl;
      exit(EXIT_FAILURE);
    }
  }
