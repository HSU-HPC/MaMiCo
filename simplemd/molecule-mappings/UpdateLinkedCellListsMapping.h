 ...
 *  - Once at the startup of the simulation. When the LinkedCellService is initialised,
 *  the molecules need to be sorted into the corresponding cells.
 *  - After each time integration step for the molecules, the cells are emptied and refilled
 *  by this mapping.
 *
...
class simplemd::moleculemappings::UpdateLinkedCellListsMapping {
  public:
    ...
    simplemd::services::LinkedCellService& linkedCellService): _parallelTopologyService(parallelTopologyService),_linkedCellService(linkedCellService){}
