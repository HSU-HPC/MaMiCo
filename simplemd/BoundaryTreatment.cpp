  ...
  for (unsigned int d = 0; d < MD_DIM; d++) {
    if (localNumberOfCells[d] < 3) {
      std::cout << "Fatal Error: "
                   "BoundaryTreatment::putBoundaryParticlesToInnerCellsFillBoun"
                   "daryCellsAndOverlapWithForceComputations: " << std::endl;
      std::cout
          ...
          << std::endl;
      std::cout
          << "Current local number of linked cells: " << localNumberOfCells
          << std::endl;
      exit(EXIT_FAILURE);
    }
  }
...
#if (MD_PARALLEL == MD_YES)
#if (MD_DEBUG == MD_YES)
  std::cout << "BoundaryTreatment::putBoundaryParticlesToInnerCellsFillBoundary"
               "CellsAndOverlapWithForceComputations: issuing send and receive "
               "calls on communication buffers." << std::endl;
#endif
  parallelTopologyService.communicationSteps_1_2();
#endif
...
#if (MD_DEBUG == MD_YES)
  std::cout << "BoundaryTreatment::putBoundaryParticlesToInnerCellsFillBoundary"
               "CellsAndOverlapWithForceComputations: unpacking local buffer"
            << std::endl;
#endif
  parallelTopologyService.unpackLocalBuffer(_moleculeService,
                                            _linkedCellService);
...
#if (MD_DEBUG == MD_YES)
  std::cout << "BoundaryTreatment::putBoundaryParticlesToInnerCellsFillBoundary"
               "CellsAndOverlapWithForceComputations: begin waiting for buffer "
               "requests to be fulfilled." << std::endl;
#endif
  parallelTopologyService.communicationSteps_3_4(_moleculeService,
                                                 _linkedCellService);
#endif
