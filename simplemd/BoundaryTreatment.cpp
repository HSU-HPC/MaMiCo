  ...
  const bool& useOpenMP)
{
  ...
  for(unsigned int d = 0; d < MD_DIM; d++) {
    if(localNumberOfCells[d] < 3) {
      ...
      std::cout << "in order for overlapping process communications with computations algorithm to work, there have to be at least 3 local linked cells along every dimension on every processor" << std::endl;
  ...
  #if (MD_PARALLEL==MD_YES)
    #if (MD_DEBUG==MD_YES)
      std::cout << "BoundaryTreatment::putBoundaryParticlesToInnerCellsFillBoundaryCellsAndOverlapWithForceComputations: issuing send and receive calls on communication buffers." << std::endl;
    #endif
    parallelTopologyService.communicationSteps_1_2();
  #endif
  ...
  #if (MD_PARALLEL==MD_YES)
    #if (MD_DEBUG==MD_YES)
      std::cout << "BoundaryTreatment::putBoundaryParticlesToInnerCellsFillBoundaryCellsAndOverlapWithForceComputations: begin waiting for buffer requests to be fulfilled." << std::endl;
    #endif
    parallelTopologyService.communicationSteps_3_4(_moleculeService,_linkedCellService);
  #endif
