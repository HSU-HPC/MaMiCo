...
class simplemd::BoundaryTreatment {
  public:
    ...
    _periodicBoundaryMapping(parallelTopologyService,moleculeService,linkedCellService),
    ...
    #endif
    _fillCellsMapping(parallelTopologyService,moleculeService,linkedCellService),
  ...
  private:
     ...
     */
    template<class Mapping>
    void applyMappingToOutermostNonBoundaryCells(
      const bool useOpenMP, Mapping& myMapping
    ) const;
     ...
     */
    template<class Mapping>
    void applyMappingToCommunicationIndependentCells(
      const bool useOpenMP, Mapping& myMapping
    ) const;
     ...
     */
    template<class Mapping>
    void applyMappingToCommunicationDependentCells(
      const bool useOpenMP, Mapping& myMapping
    ) const;
...
void simplemd::BoundaryTreatment::applyMappingToBoundaryCells(
  ...
  if (boundary[25] == boundaryType){
    ...
    numberCellsOuter = tarch::la::Vector<MD_DIM,unsigned int>(1);
    startOuter = _linkedCellService.getLocalNumberOfCells() + 2u*_linkedCellService.getLocalIndexOfFirstCell()
