...
simplemd::services::MoleculeService::MoleculeService(
  ...
  if (_numberMolecules < 1){
     std::cout << "ERROR simplemd::services::MoleculeService::MoleculeService(): _numberMolecules < 1!" << std::endl; exit(EXIT_FAILURE);
  }
    ...
    for (unsigned int y = 0; y < moleculesPerDirection[1]; y++){
      ...
      for (unsigned int x = 0; x < moleculesPerDirection[0]; x++){
        ...
        if (myNewMolecule == NULL){
          std::cout << "ERROR simplemd::services::MoleculeService::MoleculeService(): myNewMolecule==NULL!" << std::endl; exit(EXIT_FAILURE);
        }
        #endif
  ...
  // determine the number of free molecule positions in last block and set number of molecules to the real value
...
simplemd::services::MoleculeService::MoleculeService(
  ...
  numberBlocks = _numberMolecules/_blockSize + (_numberMolecules%_blockSize!=0);
  if (_numberMolecules < 1){
    std::cout << "ERROR MoleculeService::MoleculeService: _numberMolecules < 1!" << std::endl; exit(EXIT_FAILURE);
  }
...
simplemd::services::MoleculeService::MoleculeService(
  ...
  for (unsigned int i = 0; i < _numberMolecules; i++){
    ...
    // check if molecule is inside this local domain and if so: add it to dynamic vectors
    bool isInside = true;
    ...
    if (isInside){ positions.push_back(position); velocities.push_back(velocity); forces.push_back(force); }
  }
  file.close();
...
void simplemd::services::MoleculeService::
getInitialVelocity(
  const tarch::la::Vector<MD_DIM,double>& meanVelocity,
  const double &kB,
  const double& temperature,
  ...
  double stdDeviation = std::sqrt(MD_DIM*
   kB*temperature/molecularPropertiesService.getMolecularProperties().getMass()
  );
...
const unsigned int& simplemd::services::MoleculeService::getNumberMolecules() const {
