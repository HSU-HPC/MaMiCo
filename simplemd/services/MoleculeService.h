...
public:
   ...
   * Cartesian grid with moleculesPerDirection
   *  molecules in each spatial direction within the domain described by
   * domainSize and domainOffset.
   *  meanVelocity describes a mean flow velocity, temperature a temperature
   * (controlling fluctuations).
   *  In addition, the numberMoleculesPerAllocation can be set: In case that
   * molecules are added to the system, we need to
   ...
   * simulation, this method parses
   *  checkpoint files for each rank, respectively. If multiple MD simulations
   * are executed, make sure that the rank of the current
   ...
   * const. reference to a free position within the memory field
   *  or - in case no memory is available - new memory is allocated and the
   * molecule is put in there. Besides, the list _freeMoleculePositions
   *  is adapted, accordingly (and _numberMolecules is incremented).
   *  The function returns a pointer to the new molecule. If something goes
   ...
   * number of
   *  molecule deletions/ insertions, e.g., this might be useful to speed up the
   * simulation.
   *  Besides, the molecules are stored in memory such that those molecules
   * belonging to the same linked cell are
   *  located very closely in memory (actually, we sort the molecules according
   * to their linked cell position, that is
   *  lexicographically w.r.t. to the linked cell index, and store them in this
   * sequence).
   */
  void reorganiseMemory(
      const simplemd::services::ParallelTopologyService &
          parallelTopologyService,
   ...
   * force_old.
   *  In parallel cases, each process writes its own checkpoint data. The file
   * will be named
...
template <class A>
void
simplemd::services::MoleculeService::iterateMolecules(A &a,
