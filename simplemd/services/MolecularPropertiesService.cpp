...
simplemd::services::MolecularPropertiesService::MolecularPropertiesService(
  ...
  std::cout << " sigma: " << _properties.getSigma() << " cut-off rad. " << std::endl;
  std::cout << _properties.getCutOffRadius() << " kB: " << _properties.getKB() << std::endl;
  #endif
}


const simplemd::MolecularProperties& simplemd::services::MolecularPropertiesService::getMolecularProperties() const {
  return _properties;
}
