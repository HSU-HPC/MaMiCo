...
simplemd::services::LinkedCellService::LinkedCellService(
    ...
    : _cells(NULL), _domainSize(domainSize), _domainOffset(domainOffset),
      _meshWidth(getMeshwidth(domainSize,
                              parallelTopologyService.getLocalNumberOfCells())),
      _numberOfCells(parallelTopologyService.getLocalNumberOfCells()),
      _indexOffset(tarch::la::Vector<MD_DIM, unsigned int>(1)),
      _totalNumberOfCells(_numberOfCells + 2u * _indexOffset)
#if (MD_DIM > 2)
      ,
      _totalNumberOfCells_X_By_totalNumberOfCells_Y(_totalNumberOfCells[0] *
                                                    _totalNumberOfCells[1])
#endif
      {
  ...
  simplemd::moleculemappings::UpdateLinkedCellListsMapping
      updateLinkedCellLists(parallelTopologyService, *this);
...
void simplemd::services::LinkedCellService::initCellStructure() {
  ...
  if (_cells == NULL) {
    std::cout
        << "ERROR simplemd::services::LinkedCellService::initCellStructure(): "
           "_cells==NULL!" << std::endl;
    exit(EXIT_FAILURE);
  }
}
...
unsigned int
simplemd::services::LinkedCellService::getLocalIndexFromLocalVector(
...
const tarch::la::Vector<MD_DIM, double> &
simplemd::services::LinkedCellService::getMeshWidth() const {
  return _meshWidth;
}

const tarch::la::Vector<MD_DIM, double> &
simplemd::services::LinkedCellService::getLocalDomainOffset() const {
  return _domainOffset;
}

const tarch::la::Vector<MD_DIM, double> &
simplemd::services::LinkedCellService::getLocalDomainSize() const {
  return _domainSize;
}

void simplemd::services::LinkedCellService::addMoleculeToLinkedCell(
    Molecule &molecule,
  ...
  for (unsigned int d = 0; d < MD_DIM; d++) {
    if (localCellIndex[d] >= _totalNumberOfCells[d]) {
      std::cout << "ERROR "
                   "simplemd::services::LinkedCellService::addMoleculeToLinkedC"
                   "ell: localCellIndex out of range!" << std::endl;
      std::cout << "Cell index=" << localCellIndex << "; "
                << molecule.getConstPosition() << "; "
                << molecule.getConstVelocity() << ";"
                << molecule.getConstForce() << ";"
...
void simplemd::services::LinkedCellService::addMoleculeToLinkedCell(
    Molecule &molecule, const unsigned int &localCellIndex) {
  _cells[localCellIndex].addMolecule(&molecule);
}

simplemd::LinkedCell &simplemd::services::LinkedCellService::getLinkedCell(
    const tarch::la::Vector<MD_DIM, unsigned int> &localCellIndex) {
#if (MD_DEBUG == MD_YES)
  for (unsigned int d = 0; d < MD_DIM; d++) {
    if (localCellIndex[d] >= _totalNumberOfCells[d]) {
      std::cout
          << "ERROR simplemd::services::LinkedCellService::getLinkedCell: "
...
bool simplemd::services::LinkedCellService::isGhostCell(
    const unsigned int &cellIndex) const {
...
simplemd::services::LinkedCellService::getLocalCellIndexVector(
    const unsigned int cellIndex) const {
  unsigned int help = cellIndex;
...
unsigned int simplemd::services::LinkedCellService::getLocalCellIndex(
