...
#endif
{
  ...
  #endif
  if (_rank < 0){
    std::cout << "ERROR simplemd::services::ParallelTopologyService::init: Rank < 0!" << std::endl;
    exit(EXIT_FAILURE);
  }
  ...
  help = help - _processCoordinates[2]*(_numberProcesses[0]*_numberProcesses[1]);
  #endif
  #if (MD_DIM > 1)
  ...
  _boundary = computeLocalBoundaryInformation(boundary,_processCoordinates,_numberProcesses);
  // determine ranks of possible communication neighbours
  createNeighbourRanks(_processCoordinates,_boundary,_numberProcesses,_neighbourRanks,_neighbourRanksUnique,_numUniqueNeighbours, _numberOfCellsPerBuffer);
  // compute global index of first cell
  for (unsigned int d = 0; d < MD_DIM; d++){
    _globalIndexOfFirstCell[d] = _processCoordinates[d]*_localNumberOfCells[d];
  }
  ...
  #if (MD_DEBUG == MD_YES)
    std::cout << "ParallelTopologyService: Rank=" << _rank << ", process coords: " << _processCoordinates << std::endl;
    std::cout << "No. processes: " << _numberProcesses << ", boundary: " << _boundary << std::endl;
    ...
    std::cout<< std::endl;
    std::cout<< "Rank " << _rank << " will be sending the following numbers of cells: ";
    for (unsigned int i = 0; i < _numUniqueNeighbours; i++) {
...
void simplemd::services::ParallelTopologyService::initBuffers(
  ...
  double averageNumberOfMoleculesPerLinkedCell = (double)localNumberOfMolecules / (double)totalNumberOfLocalCells;
...
std::vector<tarch::la::Vector<MD_DIM,unsigned int> > simplemd::services::ParallelTopologyService::
broadcastInnerCellViaBuffer(LinkedCell& cell, const unsigned int& cellIndex,const simplemd::services::LinkedCellService& linkedCellService){
  ...
  cellCoords[0] = help;
  #if (MD_DEBUG==MD_YES)
  std::cout << "broadcastInnerCellViaBuffers: rank " << _rank << " cellCoords=" << cellCoords << std::endl;
  #endif
  ...
  neighbours[4]  = tarch::la::Vector<MD_DIM,int>(cellCoords[0],cellCoords[1],0);
  ...
  neighbours[10] = tarch::la::Vector<MD_DIM,int>(cellCoords[0],0,cellCoords[2]);
  neighbours[11] = tarch::la::Vector<MD_DIM,int>(size[0]-1,0,cellCoords[2]);

  neighbours[12] = tarch::la::Vector<MD_DIM,int>(0,        cellCoords[1],cellCoords[2]);

  neighbours[13] = tarch::la::Vector<MD_DIM,int>(size[0]-1,cellCoords[1],cellCoords[2]);

  neighbours[14] = tarch::la::Vector<MD_DIM,int>(0             ,size[1]-1,cellCoords[2]);
  neighbours[15] = tarch::la::Vector<MD_DIM,int>(cellCoords[0] ,size[1]-1,cellCoords[2]);
  neighbours[16] = tarch::la::Vector<MD_DIM,int>(size[0]-1     ,size[1]-1,cellCoords[2]);

  neighbours[17] = tarch::la::Vector<MD_DIM,int>(0,0,size[2]-1);
  neighbours[18] = tarch::la::Vector<MD_DIM,int>(cellCoords[0],0,size[2]-1);
  neighbours[19] = tarch::la::Vector<MD_DIM,int>(size[0]-1,0,size[2]-1);

  neighbours[20] = tarch::la::Vector<MD_DIM,int>(0,            cellCoords[1],size[2]-1);
  neighbours[21] = tarch::la::Vector<MD_DIM,int>(cellCoords[0],cellCoords[1],size[2]-1);
  neighbours[22] = tarch::la::Vector<MD_DIM,int>(size[0]-1,    cellCoords[1],size[2]-1);

  neighbours[23] = tarch::la::Vector<MD_DIM,int>(0             ,size[1]-1,size[2]-1);
  neighbours[24] = tarch::la::Vector<MD_DIM,int>(cellCoords[0] ,size[1]-1,size[2]-1);
  neighbours[25] = tarch::la::Vector<MD_DIM,int>(size[0]-1     ,size[1]-1,size[2]-1);
  #endif
      ...
      for (int x = -1; x < 2; x++){
        if ( !( (x==0)
          ...
          if (isNearBoundary && ( (_boundary[help] == PERIODIC_BOUNDARY) || (_boundary[help] == PARALLEL_BOUNDARY) ) ){
            ...
            #endif
            int neighbourRank = ( (x + ((int)_processCoordinates[0]) + ((int)_numberProcesses[0]))%_numberProcesses[0] )
              #if (MD_DIM>1)
              + ( (y + ((int)_processCoordinates[1]) + ((int)_numberProcesses[1]) )%_numberProcesses[1] )*_numberProcesses[0]
              #endif
              #if (MD_DIM>2)
              + ( (z + ((int)_processCoordinates[2]) + ((int)_numberProcesses[2]) )%_numberProcesses[2] )*_numberProcesses[0]*_numberProcesses[1]
              #endif
              ;
            // if this is not the identity rank, do sending
            #if (MD_DEBUG==MD_YES)
            std::cout << "NeighbourRank: " << neighbourRank << " this rank: " << _rank << std::endl;
            #endif
            ...
            // if this cell is a ghost cell and periodic boundary condition needs to be handled locally:
              ...
              for (std::list<Molecule*>::iterator it = cell.begin(); it != cell.end(); it++){
                ...
                #if (MD_DEBUG==MD_YES)
                std::cout << "Put molecule at position " << position << ", velocity " << (*it)->getVelocity() << ", forceOld" << (*it)->getForceOld() << " in local buffer" << std::endl;
                #endif
                // set correct position and push molecule
            ...
            // if this boundary is located on the same process:
              ...
              #if (MD_DEBUG==MD_YES)
              std::cout << "Serial neighbour found: Ghost cell coordinate: " << serialNeighbour << std::endl;
              #endif
...
bool simplemd::services::ParallelTopologyService::
reduceGhostCellViaBuffer(LinkedCell& cell, const unsigned int& cellIndex,const simplemd::services::LinkedCellService &linkedCellService){
#if (MD_PARALLEL==MD_YES)
  ...
  help = help - ((unsigned int) cellCoords)*(
         (   linkedCellService.getLocalNumberOfCells()[1]+2*linkedCellService.getLocalIndexOfFirstCell()[1])*
         (   linkedCellService.getLocalNumberOfCells()[0]+2*linkedCellService.getLocalIndexOfFirstCell()[0]) );
  if (cellCoords == 0){
    cellCoords = -1;
  ...
  cellCoords = (((int)(_processCoordinates[1]+_numberProcesses[1]))+cellCoords)%_numberProcesses[1];
  ...
  cellCoords = ( ((int)(_processCoordinates[0]+_numberProcesses[0]))+cellCoords)%_numberProcesses[0];
...
void simplemd::services::ParallelTopologyService::unpackLocalBuffer(simplemd::services::MoleculeService& moleculeService,simplemd::services::LinkedCellService &linkedCellService){
...
void simplemd::services::ParallelTopologyService::communicationSteps_1_2(){
    ...
    for(unsigned int bufferIndex = 0; bufferIndex < _numUniqueNeighbours; bufferIndex++) {
      bufferIrecv(_bufferService.getReceiveBuffer(bufferIndex), _neighbourRanksUnique[bufferIndex], _receiveRequests[bufferIndex]);
    }

    //issue all Isend calls
    for(unsigned int bufferIndex = 0; bufferIndex < _numUniqueNeighbours; bufferIndex++) {
      bufferIsend(_bufferService.getSendBuffer(bufferIndex), _neighbourRanksUnique[bufferIndex], _sendRequests[bufferIndex]);
    }
  #endif
}

void simplemd::services::ParallelTopologyService::communicationSteps_3_4(simplemd::services::MoleculeService& moleculeService,simplemd::services::LinkedCellService &linkedCellService){
  #if (MD_PARALLEL==MD_YES)
      ...
      //test send request
        if(!sendCompleted[i_buf]) {
          ...
          if(result != MPI_SUCCESS) {
            std::cout << "rank " << _rank << " was not able to test send request " << i_buf << std::endl;
            exit(EXIT_FAILURE);
          }
          ...
          if(flag == 1) {
            ...
            #if (MD_DEBUG == MD_YES)
              std::cout << "Rank " << _rank << " sent " << _bufferService.getSendBufferLength(i_buf) / MD_DIM / 3 << " molecules to " << _neighbourRanksUnique[i_buf] << std::endl;
            #endif
          }
        }
        ...
        if(!recvCompleted[i_buf]) {
          ...
          if(result != MPI_SUCCESS) {
            std::cout << "rank " << _rank << " was not able to test receive request " << i_buf << std::endl;
            exit(EXIT_FAILURE);
          }
          // if flag == 1, this means message request has just been completed
          if(flag == 1) {
...
bool simplemd::services::ParallelTopologyService::globalToLocalRegionOfInterest(
  ...
  for(unsigned int d = 0; d < MD_DIM; d++) {
    ...
    help = (int)globalStartCell[d] - (int)(_processCoordinates[d] * _localNumberOfCells[d]);
    ...
    if (help == 0 && _processCoordinates[d] != 0 ) help = 1; 
    ...
    //compute end point
    help = (int)globalEndCell[d] - (int)(_processCoordinates[d] * _localNumberOfCells[d]);
    // first, set values larger than the number of cells along the dimension +1 to the number of cells +1 (+1 is needed because of ghost layer)
    ...
    if ((help == (int)(_localNumberOfCells[d] + 1)) && _processCoordinates[d] != (_numberProcesses[d] -1)) help = _localNumberOfCells[d]; 
    // this is now the correct value in the local frame of reference.
...
tarch::la::Vector<MD_DIM, unsigned int> simplemd::services::ParallelTopologyService::localToGlobalCellIndexVector(
        ...
        if ( !(   (x == 0)
          ...
          if ( (isBoundary && (localBoundary[neighbourCounter]==simplemd::PERIODIC_BOUNDARY)) || (!isBoundary) ){
            ...
            if (tmpRank != _rank){
              ...
              addNeighbourToNeighbourRanksUnique(neighbourRanksUnique, numUniqueNeighbours, tmpRank);
...
void simplemd::services::ParallelTopologyService::addNeighbourToNeighbourRanksUnique(
  std::vector<int> &neighbourRanksUnique,
  unsigned int &numUniqueNeighbours,
...
tarch::la::Vector<MD_DIM,double> simplemd::services::ParallelTopologyService::computeMeshwidth(
  ...
  for (unsigned int d = 0; d < MD_DIM; d++){
    ...
    unsigned int cells = static_cast<unsigned int> (floor((domainSize[d]/numberProcesses[d])/prescribedWidth[d]+0.5));
...
tarch::la::Vector<MD_DIM,unsigned int> simplemd::services::ParallelTopologyService::computeNumberOfCells(
  const tarch::la::Vector<MD_DIM,double> &meshWidth,
  const tarch::la::Vector<MD_DIM,unsigned int> &numberProcesses,
  const tarch::la::Vector<MD_DIM,double> &domainSize
) const {
  ...
  for (unsigned int d = 0; d < MD_DIM; d++){
    // compute approx. number of cells in d-direction
    unsigned int cells = static_cast<unsigned int> (floor((domainSize[d]/numberProcesses[d])/meshWidth[d]+0.5));
    // determine correct mesh width in d-direction when using "cells" cells
    double tmpWidth = (domainSize[d]/numberProcesses[d])/cells;
    // compute real number of cells
    numberOfCells[d] = static_cast<unsigned int>(floor((domainSize[d]/numberProcesses[d])/tmpWidth+0.5));
  }
  #if (MD_DEBUG==MD_YES)
...
tarch::la::Vector<MD_DIM,unsigned int> simplemd::services::ParallelTopologyService::computeGlobalNumberOfCells(
  const tarch::la::Vector<MD_DIM,double> &meshWidth,
  const tarch::la::Vector<MD_DIM,unsigned int> &numberProcesses,
  const tarch::la::Vector<MD_DIM,double> &domainSize
) const {
  tarch::la::Vector<MD_DIM,unsigned int> globalCells = computeNumberOfCells(meshWidth,numberProcesses,domainSize);
  for (unsigned int d = 0; d < MD_DIM; d++){ globalCells[d] *= numberProcesses[d]; }
  return globalCells;
}
...
#if (MD_DIM==1)
  for (int x = ((int)processCoordinates[0])-1; x < ((int)processCoordinates[0])+2; x++){
...
#elif (MD_DIM==2)
  for (int y = ((int)processCoordinates[1])-1; y < ((int)processCoordinates[1])+2; y++){
    for (int x = ((int)processCoordinates[0])-1; x < ((int)processCoordinates[0])+2; x++){
      if ( !( (y==(int)processCoordinates[1]) && (x==(int)processCoordinates[0]) ) ){
        ...
        if ( (nextProcess[0]==-1) && (nextProcess[1]==-1) ){
          localBoundary[counter] = boundary[0];
        // lower right corner
        } else if ( (nextProcess[0]==(int)numberProcesses[0]) && (nextProcess[1]==-1) ){
          localBoundary[counter] = boundary[2];
        // upper left corner
        } else if ( (nextProcess[0]==-1) && (nextProcess[1]==(int)numberProcesses[1]) ){
          localBoundary[counter] = boundary[5];
        } else if ( (nextProcess[0]==(int)numberProcesses[0]) && (nextProcess[1]==(int)numberProcesses[1]) ){
...
#elif (MD_DIM==3)
  for (int z = ((int)processCoordinates[2])-1; z < ((int)processCoordinates[2])+2; z++){
    for (int y = ((int)processCoordinates[1])-1; y < ((int)processCoordinates[1])+2; y++){
      for (int x = ((int)processCoordinates[0])-1; x < ((int)processCoordinates[0])+2; x++){
        if ( !( (z==(int)processCoordinates[2]) && (y==(int)processCoordinates[1]) && (x==(int)processCoordinates[0]) ) ){
          ...
          if ( (nextProcess[0]==-1) && (nextProcess[1]==-1) && (nextProcess[2]==-1) ){
            localBoundary[counter] = boundary[0];
          // lower right front corner
          } else if ( (nextProcess[0]==(int)numberProcesses[0]) && (nextProcess[1]==-1) && (nextProcess[2]==-1) ){
            localBoundary[counter] = boundary[2];
          // lower left back corner
          } else if ( (nextProcess[0]==-1) && (nextProcess[1]==(int)numberProcesses[1]) && (nextProcess[2]==-1) ){
            localBoundary[counter] = boundary[6];
          // lower right back corner
          } else if ( (nextProcess[0]==(int)numberProcesses[0]) && (nextProcess[1]==(int)numberProcesses[1]) && (nextProcess[2]==-1) ){
            localBoundary[counter] = boundary[8];
          // upper left front corner
          } else if ( (nextProcess[0]==-1) && (nextProcess[1]==-1) && (nextProcess[2]==(int)numberProcesses[2]) ){
            localBoundary[counter] = boundary[17];
          // upper right front corner
          } else if ( (nextProcess[0]==(int)numberProcesses[0]) && (nextProcess[1]==-1) && (nextProcess[2]==(int)numberProcesses[2]) ){
            localBoundary[counter] = boundary[19];
          // upper left back corner
          } else if ( (nextProcess[0]==-1) && (nextProcess[1]==(int)numberProcesses[1]) && (nextProcess[2]==(int)numberProcesses[2]) ){
            localBoundary[counter] = boundary[23];
          // upper right back corner
          } else if ( (nextProcess[0]==(int)numberProcesses[0]) && (nextProcess[1]==(int)numberProcesses[1]) && (nextProcess[2]==(int)numberProcesses[2]) ){
          ...
          // lower left edge
            ...
            localBoundary[counter] = boundary[3];
          // lower right edge
          } else if ( (nextProcess[0]==(int)numberProcesses[0]) && (nextProcess[2]==-1)){
            localBoundary[counter] = boundary[5];
          // upper left edge
          } else if ( (nextProcess[0]==-1) && (nextProcess[2]==(int)numberProcesses[2]) ){
            localBoundary[counter] = boundary[20];
          // upper right edge
          } else if ( (nextProcess[0]==(int)numberProcesses[0]) && (nextProcess[2]==(int)numberProcesses[2]) ){
          ...
          // lower front edge
            ...
            localBoundary[counter] = boundary[1];
          // lower back edge
          } else if ( (nextProcess[1]==(int)numberProcesses[1]) && (nextProcess[2]==-1)){
            localBoundary[counter] = boundary[7];
          // upper front edge
          } else if ( (nextProcess[1]==-1) && (nextProcess[2]==(int)numberProcesses[2]) ){
            localBoundary[counter] = boundary[18];
          // upper back edge
          } else if ( (nextProcess[1]==(int)numberProcesses[1]) && (nextProcess[2]==(int)numberProcesses[2]) ){
...
getNumberOfTransferredCells(const int &x
#if (MD_DIM>1)
,const int &y
#endif
#if (MD_DIM>2)
...
void simplemd::services::ParallelTopologyService::
adaptPositionForPeriodicBoundaries(
...
unsigned int simplemd::services::ParallelTopologyService::getCurrentBufferIndexFromNeighbourRank(
  const int & neighbourRank) const 
{
  ...
  const tarch::la::Vector<MD_DIM,double> & pos) 
{
  ...
  isOk =
   ...
   _bufferService.pushMoleculeToLocalBuffer(pos, mol);
    ...
    if(!isOk) {
      std::cout << "Rank " << _rank << " was unable to push molecule to local buffer. " << std::endl;
      exit(EXIT_FAILURE);
    }
  #endif
  
  #if (MD_DEBUG == MD_YES)
    std::cout << "rank " << _rank << " pushed molecule to local buffer successfully, buffer length is " << _bufferService.getLocalBuffer()->getLength() << std::endl;
    std::cout << "pushed molecule with position " << pos << ", velocity " << mol->getConstVelocity() << ", forceOld" << mol->getConstForceOld() << std::endl;
  #endif
}
  ...
  isOk =
   ...
   _bufferService.pushMoleculeToSendBuffer(pos, mol, bufferIndex);
    ...
    if(!isOk) {
      ...
      std::cout << "Upper bound of buffer (" << _bufferService.getBufferCapacity(bufferIndex) <<") exceeded. Fatal error. Please contact developers as this is a principal issue." << std::endl;
      exit(EXIT_FAILURE);
    }
  #endif
  
  #if (MD_DEBUG == MD_YES)
    std::cout << "Rank " << _rank << " pushed molecule to send buffer " << bufferIndex << " successfully, buffer length is " << _bufferService.getSendBufferLength(bufferIndex) << std::endl;
    std::cout << "pushed molecule at position " << pos << ", velocity " << mol->getConstVelocity() << ", forceOld" << mol->getConstForceOld() << std::endl;
  #endif
}
#endif
  ...
  MPI_Request& request) const 
{
    ...
    result = MPI_Isend(
       buffer->getValues(), buffer->getLength(), MPI_DOUBLE, neighbourRank,
       _bufferTag, _communicator, &request);

    if  (result!=MPI_SUCCESS) {
      std::cout << "rank " << _rank << " was not able to Isend buffer to " << neighbourRank << ": " << std::endl;
      //std::cout << tarch::parallel::MPIReturnValueToString(result);
      exit(EXIT_FAILURE);
    }

    #if (MD_DEBUG == MD_YES)
      std::cout << "Rank " << _rank << " is sending " << buffer->getLength() /3 /MD_DIM << " molecules to " << neighbourRank << std::endl;
    #endif
}
#endif
  ...
  MPI_Request& request) const 
{
    ...
    result = MPI_Irecv(
       buffer->getValues(), buffer->getCapacity(), MPI_DOUBLE, neighbourRank,
       _bufferTag, _communicator, &request);

    if  (result!=MPI_SUCCESS) {
      std::cout << "rank " << _rank << " was not able to Irecv buffer from " << neighbourRank << ": " << std::endl;
      exit(EXIT_FAILURE);
    }

    #if (MD_DEBUG == MD_YES)
      std::cout << "Rank " << _rank << " is receiving " << buffer->getLength() /3 /MD_DIM << " molecules from " << neighbourRank << std::endl;
    #endif
}
#endif
