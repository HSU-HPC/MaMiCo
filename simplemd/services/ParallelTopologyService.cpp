...
#endif
            int neighbourRank =
                ((x + ((int)_processCoordinates[0]) + ((int)_numberProcesses[0])) % _numberProcesses[0])
#if (MD_DIM > 1)
...
#if (MD_DEBUG == MD_YES)
          std::cout << "Rank " << _rank << " sent " << _bufferService.getSendBufferLength(i_buf) / MD_DIM / 3 << " molecules to "
                    << _neighbourRanksUnique[i_buf] << std::endl;
#endif
        }
      }
...
unsigned int simplemd::services::ParallelTopologyService::getNumberOfTransferredCells(const int &x
  ...
  if ((sum != 1) && (sum != 2)) {
    std::cout << "ERROR "
                 "simplemd::services::ParallelTopologyService::getNumberOfTrans"
                 "ferredCells: Unknown neighbour relation!"
              << std::endl;
    exit(EXIT_FAILURE);
  }
#endif
  ...
  } else if ((x == 0) && (((y == -1) && (z == -1)) || ((y == -1) && (z == 1)) || ((y == 1) && (z == -1)) || ((y == 1) && (z == 1)))) {
    return _localNumberOfCells[0];
    // y-axis aligned edges
  } else if ((y == 0) && (((x == -1) && (z == -1)) || ((x == -1) && (z == 1)) || ((x == 1) && (z == -1)) || ((x == 1) && (z == 1)))) {
    return _localNumberOfCells[1];
    // z-axis aligned edges
  } else if ((z == 0) && (((y == -1) && (x == -1)) || ((y == -1) && (x == 1)) || ((y == 1) && (x == -1)) || ((y == 1) && (x == 1)))) {
...
unsigned int simplemd::services::ParallelTopologyService::getCurrentBufferIndexFromNeighbourRank(const int &neighbourRank) const {
  ...
  if (!found) {
    std::cout << "Problems: "
                 ...
                 "urRank() no corresponding index found"
              << std::endl;
    exit(EXIT_FAILURE);
    return 0;
  } else {
...
#if (MD_DEBUG == MD_YES)
  std::cout << "rank " << _rank << " pushed molecule to local buffer successfully, buffer length is " << _bufferService.getLocalBuffer()->getLength()
            << std::endl;
...
#if (MD_DEBUG == MD_YES)
  std::cout << "Rank " << _rank << " pushed molecule to send buffer " << bufferIndex << " successfully, buffer length is "
...
void simplemd::services::ParallelTopologyService::bufferIsend(ParallelAndLocalBufferService::SimpleBuffer *buffer, const int &neighbourRank,
  ...
  result = MPI_Isend(buffer->getValues(), buffer->getLength(), MPI_DOUBLE, neighbourRank, _bufferTag, _communicator, &request);
...
void simplemd::services::ParallelTopologyService::bufferIrecv(ParallelAndLocalBufferService::SimpleBuffer *buffer, const int &neighbourRank,
  ...
  result = MPI_Irecv(buffer->getValues(), buffer->getCapacity(), MPI_DOUBLE, neighbourRank, _bufferTag, _communicator, &request);
