...
public:
  ...
  bool pushMoleculeToLocalBuffer(
...
private:
  ...
  /** method that calculates upper bound on number of molecules to be stored in
    ...
    * Determined from average number of molecules per linked cell, number of
    * cells to be transferred
    * via respective buffer and a special function A:
    * upperBound[bufferIndex] = ceil(numCells[bufferIndex] * avMolPerCell *
    * A(numCells[bufferIndex]))
    *
    ...
    * average number of molecules is exceeded is
    * high, whereas, when we send thousands of cells, the probability that the
    * average number of molecules is exceeded in all
    * of them is much lower. We also need to keep buffer size low,  so we choose
    * the function A to satisfy the following:
    * 1. A: N -> R
