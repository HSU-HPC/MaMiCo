...
class simplemd::services::LinkedCellService {
  public:
    ...
    /** puts the molecule into the cell defined by the local index (vector) coordinates localCellIndex */
    ...
    /** iterates over all cells in the range defined by the lower left front corner cell lowerLeftFrontCell
     *  and the size of the domain cellRange. cellRange defines a number of cells in each spatial direction that
     *  the class A shall be applied to. lowerLeftFrontCell needs to be given in local coordinates.
     */
    template<class A>
    void iterateCells(
      A &a,
      const tarch::la::Vector<MD_DIM,unsigned int>& lowerLeftFrontCell,
    ...
    template<class A>
    void iterateCells(A &a,const bool& useOpenMP);

    /** iterates over all cell pairs for the cells in the inner region of each local process */
    template<class A>
    ...
    /** iterates over all cell pairs cell1 and cell2 with cell1 in the range described by lowerLeftFrontCell and cellRange;
     *  cell2 does not need to lie within the range (example: iterate only over lowerLeftFrontCell=(1,1,1) and cellRange=(1,1,1).
    ...
    /** returns true if the local cell index cellIndex describes a linked cell within the ghost layer */
  ...
  private:
     ...
     *  within the local cell structure; the grid has a total of numberOfCells cells.
     *  globalIndexFirstCell denotes the global index of the lower left cell of the simulation.
    ...
    tarch::la::Vector<MD_DIM,double> getMeshwidth(const tarch::la::Vector<MD_DIM,double>& domainSize, const tarch::la::Vector<MD_DIM,unsigned int>& localNumberCells)
    const{
    ...
    /** index of first cell under consideration. It is 1,1,1, due to a ghost cell layer
     *  around the domain.
     */
    ...
    /** _totalNumberOfCells(0)*_totalNumberOfCells(1); only stored for performance reasons */
    #if (MD_DIM>2)
...
void simplemd::services::LinkedCellService::iterateCells(
  ...
  for (unsigned int d = 0; d < MD_DIM; d++){
    ...
    if (lowerLeftFrontCell[d]+cellRange[d] > 2*_indexOffset[d]+_numberOfCells[d]){
      std::cout << "ERROR simplemd::services::LinkedCellService::iterateCells(): defined Range does not fit into local sub-domain!" << std::endl;
      exit(EXIT_FAILURE);
    }
  }
#endif
  ...
  if (useOpenMP){
    const tarch::la::Vector<MD_DIM,unsigned int> size(
      simplemd::services::LinkedCellService::getInstance().getLocalNumberOfCells()
...
void simplemd::services::LinkedCellService::iterateCellPairs(
  ...
  if (useOpenMP){
    ...
    for (unsigned int x=0; x < 2; x++){
      ...
      // For odd block sizes, we need to do some more work in the x/y/z==0-traversals.
      // The second x/y/z==1-traversals are reduced by the normal integer-rounding in this case.
...
template<class A>
void simplemd::services::LinkedCellService::iterateCellPairs(A &a,const bool& useOpenMP) const{
