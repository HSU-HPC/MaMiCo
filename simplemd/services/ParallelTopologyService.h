...
public:
  ...
  getLocalBoundaryInformation() const {
    return _boundary;
  }
  ...
  const tarch::la::Vector<MD_DIM, unsigned int> &
  getGlobalIndexOfFirstCell() const {
    return _globalIndexOfFirstCell;
  }
  ...
  /** sends all molecules from cell cellIndex to the respective neighbouring
    ...
    * The MPI send calls are executed later (from within BoundaryTreatment).
    */
  bool reduceGhostCellViaBuffer(
      LinkedCell &cell, const unsigned int &cellIndex,
      const simplemd::services::LinkedCellService &linkedCellService);

  /** unpack and resort local buffer
    */
  void unpackLocalBuffer(
  ...
  tarch::la::Vector<MD_DIM, unsigned int>
      localToGlobalCellIndexVector(const tarch::la::Vector<
          MD_DIM, unsigned int> &localCellIndexVector) const;

#if (MD_PARALLEL == MD_YES)
  int MD_Allreduce(void *sendbuf, void *recvbuf, int count,
...
private:
  ...
  void createNeighbourRanks(
      ...
      const tarch::la::Vector<MD_LINKED_CELL_NEIGHBOURS,
                              simplemd::BoundaryType> &localBoundary,
      const tarch::la::Vector<MD_DIM, unsigned int> &numberProcesses,
  ...
  /** if addedNeighbour is not already present in neighbourRanksUnique,
    ...
    */
  void
      addNeighbourToNeighbourRanksUnique(std::vector<int> &neighbourRanksUnique,
                                         unsigned int &numUniqueNeighbours,
...
* linked cells.
    */
  void unpackBuffer(ParallelAndLocalBufferService::SimpleBuffer *buf,
                    simplemd::services::MoleculeService &moleculeService,
                    simplemd::services::LinkedCellService &linkedCellService);
...
* respective local buffer.
    */
  void pushMoleculeToLocalBuffer(const Molecule *mol,
...
* respective sendBuffer.
    */
  void pushMoleculeToSendBuffer(const unsigned int &bufferIndex,
                                const Molecule *mol,
                                const tarch::la::Vector<MD_DIM, double> &pos);
  ...
  /** Issues an Isend call on buffer.
    ...
    * request
    */
  void bufferIsend(ParallelAndLocalBufferService::SimpleBuffer *buffer,
  ...
  /** Issues an Irecv call on buffer.
    ...
    * request
    */
  void bufferIrecv(ParallelAndLocalBufferService::SimpleBuffer *buffer,
                   const int &neighbourRank, MPI_Request &request) const;
#endif
