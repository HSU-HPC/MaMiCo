...
void simplemd::configurations::ProfilePlotterConfiguration::parseSubtag( tinyxml2::XMLElement* node ){
  ...
  for (unsigned int d = 0; d < MD_DIM; d++){
    if (buf[d] < 0){
      std::cout << "ERROR simplemd::configurations::ProfilePlotterConfiguration::parseSubtag: " << START_CELL.c_str() << " (entry " << d << ") is smaller than zero!" << std::endl;
      exit(EXIT_FAILURE);
      _isValid = false;
    }
  ...
  for (unsigned int d = 0; d < MD_DIM; d++){
    if (buf[d] < 0){
      std::cout << "ERROR simplemd::configurations::ProfilePlotterConfiguration::parseSubtag: " << RANGE.c_str() << " (entry " << d << ") is smaller than zero!" << std::endl;
      exit(EXIT_FAILURE);
      _isValid = false;
    }
  ...
  if (intBuf < 0){
    std::cout << "ERROR simplemd::configurations::ProfilePlotterConfiguration::parseSubtag: " << WRITE_EVERY_TIMESTEP.c_str() << " is smaller than zero!" << std::endl;
    exit(EXIT_FAILURE);
    _isValid = false;
  }
  ...
  if (intBuf < 0){
    std::cout << "ERROR simplemd::configurations::ProfilePlotterConfiguration::parseSubtag: " << SAMPLE_EVERY_TIMESTEP.c_str() << " is smaller than zero!" << std::endl;
    exit(EXIT_FAILURE);
    _isValid = false;
  }
  ...
  if (intBuf < 0){
    std::cout << "ERROR simplemd::configurations::ProfilePlotterConfiguration::parseSubtag: " << START_AT_TIMESTEP.c_str() << " is smaller than zero!" << std::endl;
    exit(EXIT_FAILURE);
    _isValid = false;
  }
