...
class CouplingCellsWithLinkedCellsTest : public CppUnit::TestFixture {

  CPPUNIT_TEST_SUITE(CouplingCellsWithLinkedCellsTest);
...
private:
  ...
  public:
    ...
    virtual simplemd::LinkedCell& getLinkedCell(const tarch::la::Vector<3, unsigned int>& macroscopicCellIndex,
    ...
    virtual coupling::interface::MoleculeIterator<simplemd::LinkedCell, 3>* getMoleculeIterator(simplemd::LinkedCell& cell) {
      return NULL;
    }
  ...
  public:
    ...
    void apply(coupling::datastructures::MacroscopicCellWithLinkedCells<simplemd::LinkedCell, 3>& cell, const unsigned int& index) {
      _applicationCount++;
  ...
  private:
     ...
     */
    int getLayer(unsigned int index) {
      CellIndex<3, IndexTrait::local> idx{index};
      if (_local)
        return getLayer(CellIndex<3, IndexTrait::vector, IndexTrait::local>{idx}.get(),
                        CellIndex<3, IndexTrait::vector, IndexTrait::local>::numberCellsInDomain);
      else
        return getLayer(CellIndex<3, IndexTrait::vector>{idx}.get(), CellIndex<3, IndexTrait::vector>::numberCellsInDomain);
    }
...
public:
  void setUp() {
    ...
    _indexConversion = new coupling::IndexConversion<3>(globalNumberCells, numberProcesses, _rank, _testInterface->getGlobalMDDomainSize(),
                                                        _testInterface->getGlobalMDDomainOffset(), coupling::paralleltopology::XYZ);
    IndexingService<3>::getInstance().init(globalNumberCells, numberProcesses, coupling::paralleltopology::XYZ, 3, (unsigned int)_rank);
    const tarch::la::Vector<3, unsigned int> numberLinkedCellsPerMacroscopicCell(2);
    coupling::datastructures::MacroscopicCells<simplemd::LinkedCell, 3> macroscopicCells(numberLinkedCellsPerMacroscopicCell, *_indexConversion,
                                                                                         _testInterface);
    _cells = std::make_unique<coupling::datastructures::MacroscopicCells<simplemd::LinkedCell, 3>>(numberLinkedCellsPerMacroscopicCell, *_indexConversion,
                                                                                                   _testInterface);
  }
  ...
  void testApplyToLocalNonGhostCouplingCellsWithLinkedCells() {
    ...
    _cells->applyToLocalNonGhostMacroscopicCellsWithLinkedCells(check);
    CPPUNIT_ASSERT_EQUAL(check.getApplicationCount(), 12 * 12 * 12 / _size);
  }

  void testApplyToLocalGhostCouplingCellsWithLinkedCells() {
    LayerChecker check{true, [](int layer) { return layer == 0; }};
    _cells->applyToLocalGhostMacroscopicCellsWithLinkedCells(check);
  ...
  void testApplyToAllLocalCouplingCellsWithLinkedCells() {
    ...
    _cells->applyToAllLocalMacroscopicCellsWithLinkedCells(check);
...
private:
  int _size, _rank;
  std::unique_ptr<coupling::datastructures::MacroscopicCells<simplemd::LinkedCell, 3>> _cells;
  TestMDSolverInterface* _testInterface;
  coupling::IndexConversion<3>* _indexConversion;
};

CPPUNIT_TEST_SUITE_REGISTRATION(CouplingCellsWithLinkedCellsTest);