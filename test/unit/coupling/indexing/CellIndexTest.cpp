...
public:
  ...
  template <Testmode mode> void testAll() {
    ...
    for (_rank = 0; _rank < 4; _rank++) {
      IndexingService<3>::getInstance().initWithCells({12}, {2, 2, 1}, coupling::paralleltopology::XYZ, 3, _rank);
      runtests<mode, T00>();
      runtests<mode, T01>();
      runtests<mode, T02>();
      runtests<mode, T03>();
  ...
  template <class T> void testLoop() {
    ...
    IndexingService<3>::getInstance().finalize();
    IndexingService<3>::getInstance().initWithCells({12}, {4, 1, 1}, coupling::paralleltopology::XYZ, 3, _rank);
