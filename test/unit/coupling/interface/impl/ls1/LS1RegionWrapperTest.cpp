...
public:
  ...
  void testPointIsInRegion() {
    ...
    for (int i = 0; i < numberOfPoints; i++) {
      ...
      for (int j = 0; j < 3; j++) {
        // cos theta = (a.b)/(|a||b|)
        isPointInRegion[i] &=
            (tarch::la::dot(regionCenter - lowerCornerFaceCentres[j], pointsToCheck[i] - lowerCornerFaceCentres[j]) /
             (tarch::la::norm2(regionCenter - lowerCornerFaceCentres[j]) * tarch::la::norm2(pointsToCheck[i] - lowerCornerFaceCentres[j]))) >= 0;
  ...
  void testRegionIsInRegion() {
    ...
    for (int i = 0; i < numberOfPoints; i++) {
      ...
      tarch::la::Vector<3, double> upper = {lower[0] + static_cast<double>(rand() % 500), lower[1] + static_cast<double>(rand() % 500),
                                            lower[2] + static_cast<double>(rand() % 500)};
    ...
    for (int i = 4; i < 8; i++) {
      for (int j = 4; j < 8; j++) {
        for (int k = 4; k < 8; k++) {
          ...
          ls1::LS1RegionWrapper fullDomainWrapper(_testSimulation->getEnsemble()->domain()->rmin(), _testSimulation->getEnsemble()->domain()->rmax(),
                                                  _testSimulation);
  ...
  void testResetIterator() {
    ...
    fullRegion.iteratorReset();
    CPPUNIT_ASSERT(firstParticle->r(0) == fullRegion.getParticleAtIterator()->r(0) && firstParticle->r(1) == fullRegion.getParticleAtIterator()->r(1) &&
