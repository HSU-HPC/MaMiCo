...
public:
  ...
  void tearDown() {
   ...
   void testGetProcessCoordinates3D() {
    ...
    coupling::paralleltopology::XYZTopology<3> topology{numberOfProcesses, topologyOffset};
    for (unsigned int rank = topologyOffset; rank < topologyOffset + nx*ny*nz; ++rank) {
      tarch::la::Vector<3, unsigned int> processCoordinates = topology.getProcessCoordinates(rank);
  ...
  void testGetProcessCoordinates2D() {
    ...
    coupling::paralleltopology::XYZTopology<2> topology{numberOfProcesses, topologyOffset};
    for (unsigned int rank = topologyOffset; rank < topologyOffset + nx*ny; ++rank) {
      tarch::la::Vector<2, unsigned int> processCoordinates = topology.getProcessCoordinates(rank);
    ...
    for (unsigned int k = 0; k < nz; ++k) {
      for (unsigned int j = 0; j < ny; ++j) {
        for (unsigned int i = 0; i < nx; ++i) {
          ...
          unsigned int rank = topology.getRank(processCoordinates);
