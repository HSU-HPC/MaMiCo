...
public:
  ...
  virtual void run() {
    ...
    // compute std dev
    for (int i = 0; i < numberExperiments; i++) {
      expStdDev = expStdDev + tarch::la::dot(velocities[i] - expAvg, velocities[i] - expAvg);
    }
    ...
    if (tarch::la::norm2(meanVelocity - expAvg) > tol) {
      std::cout << "ERROR TestLammpsInitialVelocity: Avg. velocity deviation exceeds " << tol << "!" << std::endl;
      exit(EXIT_FAILURE);
    }
    if (fabs(stdDev - expStdDev) > tol) {
      std::cout << "ERROR TestLammpsInitialVelocity: Std. dev. deviation exceeds " << tol << "!" << std::endl;
      exit(EXIT_FAILURE);
    }
  }
};
